...
//
// Table class for akun4
//
class cakun4 extends cTable {
	var $kel1;
	var $kel2;
	var $kel3;
	var $kel4;
	...
	function __construct() {
		...
		$this->TableVar = 'akun4';
		$this->TableName = 'akun4';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`akun4`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kel1
		$this->kel1 = new cField('akun4', 'akun4', 'x_kel1', 'kel1', '`kel1`', '`kel1`', 200, -1, FALSE, '`kel1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel1->Sortable = TRUE; // Allow sort
		$this->fields['kel1'] = &$this->kel1;

		// kel2
		$this->kel2 = new cField('akun4', 'akun4', 'x_kel2', 'kel2', '`kel2`', '`kel2`', 200, -1, FALSE, '`kel2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel2->Sortable = TRUE; // Allow sort
		$this->fields['kel2'] = &$this->kel2;

		// kel3
		$this->kel3 = new cField('akun4', 'akun4', 'x_kel3', 'kel3', '`kel3`', '`kel3`', 200, -1, FALSE, '`kel3`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel3->Sortable = TRUE; // Allow sort
		$this->fields['kel3'] = &$this->kel3;

		// kel4
		$this->kel4 = new cField('akun4', 'akun4', 'x_kel4', 'kel4', '`kel4`', '`kel4`', 200, -1, FALSE, '`kel4`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel4->Sortable = TRUE; // Allow sort
		$this->fields['kel4'] = &$this->kel4;

		// kel5
		$this->kel5 = new cField('akun4', 'akun4', 'x_kel5', 'kel5', '`kel5`', '`kel5`', 200, -1, FALSE, '`kel5`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel5->Sortable = TRUE; // Allow sort
		$this->fields['kel5'] = &$this->kel5;

		// nmkel4
		$this->nmkel4 = new cField('akun4', 'akun4', 'x_nmkel4', 'nmkel4', '`nmkel4`', '`nmkel4`', 200, -1, FALSE, '`nmkel4`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`akun4`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kel5', $rs))
				ew_AddFilter($where, ew_QuotedName('kel5', $this->DBID) . '=' . ew_QuotedValue($rs['kel5'], $this->kel5->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@kel5@", ew_AdjustSql($this->kel5->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("akun4view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("akun4view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "akun4add.php?" . $this->UrlParm($parm);
		else
			$url = "akun4add.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("akun4edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("akun4add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("akun4delete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kel5:" . ew_VarToJson($this->kel5->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kel5->CurrentValue)) {
			$sUrl .= "kel5=" . urlencode($this->kel5->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["kel5"]))
				$arKeys[] = ew_StripSlashes($_POST["kel5"]);
			elseif (isset($_GET["kel5"]))
				$arKeys[] = ew_StripSlashes($_GET["kel5"]);
			else
