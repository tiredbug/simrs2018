...
$vw_bill_ranap_detail_tindakan_lain_edit = NULL; // Initialize page object first

class cvw_bill_ranap_detail_tindakan_lain_edit extends cvw_bill_ranap_detail_tindakan_lain {
	...
	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_tindakan_lain_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("vw_bill_ranap_detail_tindakan_lainlist.php"));
			else
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("vw_bill_ranap_detail_tindakan_lainlist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("vw_bill_ranap_detail_tindakan_lainlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "vw_bill_ranap_detail_tindakan_lainlist.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("vw_bill_ranap_detail_tindakan_lainlist.php"), "", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($vw_bill_ranap_detail_tindakan_lain_edit)) $vw_bill_ranap_detail_tindakan_lain_edit = new cvw_bill_ranap_detail_tindakan_lain_edit();

// Page init
$vw_bill_ranap_detail_tindakan_lain_edit->Page_Init();

// Page main
$vw_bill_ranap_detail_tindakan_lain_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_bill_ranap_detail_tindakan_lain_edit->Page_Render();
?>
...
var CurrentForm = fvw_bill_ranap_detail_tindakan_lainedit = new ew_Form("fvw_bill_ranap_detail_tindakan_lainedit", "edit");

// Validate form
fvw_bill_ranap_detail_tindakan_lainedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tindakan_lain->id_admission->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_statusbayar");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tindakan_lain->statusbayar->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelas");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tindakan_lain->kelas->FldErrMsg()) ?>");
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tindakan_lain->kelompok_tindakan->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok1");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tindakan_lain->kelompok1->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok2");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tindakan_lain->kelompok2->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kode_dokter");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tindakan_lain->kode_dokter->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_no_ruang");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tindakan_lain->no_ruang->FldErrMsg()) ?>");
...
fvw_bill_ranap_detail_tindakan_lainedit.Form_CustomValidate = 
...
fvw_bill_ranap_detail_tindakan_lainedit.ValidateRequired = true;
<?php } else { ?>
fvw_bill_ranap_detail_tindakan_lainedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_bill_ranap_detail_tindakan_lainedit.Lists["x_kode_tindakan"] = {"LinkField":"x_kode","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama_tindakan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_bill_ranap_data_tarif_tindakan"};
...
<?php if (!$vw_bill_ranap_detail_tindakan_lain_edit->IsModal) { ?>
<?php } ?>
<?php $vw_bill_ranap_detail_tindakan_lain_edit->ShowPageHeader(); ?>
<?php
$vw_bill_ranap_detail_tindakan_lain_edit->ShowMessage();
?>
<form name="fvw_bill_ranap_detail_tindakan_lainedit" id="fvw_bill_ranap_detail_tindakan_lainedit" class="<?php echo $vw_bill_ranap_detail_tindakan_lain_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_bill_ranap_detail_tindakan_lain_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->getCurrentMasterTable() == "vw_bill_ranap") { ?>
...
<?php } ?>
<div>
<?php if ($vw_bill_ranap_detail_tindakan_lain->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_id" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->id->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_id">
<span<?php echo $vw_bill_ranap_detail_tindakan_lain->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tindakan_lain->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->id->CurrentValue) ?>">
<?php echo $vw_bill_ranap_detail_tindakan_lain->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->id_admission->Visible) { // id_admission ?>
	<div id="r_id_admission" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_id_admission" for="x_id_admission" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->id_admission->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->id_admission->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tindakan_lain->id_admission->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tindakan_lain_id_admission">
<span<?php echo $vw_bill_ranap_detail_tindakan_lain->id_admission->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tindakan_lain->id_admission->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_id_admission" name="x_id_admission" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->id_admission->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tindakan_lain_id_admission">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_id_admission" name="x_id_admission" id="x_id_admission" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->id_admission->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->id_admission->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->id_admission->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tindakan_lain->id_admission->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->nomr->Visible) { // nomr ?>
	<div id="r_nomr" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_nomr" for="x_nomr" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->nomr->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->nomr->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tindakan_lain->nomr->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tindakan_lain_nomr">
<span<?php echo $vw_bill_ranap_detail_tindakan_lain->nomr->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tindakan_lain->nomr->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_nomr" name="x_nomr" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->nomr->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tindakan_lain_nomr">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_nomr" name="x_nomr" id="x_nomr" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->nomr->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->nomr->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->nomr->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tindakan_lain->nomr->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->statusbayar->Visible) { // statusbayar ?>
	<div id="r_statusbayar" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_statusbayar" for="x_statusbayar" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->statusbayar->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->statusbayar->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tindakan_lain->statusbayar->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tindakan_lain_statusbayar">
<span<?php echo $vw_bill_ranap_detail_tindakan_lain->statusbayar->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tindakan_lain->statusbayar->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_statusbayar" name="x_statusbayar" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->statusbayar->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tindakan_lain_statusbayar">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_statusbayar" name="x_statusbayar" id="x_statusbayar" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->statusbayar->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->statusbayar->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->statusbayar->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tindakan_lain->statusbayar->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->kelas->Visible) { // kelas ?>
	<div id="r_kelas" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_kelas" for="x_kelas" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->kelas->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->kelas->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tindakan_lain->kelas->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tindakan_lain_kelas">
<span<?php echo $vw_bill_ranap_detail_tindakan_lain->kelas->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tindakan_lain->kelas->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_kelas" name="x_kelas" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->kelas->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tindakan_lain_kelas">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_kelas" name="x_kelas" id="x_kelas" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->kelas->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->kelas->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->kelas->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tindakan_lain->kelas->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->tanggal->Visible) { // tanggal ?>
	<div id="r_tanggal" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_tanggal" for="x_tanggal" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->tanggal->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->tanggal->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_tanggal">
...
<script type="text/javascript">
ew_CreateCalendar("fvw_bill_ranap_detail_tindakan_lainedit", "x_tanggal", 7);
</script>
<?php } ?>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->tanggal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->kode_tindakan->Visible) { // kode_tindakan ?>
	<div id="r_kode_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_kode_tindakan" for="x_kode_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->kode_tindakan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_kode_tindakan">
...
<select data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_kode_tindakan" data-value-separator="<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_tindakan->DisplayValueSeparatorAttribute() ?>" id="x_kode_tindakan" name="x_kode_tindakan"<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_tindakan->EditAttributes() ?>>
<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_tindakan->SelectOptionListHtml("x_kode_tindakan") ?>
</select>
<input type="hidden" name="s_x_kode_tindakan" id="s_x_kode_tindakan" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_tindakan->LookupFilterQuery() ?>">
<input type="hidden" name="ln_x_kode_tindakan" id="ln_x_kode_tindakan" value="x_tarif,x_bhp">
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->qty->Visible) { // qty ?>
	<div id="r_qty" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_qty" for="x_qty" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->qty->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->qty->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_qty">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_qty" name="x_qty" id="x_qty" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->qty->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->qty->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->qty->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->qty->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->tarif->Visible) { // tarif ?>
	<div id="r_tarif" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_tarif" for="x_tarif" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->tarif->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->tarif->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_tarif">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_tarif" name="x_tarif" id="x_tarif" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->tarif->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->tarif->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->tarif->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->tarif->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->bhp->Visible) { // bhp ?>
	<div id="r_bhp" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_bhp" for="x_bhp" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->bhp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->bhp->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_bhp">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_bhp" name="x_bhp" id="x_bhp" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->bhp->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->bhp->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->bhp->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->bhp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->user->Visible) { // user ?>
	<div id="r_user" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_user" for="x_user" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->user->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->user->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_user">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_user" name="x_user" id="x_user" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->user->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->user->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->user->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->user->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->nama_tindakan->Visible) { // nama_tindakan ?>
	<div id="r_nama_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_nama_tindakan" for="x_nama_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->nama_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->nama_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_nama_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_nama_tindakan" name="x_nama_tindakan" id="x_nama_tindakan" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->nama_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->nama_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->nama_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->nama_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->kelompok_tindakan->Visible) { // kelompok_tindakan ?>
	<div id="r_kelompok_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_kelompok_tindakan" for="x_kelompok_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_kelompok_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_kelompok_tindakan" name="x_kelompok_tindakan" id="x_kelompok_tindakan" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->kelompok_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->kelompok1->Visible) { // kelompok1 ?>
	<div id="r_kelompok1" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_kelompok1" for="x_kelompok1" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok1->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok1->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_kelompok1">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_kelompok1" name="x_kelompok1" id="x_kelompok1" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->kelompok1->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok1->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok1->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok1->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->kelompok2->Visible) { // kelompok2 ?>
	<div id="r_kelompok2" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_kelompok2" for="x_kelompok2" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok2->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok2->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_kelompok2">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_kelompok2" name="x_kelompok2" id="x_kelompok2" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->kelompok2->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok2->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok2->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->kelompok2->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->kode_dokter->Visible) { // kode_dokter ?>
	<div id="r_kode_dokter" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_kode_dokter" for="x_kode_dokter" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->kode_dokter->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_dokter->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_kode_dokter">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_kode_dokter" name="x_kode_dokter" id="x_kode_dokter" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->kode_dokter->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_dokter->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_dokter->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->kode_dokter->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_lain->no_ruang->Visible) { // no_ruang ?>
	<div id="r_no_ruang" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tindakan_lain_no_ruang" for="x_no_ruang" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tindakan_lain->no_ruang->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tindakan_lain->no_ruang->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tindakan_lain_no_ruang">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_lain" data-field="x_no_ruang" name="x_no_ruang" id="x_no_ruang" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_lain->no_ruang->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_lain->no_ruang->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_lain->no_ruang->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tindakan_lain->no_ruang->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$vw_bill_ranap_detail_tindakan_lain_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_bill_ranap_detail_tindakan_lain_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvw_bill_ranap_detail_tindakan_lainedit.Init();
</script>
<?php
$vw_bill_ranap_detail_tindakan_lain_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_bill_ranap_detail_tindakan_lain_edit->Page_Terminate();
?>
