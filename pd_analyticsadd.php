...
$pd_analytics_add = NULL; // Initialize page object first

class cpd_analytics_add extends cpd_analytics {
	...
	// Page object name
	var $PageObjName = 'pd_analytics_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "pd_analyticsview.php")
	...
	function LoadDefaultValues() {
		$this->v_ipaddr->CurrentValue = NULL;
		$this->v_ipaddr->OldValue = $this->v_ipaddr->CurrentValue;
		$this->v_datetime->CurrentValue = NULL;
		$this->v_datetime->OldValue = $this->v_datetime->CurrentValue;
		$this->v_referer->CurrentValue = NULL;
		$this->v_referer->OldValue = $this->v_referer->CurrentValue;
		$this->v_language->CurrentValue = NULL;
		$this->v_language->OldValue = $this->v_language->CurrentValue;
		$this->v_http_cookie->CurrentValue = NULL;
		$this->v_http_cookie->OldValue = $this->v_http_cookie->CurrentValue;
		$this->v_locale->CurrentValue = NULL;
		$this->v_locale->OldValue = $this->v_locale->CurrentValue;
		$this->v_useragent->CurrentValue = NULL;
		$this->v_useragent->OldValue = $this->v_useragent->CurrentValue;
		$this->v_remote_addr->CurrentValue = NULL;
		$this->v_remote_addr->OldValue = $this->v_remote_addr->CurrentValue;
		$this->v_browser->CurrentValue = NULL;
		$this->v_browser->OldValue = $this->v_browser->CurrentValue;
		$this->v_platform->CurrentValue = NULL;
		$this->v_platform->OldValue = $this->v_platform->CurrentValue;
		$this->v_version->CurrentValue = NULL;
		$this->v_version->OldValue = $this->v_version->CurrentValue;
		$this->v_city->CurrentValue = NULL;
		$this->v_city->OldValue = $this->v_city->CurrentValue;
		$this->v_country->CurrentValue = NULL;
		$this->v_country->OldValue = $this->v_country->CurrentValue;
		$this->v_countrycode->CurrentValue = NULL;
		$this->v_countrycode->OldValue = $this->v_countrycode->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// v_ipaddr
		$this->v_ipaddr->SetDbValueDef($rsnew, $this->v_ipaddr->CurrentValue, NULL, FALSE);

		// v_datetime
		$this->v_datetime->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->v_datetime->CurrentValue, 0), NULL, FALSE);

		// v_referer
		$this->v_referer->SetDbValueDef($rsnew, $this->v_referer->CurrentValue, NULL, FALSE);

		// v_language
		$this->v_language->SetDbValueDef($rsnew, $this->v_language->CurrentValue, NULL, FALSE);

		// v_http_cookie
		$this->v_http_cookie->SetDbValueDef($rsnew, $this->v_http_cookie->CurrentValue, NULL, FALSE);

		// v_locale
		$this->v_locale->SetDbValueDef($rsnew, $this->v_locale->CurrentValue, NULL, FALSE);

		// v_useragent
		$this->v_useragent->SetDbValueDef($rsnew, $this->v_useragent->CurrentValue, NULL, FALSE);

		// v_remote_addr
		$this->v_remote_addr->SetDbValueDef($rsnew, $this->v_remote_addr->CurrentValue, NULL, FALSE);

		// v_browser
		$this->v_browser->SetDbValueDef($rsnew, $this->v_browser->CurrentValue, NULL, FALSE);

		// v_platform
		$this->v_platform->SetDbValueDef($rsnew, $this->v_platform->CurrentValue, NULL, FALSE);

		// v_version
		$this->v_version->SetDbValueDef($rsnew, $this->v_version->CurrentValue, NULL, FALSE);

		// v_city
		$this->v_city->SetDbValueDef($rsnew, $this->v_city->CurrentValue, NULL, FALSE);

		// v_country
		$this->v_country->SetDbValueDef($rsnew, $this->v_country->CurrentValue, NULL, FALSE);

		// v_countrycode
		$this->v_countrycode->SetDbValueDef($rsnew, $this->v_countrycode->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($pd_analytics_add)) $pd_analytics_add = new cpd_analytics_add();

// Page init
$pd_analytics_add->Page_Init();

// Page main
$pd_analytics_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pd_analytics_add->Page_Render();
?>
...
var CurrentForm = fpd_analyticsadd = new ew_Form("fpd_analyticsadd", "add");

// Validate form
fpd_analyticsadd.Validate = function() {
...
fpd_analyticsadd.Form_CustomValidate = 
...
fpd_analyticsadd.ValidateRequired = true;
<?php } else { ?>
fpd_analyticsadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$pd_analytics_add->IsModal) { ?>
<?php } ?>
<?php $pd_analytics_add->ShowPageHeader(); ?>
<?php
$pd_analytics_add->ShowMessage();
?>
<form name="fpd_analyticsadd" id="fpd_analyticsadd" class="<?php echo $pd_analytics_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pd_analytics_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pd_analytics_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $pd_analytics_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fpd_analyticsadd.Init();
</script>
<?php
$pd_analytics_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$pd_analytics_add->Page_Terminate();
?>
