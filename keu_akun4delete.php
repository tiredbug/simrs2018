...
$keu_akun4_delete = NULL; // Initialize page object first

class ckeu_akun4_delete extends ckeu_akun4 {
	...
	// Page object name
	var $PageObjName = 'keu_akun4_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("keu_akun4list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in keu_akun4 class, keu_akun4info.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("keu_akun4list.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($keu_akun4_delete)) $keu_akun4_delete = new ckeu_akun4_delete();

// Page init
$keu_akun4_delete->Page_Init();

// Page main
$keu_akun4_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$keu_akun4_delete->Page_Render();
?>
...
var CurrentForm = fkeu_akun4delete = new ew_Form("fkeu_akun4delete", "delete");

// Form_CustomValidate event
fkeu_akun4delete.Form_CustomValidate = 
...
fkeu_akun4delete.ValidateRequired = true;
<?php } else { ?>
fkeu_akun4delete.ValidateRequired = false; 
<?php } ?>
...
<?php $keu_akun4_delete->ShowPageHeader(); ?>
<?php
$keu_akun4_delete->ShowMessage();
?>
<form name="fkeu_akun4delete" id="fkeu_akun4delete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($keu_akun4_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $keu_akun4_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="keu_akun4">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($keu_akun4_delete->RecKeys as $key) { ?>
...
<?php if ($keu_akun4->kd_akun->Visible) { // kd_akun ?>
		<th><span id="elh_keu_akun4_kd_akun" class="keu_akun4_kd_akun"><?php echo $keu_akun4->kd_akun->FldCaption() ?></span></th>
<?php } ?>
<?php if ($keu_akun4->kel4->Visible) { // kel4 ?>
		<th><span id="elh_keu_akun4_kel4" class="keu_akun4_kel4"><?php echo $keu_akun4->kel4->FldCaption() ?></span></th>
<?php } ?>
<?php if ($keu_akun4->nmkel4->Visible) { // nmkel4 ?>
		<th><span id="elh_keu_akun4_nmkel4" class="keu_akun4_nmkel4"><?php echo $keu_akun4->nmkel4->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$keu_akun4_delete->RecCnt = 0;
$i = 0;
while (!$keu_akun4_delete->Recordset->EOF) {
	$keu_akun4_delete->RecCnt++;
	$keu_akun4_delete->RowCnt++;

	// Set row properties
	$keu_akun4->ResetAttrs();
	$keu_akun4->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$keu_akun4_delete->LoadRowValues($keu_akun4_delete->Recordset);

	// Render row
	$keu_akun4_delete->RenderRow();
?>
	<tr<?php echo $keu_akun4->RowAttributes() ?>>
<?php if ($keu_akun4->kd_akun->Visible) { // kd_akun ?>
		<td<?php echo $keu_akun4->kd_akun->CellAttributes() ?>>
<span id="el<?php echo $keu_akun4_delete->RowCnt ?>_keu_akun4_kd_akun" class="keu_akun4_kd_akun">
...
<span id="el<?php echo $keu_akun4_delete->RowCnt ?>_keu_akun4_kel4" class="keu_akun4_kel4">
...
<span id="el<?php echo $keu_akun4_delete->RowCnt ?>_keu_akun4_nmkel4" class="keu_akun4_nmkel4">
...
<?php
	$keu_akun4_delete->Recordset->MoveNext();
}
$keu_akun4_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $keu_akun4_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fkeu_akun4delete.Init();
</script>
<?php
$keu_akun4_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$keu_akun4_delete->Page_Terminate();
?>
