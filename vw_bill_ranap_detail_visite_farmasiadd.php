...
$vw_bill_ranap_detail_visite_farmasi_add = NULL; // Initialize page object first

class cvw_bill_ranap_detail_visite_farmasi_add extends cvw_bill_ranap_detail_visite_farmasi {
	...
	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_visite_farmasi_add';
	...
	function Page_Init() {
		...
		if (!$Security->CanAdd()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("vw_bill_ranap_detail_visite_farmasilist.php"));
			else
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("vw_bill_ranap_detail_visite_farmasilist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "vw_bill_ranap_detail_visite_farmasilist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "vw_bill_ranap_detail_visite_farmasiview.php")
	...
	function RenderRow() {
		...
		$this->no_ruang->ViewCustomAttributes = "";
			...
			if (trim(strval($this->kode_farmasi->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->kode_farmasi->CurrentValue, EW_DATATYPE_NUMBER, "");
			}
			$sSqlWrk = "SELECT `id`, `nama` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `m_farmasi`";
			$sWhereWrk = "";
			$this->kode_farmasi->LookupFilters = array();
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->kode_farmasi, $sWhereWrk); // Call Lookup selecting
			...
			$arwrk = ($rswrk) ? $rswrk->GetRows() : array();
			if ($rswrk) $rswrk->Close();
			$this->kode_farmasi->EditValue = $arwrk;
	...
	function ValidateForm() {
		...
		if (!$this->kode_farmasi->FldIsDetailKey && !is_null($this->kode_farmasi->FormValue) && $this->kode_farmasi->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->kode_farmasi->FldCaption(), $this->kode_farmasi->ReqErrMsg));
		}
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("vw_bill_ranap_detail_visite_farmasilist.php"), "", $this->TableVar, TRUE);
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_kode_farmasi":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `nama` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `m_farmasi`";
			$sWhereWrk = "";
			$this->kode_farmasi->LookupFilters = array();
...
// Create page object
if (!isset($vw_bill_ranap_detail_visite_farmasi_add)) $vw_bill_ranap_detail_visite_farmasi_add = new cvw_bill_ranap_detail_visite_farmasi_add();

// Page init
$vw_bill_ranap_detail_visite_farmasi_add->Page_Init();

// Page main
$vw_bill_ranap_detail_visite_farmasi_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_bill_ranap_detail_visite_farmasi_add->Page_Render();
?>
...
var CurrentForm = fvw_bill_ranap_detail_visite_farmasiadd = new ew_Form("fvw_bill_ranap_detail_visite_farmasiadd", "add");

// Validate form
fvw_bill_ranap_detail_visite_farmasiadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_visite_farmasi->id_admission->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_statusbayar");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_visite_farmasi->statusbayar->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelas");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_visite_farmasi->kelas->FldErrMsg()) ?>");
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_visite_farmasi->kelompok1->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok2");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_visite_farmasi->kelompok2->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok_tindakan");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_visite_farmasi->kelompok_tindakan->FldErrMsg()) ?>");
...
fvw_bill_ranap_detail_visite_farmasiadd.Form_CustomValidate = 
...
fvw_bill_ranap_detail_visite_farmasiadd.ValidateRequired = true;
<?php } else { ?>
fvw_bill_ranap_detail_visite_farmasiadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_bill_ranap_detail_visite_farmasiadd.Lists["x_kode_tindakan"] = {"LinkField":"x_kode","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama_tindakan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_bill_ranap_data_tarif_tindakan"};
fvw_bill_ranap_detail_visite_farmasiadd.Lists["x_kode_farmasi"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"m_farmasi"};
...
<?php if (!$vw_bill_ranap_detail_visite_farmasi_add->IsModal) { ?>
<?php } ?>
<?php $vw_bill_ranap_detail_visite_farmasi_add->ShowPageHeader(); ?>
<?php
$vw_bill_ranap_detail_visite_farmasi_add->ShowMessage();
?>
<form name="fvw_bill_ranap_detail_visite_farmasiadd" id="fvw_bill_ranap_detail_visite_farmasiadd" class="<?php echo $vw_bill_ranap_detail_visite_farmasi_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_bill_ranap_detail_visite_farmasi_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi_add->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->getCurrentMasterTable() == "vw_bill_ranap") { ?>
...
<?php } ?>
<div>
<?php if ($vw_bill_ranap_detail_visite_farmasi->id_admission->Visible) { // id_admission ?>
	<div id="r_id_admission" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_id_admission" for="x_id_admission" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->id_admission->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->id_admission->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_visite_farmasi->id_admission->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_id_admission">
<span<?php echo $vw_bill_ranap_detail_visite_farmasi->id_admission->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_visite_farmasi->id_admission->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_id_admission" name="x_id_admission" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->id_admission->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_id_admission">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_id_admission" name="x_id_admission" id="x_id_admission" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->id_admission->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->id_admission->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->id_admission->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_visite_farmasi->id_admission->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->nomr->Visible) { // nomr ?>
	<div id="r_nomr" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_nomr" for="x_nomr" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->nomr->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->nomr->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_visite_farmasi->nomr->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_nomr">
<span<?php echo $vw_bill_ranap_detail_visite_farmasi->nomr->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_visite_farmasi->nomr->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_nomr" name="x_nomr" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->nomr->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_nomr">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_nomr" name="x_nomr" id="x_nomr" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->nomr->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->nomr->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->nomr->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_visite_farmasi->nomr->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->statusbayar->Visible) { // statusbayar ?>
	<div id="r_statusbayar" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_statusbayar" for="x_statusbayar" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->statusbayar->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->statusbayar->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_visite_farmasi->statusbayar->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_statusbayar">
<span<?php echo $vw_bill_ranap_detail_visite_farmasi->statusbayar->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_visite_farmasi->statusbayar->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_statusbayar" name="x_statusbayar" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->statusbayar->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_statusbayar">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_statusbayar" name="x_statusbayar" id="x_statusbayar" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->statusbayar->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->statusbayar->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->statusbayar->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_visite_farmasi->statusbayar->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->kelas->Visible) { // kelas ?>
	<div id="r_kelas" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_kelas" for="x_kelas" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->kelas->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->kelas->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_visite_farmasi->kelas->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_kelas">
<span<?php echo $vw_bill_ranap_detail_visite_farmasi->kelas->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_visite_farmasi->kelas->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_kelas" name="x_kelas" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->kelas->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_kelas">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_kelas" name="x_kelas" id="x_kelas" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->kelas->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->kelas->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->kelas->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_visite_farmasi->kelas->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->tanggal->Visible) { // tanggal ?>
	<div id="r_tanggal" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_tanggal" for="x_tanggal" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->tanggal->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->tanggal->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_tanggal">
...
<script type="text/javascript">
ew_CreateCalendar("fvw_bill_ranap_detail_visite_farmasiadd", "x_tanggal", 7);
</script>
<?php } ?>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->tanggal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->kode_tindakan->Visible) { // kode_tindakan ?>
	<div id="r_kode_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_kode_tindakan" for="x_kode_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->kode_tindakan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_kode_tindakan">
...
<select data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_kode_tindakan" data-value-separator="<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_tindakan->DisplayValueSeparatorAttribute() ?>" id="x_kode_tindakan" name="x_kode_tindakan"<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_tindakan->EditAttributes() ?>>
<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_tindakan->SelectOptionListHtml("x_kode_tindakan") ?>
</select>
<input type="hidden" name="s_x_kode_tindakan" id="s_x_kode_tindakan" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_tindakan->LookupFilterQuery() ?>">
<input type="hidden" name="ln_x_kode_tindakan" id="ln_x_kode_tindakan" value="x_tarif,x_bhp">
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->kode_farmasi->Visible) { // kode_farmasi ?>
	<div id="r_kode_farmasi" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_kode_farmasi" for="x_kode_farmasi" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->kode_farmasi->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_farmasi->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_kode_farmasi">
<select data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_kode_farmasi" data-value-separator="<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_farmasi->DisplayValueSeparatorAttribute() ?>" id="x_kode_farmasi" name="x_kode_farmasi"<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_farmasi->EditAttributes() ?>>
<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_farmasi->SelectOptionListHtml("x_kode_farmasi") ?>
</select>
<input type="hidden" name="s_x_kode_farmasi" id="s_x_kode_farmasi" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_farmasi->LookupFilterQuery() ?>">
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->kode_farmasi->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->qty->Visible) { // qty ?>
	<div id="r_qty" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_qty" for="x_qty" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->qty->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->qty->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_qty">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_qty" name="x_qty" id="x_qty" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->qty->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->qty->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->qty->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->qty->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->tarif->Visible) { // tarif ?>
	<div id="r_tarif" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_tarif" for="x_tarif" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->tarif->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->tarif->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_tarif">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_tarif" name="x_tarif" id="x_tarif" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->tarif->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->tarif->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->tarif->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->tarif->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->bhp->Visible) { // bhp ?>
	<div id="r_bhp" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_bhp" for="x_bhp" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->bhp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->bhp->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_bhp">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_bhp" name="x_bhp" id="x_bhp" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->bhp->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->bhp->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->bhp->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->bhp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->kelompok1->Visible) { // kelompok1 ?>
	<div id="r_kelompok1" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_kelompok1" for="x_kelompok1" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok1->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok1->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_kelompok1">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_kelompok1" name="x_kelompok1" id="x_kelompok1" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->kelompok1->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok1->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok1->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok1->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->kelompok2->Visible) { // kelompok2 ?>
	<div id="r_kelompok2" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_kelompok2" for="x_kelompok2" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok2->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok2->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_kelompok2">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_kelompok2" name="x_kelompok2" id="x_kelompok2" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->kelompok2->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok2->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok2->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok2->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->kelompok_tindakan->Visible) { // kelompok_tindakan ?>
	<div id="r_kelompok_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_kelompok_tindakan" for="x_kelompok_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_kelompok_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_kelompok_tindakan" name="x_kelompok_tindakan" id="x_kelompok_tindakan" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->kelompok_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->kelompok_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->nama_tindakan->Visible) { // nama_tindakan ?>
	<div id="r_nama_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_nama_tindakan" for="x_nama_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->nama_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->nama_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_nama_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_nama_tindakan" name="x_nama_tindakan" id="x_nama_tindakan" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->nama_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->nama_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->nama_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->nama_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->user->Visible) { // user ?>
	<div id="r_user" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_user" for="x_user" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->user->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->user->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_farmasi_user">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_user" name="x_user" id="x_user" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->user->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->user->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->user->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_visite_farmasi->user->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_visite_farmasi->no_ruang->Visible) { // no_ruang ?>
	<div id="r_no_ruang" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_farmasi_no_ruang" for="x_no_ruang" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_farmasi->no_ruang->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_farmasi->no_ruang->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_visite_farmasi->no_ruang->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_no_ruang">
...
<input type="hidden" id="x_no_ruang" name="x_no_ruang" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->no_ruang->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_visite_farmasi_no_ruang">
<input type="text" data-table="vw_bill_ranap_detail_visite_farmasi" data-field="x_no_ruang" name="x_no_ruang" id="x_no_ruang" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_farmasi->no_ruang->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_visite_farmasi->no_ruang->EditValue ?>"<?php echo $vw_bill_ranap_detail_visite_farmasi->no_ruang->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_visite_farmasi->no_ruang->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$vw_bill_ranap_detail_visite_farmasi_add->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_bill_ranap_detail_visite_farmasi_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvw_bill_ranap_detail_visite_farmasiadd.Init();
</script>
<?php
$vw_bill_ranap_detail_visite_farmasi_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_bill_ranap_detail_visite_farmasi_add->Page_Terminate();
?>
