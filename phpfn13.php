...
function &DbHelper($dbid = "") {
	if ($dbid == "dbantrianSis" || $dbid === 1) // dbantrianSis
		$dbclass = "cdbantrianSis_db";
	else // DB
		$dbclass = "csimrs2012_db";
...
function CurrentProjectID() {
	...
	return "{9EA43BBE-9546-4DFA-A9E1-2DE91AC35CAA}";
}
	...
	function RenderButtonGroup($body) {
		...
		if (preg_match_all('/<(a|button)([^>]*)>([\s\S]*?)<\/(a|button)\s*>/i', $body, $matches, PREG_SET_ORDER)) {
			foreach ($matches as $match) {
				...
				$caption = $match[3];
				if (strpos($class, 'btn-xs') === FALSE) // Prepend button classes
		...
		if ($links <> "")
			$btngroup = '<div class="btn-group pull-left">' . $links . '</div>';
		else
			$btngroup = "";
		foreach ($btns as $btn)
			$btngroup .= $btn;
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$validateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$validateUser = $customValidateUser || ew_ComparePassword($rs->fields('PWD'), $pwd, $encrypted);
					if ($validateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('NIP')); // Load user name
						$this->setSessionUserID($rs->fields('id')); // Load User ID
						$this->setSessionParentUserID($rs->fields('pd_parentid')); // Load parent User ID
						if (is_null($rs->fields('role_id'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('role_id'))); // Load User Level
						}
						$this->SetUpUserLevel();
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`pd_parentid` IN (' . $sCurUserIDList . ')';
					$sSql = $UserTable->GetSQL($sFilter, '');
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('id'));
...
include_once($EW_RELATIVE_PATH . "phpmailer/PHPMailerAutoload.php");
