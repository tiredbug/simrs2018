...
$keu_akun3_delete = NULL; // Initialize page object first

class ckeu_akun3_delete extends ckeu_akun3 {
	...
	// Page object name
	var $PageObjName = 'keu_akun3_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("keu_akun3list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in keu_akun3 class, keu_akun3info.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("keu_akun3list.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($keu_akun3_delete)) $keu_akun3_delete = new ckeu_akun3_delete();

// Page init
$keu_akun3_delete->Page_Init();

// Page main
$keu_akun3_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$keu_akun3_delete->Page_Render();
?>
...
var CurrentForm = fkeu_akun3delete = new ew_Form("fkeu_akun3delete", "delete");

// Form_CustomValidate event
fkeu_akun3delete.Form_CustomValidate = 
...
fkeu_akun3delete.ValidateRequired = true;
<?php } else { ?>
fkeu_akun3delete.ValidateRequired = false; 
<?php } ?>
...
<?php $keu_akun3_delete->ShowPageHeader(); ?>
<?php
$keu_akun3_delete->ShowMessage();
?>
<form name="fkeu_akun3delete" id="fkeu_akun3delete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($keu_akun3_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $keu_akun3_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="keu_akun3">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($keu_akun3_delete->RecKeys as $key) { ?>
...
<?php if ($keu_akun3->kd_akun->Visible) { // kd_akun ?>
		<th><span id="elh_keu_akun3_kd_akun" class="keu_akun3_kd_akun"><?php echo $keu_akun3->kd_akun->FldCaption() ?></span></th>
<?php } ?>
<?php if ($keu_akun3->kel3->Visible) { // kel3 ?>
		<th><span id="elh_keu_akun3_kel3" class="keu_akun3_kel3"><?php echo $keu_akun3->kel3->FldCaption() ?></span></th>
<?php } ?>
<?php if ($keu_akun3->nmkel3->Visible) { // nmkel3 ?>
		<th><span id="elh_keu_akun3_nmkel3" class="keu_akun3_nmkel3"><?php echo $keu_akun3->nmkel3->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$keu_akun3_delete->RecCnt = 0;
$i = 0;
while (!$keu_akun3_delete->Recordset->EOF) {
	$keu_akun3_delete->RecCnt++;
	$keu_akun3_delete->RowCnt++;

	// Set row properties
	$keu_akun3->ResetAttrs();
	$keu_akun3->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$keu_akun3_delete->LoadRowValues($keu_akun3_delete->Recordset);

	// Render row
	$keu_akun3_delete->RenderRow();
?>
	<tr<?php echo $keu_akun3->RowAttributes() ?>>
<?php if ($keu_akun3->kd_akun->Visible) { // kd_akun ?>
		<td<?php echo $keu_akun3->kd_akun->CellAttributes() ?>>
<span id="el<?php echo $keu_akun3_delete->RowCnt ?>_keu_akun3_kd_akun" class="keu_akun3_kd_akun">
...
<span id="el<?php echo $keu_akun3_delete->RowCnt ?>_keu_akun3_kel3" class="keu_akun3_kel3">
...
<span id="el<?php echo $keu_akun3_delete->RowCnt ?>_keu_akun3_nmkel3" class="keu_akun3_nmkel3">
...
<?php
	$keu_akun3_delete->Recordset->MoveNext();
}
$keu_akun3_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $keu_akun3_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fkeu_akun3delete.Init();
</script>
<?php
$keu_akun3_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$keu_akun3_delete->Page_Terminate();
?>
