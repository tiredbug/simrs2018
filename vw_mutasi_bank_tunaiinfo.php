...
$vw_mutasi_bank_tunai = NULL;

//
// Table class for vw_mutasi_bank_tunai
//
class cvw_mutasi_bank_tunai extends cTable {
	var $id;
	var $jenis_mutasi;
	var $no_bukti;
	var $tgl_bukti;
	...
	function __construct() {
		...
		$this->TableVar = 'vw_mutasi_bank_tunai';
		$this->TableName = 'vw_mutasi_bank_tunai';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vw_mutasi_bank_tunai`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('vw_mutasi_bank_tunai', 'vw_mutasi_bank_tunai', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// jenis_mutasi
		$this->jenis_mutasi = new cField('vw_mutasi_bank_tunai', 'vw_mutasi_bank_tunai', 'x_jenis_mutasi', 'jenis_mutasi', '`jenis_mutasi`', '`jenis_mutasi`', 3, -1, FALSE, '`jenis_mutasi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->jenis_mutasi->Sortable = TRUE; // Allow sort
		$this->jenis_mutasi->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->jenis_mutasi->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->jenis_mutasi->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['jenis_mutasi'] = &$this->jenis_mutasi;

		// no_bukti
		$this->no_bukti = new cField('vw_mutasi_bank_tunai', 'vw_mutasi_bank_tunai', 'x_no_bukti', 'no_bukti', '`no_bukti`', '`no_bukti`', 200, -1, FALSE, '`no_bukti`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->no_bukti->Sortable = TRUE; // Allow sort
		$this->fields['no_bukti'] = &$this->no_bukti;

		// tgl_bukti
		$this->tgl_bukti = new cField('vw_mutasi_bank_tunai', 'vw_mutasi_bank_tunai', 'x_tgl_bukti', 'tgl_bukti', '`tgl_bukti`', ew_CastDateFieldForLike('`tgl_bukti`', 7, "DB"), 135, 7, FALSE, '`tgl_bukti`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// uraian
		$this->uraian = new cField('vw_mutasi_bank_tunai', 'vw_mutasi_bank_tunai', 'x_uraian', 'uraian', '`uraian`', '`uraian`', 200, -1, FALSE, '`uraian`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->uraian->Sortable = TRUE; // Allow sort
		$this->fields['uraian'] = &$this->uraian;

		// jumlah
		$this->jumlah = new cField('vw_mutasi_bank_tunai', 'vw_mutasi_bank_tunai', 'x_jumlah', 'jumlah', '`jumlah`', '`jumlah`', 5, -1, FALSE, '`jumlah`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`vw_mutasi_bank_tunai`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "vw_mutasi_bank_tunailist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("vw_mutasi_bank_tunaiview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("vw_mutasi_bank_tunaiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "vw_mutasi_bank_tunaiadd.php?" . $this->UrlParm($parm);
		else
			$url = "vw_mutasi_bank_tunaiadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("vw_mutasi_bank_tunaiedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("vw_mutasi_bank_tunaiadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("vw_mutasi_bank_tunaidelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->jenis_mutasi->Exportable) $Doc->ExportCaption($this->jenis_mutasi);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->jenis_mutasi->Exportable) $Doc->ExportField($this->jenis_mutasi);
