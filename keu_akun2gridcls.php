...
$keu_akun2_grid = NULL; // Initialize page object first

class ckeu_akun2_grid extends ckeu_akun2 {
	...
	// Page object name
	var $PageObjName = 'keu_akun2_grid';

	// Grid form hidden field names
	var $FormName = 'fkeu_akun2grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["keu_akun2"];

		}
		$this->AddUrl = "keu_akun2add.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_kd_akun") && $objForm->HasValue("o_kd_akun") && $this->kd_akun->CurrentValue <> $this->kd_akun->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_kel2") && $objForm->HasValue("o_kel2") && $this->kel2->CurrentValue <> $this->kel2->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nmkel2") && $objForm->HasValue("o_nmkel2") && $this->nmkel2->CurrentValue <> $this->nmkel2->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->kel2->setOldValue($objForm->GetValue("o_kel2"));
		if (!$this->nmkel2->FldIsDetailKey) {
			$this->nmkel2->setFormValue($objForm->GetValue("x_nmkel2"));
		}
		$this->nmkel2->setOldValue($objForm->GetValue("o_nmkel2"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "keu_akun1") {
			...
			if ($GLOBALS["keu_akun1"]->EventCancelled) $this->EventCancelled = TRUE;
		}
