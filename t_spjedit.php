...
$t_spj_edit = NULL; // Initialize page object first

class ct_spj_edit extends ct_spj {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("t_spjlist.php"); // Invalid key, return to list
		}
	...
	function ValidateForm() {
		...
		if (in_array("detail_spj", $DetailTblVar) && $GLOBALS["detail_spj"]->DetailEdit) {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("detail_spj", $DetailTblVar) && $GLOBALS["detail_spj"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["detail_spj_grid"]->GridUpdate();
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("detail_spj", $DetailTblVar)) {
				...
				if ($GLOBALS["detail_spj_grid"]->DetailEdit) {
					$GLOBALS["detail_spj_grid"]->CurrentMode = "edit";
					$GLOBALS["detail_spj_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($t_spj_edit)) $t_spj_edit = new ct_spj_edit();

// Page init
$t_spj_edit->Page_Init();

// Page main
$t_spj_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_spj_edit->Page_Render();
?>
...
var CurrentForm = ft_spjedit = new ew_Form("ft_spjedit", "edit");

// Validate form
ft_spjedit.Validate = function() {
...
ft_spjedit.Form_CustomValidate = 
...
ft_spjedit.ValidateRequired = true;
<?php } else { ?>
ft_spjedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_spjedit.Lists["x_jenis_spj"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_spjedit.Lists["x_jenis_spj"].Options = <?php echo json_encode($t_spj->jenis_spj->Options()) ?>;
ft_spjedit.Lists["x_program"] = {"LinkField":"x_kode_program","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_program","","",""],"ParentFields":[],"ChildFields":["x_kegiatan","x_sub_kegiatan"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"m_program"};
ft_spjedit.Lists["x_kegiatan"] = {"LinkField":"x_kode_kegiatan","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_kegiatan","","",""],"ParentFields":["x_program"],"ChildFields":["x_sub_kegiatan"],"FilterFields":["x_kode_program"],"Options":[],"Template":"","LinkTable":"m_kegiatan"};
ft_spjedit.Lists["x_sub_kegiatan"] = {"LinkField":"x_kode_sub_kegiatan","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_sub_kegiatan","","",""],"ParentFields":["x_kegiatan","x_program"],"ChildFields":[],"FilterFields":["x_kode_kegiatan","x_kode_program"],"Options":[],"Template":"","LinkTable":"m_sub_kegiatan"};
...
<?php if (!$t_spj_edit->IsModal) { ?>
<?php } ?>
<?php $t_spj_edit->ShowPageHeader(); ?>
<?php
$t_spj_edit->ShowMessage();
?>
<form name="ft_spjedit" id="ft_spjedit" class="<?php echo $t_spj_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_spj_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_spj_edit->Token ?>">
<?php } ?>
...
<?php if ($t_spj->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_t_spj_id" class="col-sm-2 control-label ewLabel"><?php echo $t_spj->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $t_spj->id->CellAttributes() ?>>
<span id="el_t_spj_id">
<span<?php echo $t_spj->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $t_spj->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="t_spj" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($t_spj->id->CurrentValue) ?>">
<?php echo $t_spj->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("ft_spjedit", "x_tgl_spj", 7);
</script>
<?php } ?>
</span>
...
<?php
	if (in_array("detail_spj", explode(",", $t_spj->getCurrentDetailTable())) && $detail_spj->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_spj_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
