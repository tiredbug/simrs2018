...
$keu_akun4_list = NULL; // Initialize page object first

class ckeu_akun4_list extends ckeu_akun4 {
	...
	// Page object name
	var $PageObjName = 'keu_akun4_list';

	// Grid form hidden field names
	var $FormName = 'fkeu_akun4list';
	...
	function __construct() {
		...
		$this->AddUrl = "keu_akun4add.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "keu_akun4delete.php";
		$this->MultiUpdateUrl = "keu_akun4update.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fkeu_akun4listsrch";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "keu_akun3") {
			global $keu_akun3;
			$rsmaster = $keu_akun3->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("keu_akun3list.php"); // Return to master page
			} else {
				$keu_akun3->LoadListRowValues($rsmaster);
				$keu_akun3->RowType = EW_ROWTYPE_MASTER; // Master row
				$keu_akun3->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fkeu_akun4listsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
		...
		$sFilterList = ew_Concat($sFilterList, $this->kode_akun3->AdvancedSearch->ToJSON(), ","); // Field kode_akun3
		if ($this->BasicSearch->Keyword <> "") {
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fkeu_akun4listsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field kode_akun3
		$this->kode_akun3->AdvancedSearch->SearchValue = @$filter["x_kode_akun3"];
		$this->kode_akun3->AdvancedSearch->SearchOperator = @$filter["z_kode_akun3"];
		$this->kode_akun3->AdvancedSearch->SearchCondition = @$filter["v_kode_akun3"];
		$this->kode_akun3->AdvancedSearch->SearchValue2 = @$filter["y_kode_akun3"];
		$this->kode_akun3->AdvancedSearch->SearchOperator2 = @$filter["w_kode_akun3"];
		$this->kode_akun3->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->kode_akun3, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetupListOptions() {
		...
		// "detail_keu_akun5"
		$item = &$this->ListOptions->Add("detail_keu_akun5");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'keu_akun5') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
	...
	function RenderListOptions() {
		...
		// "detail_keu_akun5"
		$oListOpt = &$this->ListOptions->Items["detail_keu_akun5"];
		if ($Security->AllowList(CurrentProjectID() . 'keu_akun5')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("keu_akun5", "TblCaption");
			...
			$links = "";
			if ($GLOBALS["keu_akun5_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'keu_akun5')) {
			...
			if ($GLOBALS["keu_akun5_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'keu_akun5')) {
				$links .= "<li><a class=\"btn btn-warning btn-xs\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=keu_akun5")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
				if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
				$DetailEditTblVar .= "keu_akun5";
			}
			if ($GLOBALS["keu_akun5_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'keu_akun5')) {
				$links .= "<li><a class=\"btn btn-success btn-xs\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=keu_akun5")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_keu_akun5");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=keu_akun5");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["keu_akun5"]->TableCaption();
		...
		$item->Visible = ($GLOBALS["keu_akun5"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'keu_akun5') && $Security->CanAdd());
		if ($item->Visible) {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fkeu_akun4listsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fkeu_akun4listsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fkeu_akun4list}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fkeu_akun4listsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($keu_akun4_list)) $keu_akun4_list = new ckeu_akun4_list();

// Page init
$keu_akun4_list->Page_Init();

// Page main
$keu_akun4_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$keu_akun4_list->Page_Render();
?>
...
var CurrentForm = fkeu_akun4list = new ew_Form("fkeu_akun4list", "list");
fkeu_akun4list.FormKeyCountName = '<?php echo $keu_akun4_list->FormKeyCountName ?>';

// Form_CustomValidate event
fkeu_akun4list.Form_CustomValidate = 
...
fkeu_akun4list.ValidateRequired = true;
<?php } else { ?>
fkeu_akun4list.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var CurrentSearchForm = fkeu_akun4listsrch = new ew_Form("fkeu_akun4listsrch");
</script>
<script type="text/javascript">

// Write your client script here, no need to add script tags.
</script>
<?php if ($keu_akun4_list->TotalRecs > 0 && $keu_akun4_list->ExportOptions->Visible()) { ?>
<?php $keu_akun4_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($keu_akun4_list->SearchOptions->Visible()) { ?>
<?php $keu_akun4_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($keu_akun4_list->FilterOptions->Visible()) { ?>
<?php $keu_akun4_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php if (($keu_akun4->Export == "") || (EW_EXPORT_MASTER_RECORD && $keu_akun4->Export == "print")) { ?>
<?php
if ($keu_akun4_list->DbMasterFilter <> "" && $keu_akun4->getCurrentMasterTable() == "keu_akun3") {
	if ($keu_akun4_list->MasterRecordExists) {
?>
<?php include_once "keu_akun3master.php" ?>
<?php
	}
}
?>
<?php } ?>
<?php
	$bSelectLimit = $keu_akun4_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($keu_akun4_list->TotalRecs <= 0)
			$keu_akun4_list->TotalRecs = $keu_akun4->SelectRecordCount();
	} else {
		if (!$keu_akun4_list->Recordset && ($keu_akun4_list->Recordset = $keu_akun4_list->LoadRecordset()))
			$keu_akun4_list->TotalRecs = $keu_akun4_list->Recordset->RecordCount();
	}
	$keu_akun4_list->StartRec = 1;
	if ($keu_akun4_list->DisplayRecs <= 0 || ($keu_akun4->Export <> "" && $keu_akun4->ExportAll)) // Display all records
		$keu_akun4_list->DisplayRecs = $keu_akun4_list->TotalRecs;
	if (!($keu_akun4->Export <> "" && $keu_akun4->ExportAll))
		$keu_akun4_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$keu_akun4_list->Recordset = $keu_akun4_list->LoadRecordset($keu_akun4_list->StartRec-1, $keu_akun4_list->DisplayRecs);

	// Set no record found message
	if ($keu_akun4->CurrentAction == "" && $keu_akun4_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$keu_akun4_list->setWarningMessage(ew_DeniedMsg());
		if ($keu_akun4_list->SearchWhere == "0=101")
			$keu_akun4_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$keu_akun4_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$keu_akun4_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($keu_akun4->Export == "" && $keu_akun4->CurrentAction == "") { ?>
<form name="fkeu_akun4listsrch" id="fkeu_akun4listsrch" class="form-horizontal ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($keu_akun4_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fkeu_akun4listsrch_SearchPanel" class="box box-default collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="keu_akun4">
<div class="box-body">
<div id="xsr_1">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($keu_akun4_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($keu_akun4_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $keu_akun4_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($keu_akun4_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($keu_akun4_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($keu_akun4_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($keu_akun4_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $keu_akun4_list->ShowPageHeader(); ?>
<?php
$keu_akun4_list->ShowMessage();
?>
<?php if ($keu_akun4_list->TotalRecs > 0 || $keu_akun4->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid keu_akun4">
<form name="fkeu_akun4list" id="fkeu_akun4list" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($keu_akun4_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $keu_akun4_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="keu_akun4">
<?php if ($keu_akun4->getCurrentMasterTable() == "keu_akun3" && $keu_akun4->CurrentAction <> "") { ?>
...
<?php if ($keu_akun4_list->TotalRecs > 0 || $keu_akun4->CurrentAction == "gridedit") { ?>
<table id="tbl_keu_akun4list" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$keu_akun4_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$keu_akun4_list->RenderListOptions();

// Render list options (header, left)
$keu_akun4_list->ListOptions->Render("header", "left");
?>
<?php if ($keu_akun4->kd_akun->Visible) { // kd_akun ?>
	...
	<?php } else { ?>
		<th data-name="kd_akun"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $keu_akun4->SortUrl($keu_akun4->kd_akun) ?>',2);"><div id="elh_keu_akun4_kd_akun" class="keu_akun4_kd_akun">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $keu_akun4->kd_akun->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($keu_akun4->kd_akun->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($keu_akun4->kd_akun->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($keu_akun4->kel4->Visible) { // kel4 ?>
	...
	<?php } else { ?>
		<th data-name="kel4"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $keu_akun4->SortUrl($keu_akun4->kel4) ?>',2);"><div id="elh_keu_akun4_kel4" class="keu_akun4_kel4">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $keu_akun4->kel4->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($keu_akun4->kel4->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($keu_akun4->kel4->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($keu_akun4->nmkel4->Visible) { // nmkel4 ?>
	...
	<?php } else { ?>
		<th data-name="nmkel4"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $keu_akun4->SortUrl($keu_akun4->nmkel4) ?>',2);"><div id="elh_keu_akun4_nmkel4" class="keu_akun4_nmkel4">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $keu_akun4->nmkel4->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($keu_akun4->nmkel4->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($keu_akun4->nmkel4->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
$keu_akun4_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($keu_akun4->ExportAll && $keu_akun4->Export <> "") {
	$keu_akun4_list->StopRec = $keu_akun4_list->TotalRecs;
} else {

	// Set the last record to display
	if ($keu_akun4_list->TotalRecs > $keu_akun4_list->StartRec + $keu_akun4_list->DisplayRecs - 1)
		$keu_akun4_list->StopRec = $keu_akun4_list->StartRec + $keu_akun4_list->DisplayRecs - 1;
	else
		$keu_akun4_list->StopRec = $keu_akun4_list->TotalRecs;
}
$keu_akun4_list->RecCnt = $keu_akun4_list->StartRec - 1;
if ($keu_akun4_list->Recordset && !$keu_akun4_list->Recordset->EOF) {
	$keu_akun4_list->Recordset->MoveFirst();
	$bSelectLimit = $keu_akun4_list->UseSelectLimit;
	if (!$bSelectLimit && $keu_akun4_list->StartRec > 1)
		$keu_akun4_list->Recordset->Move($keu_akun4_list->StartRec - 1);
} elseif (!$keu_akun4->AllowAddDeleteRow && $keu_akun4_list->StopRec == 0) {
	$keu_akun4_list->StopRec = $keu_akun4->GridAddRowCount;
}

// Initialize aggregate
$keu_akun4->RowType = EW_ROWTYPE_AGGREGATEINIT;
$keu_akun4->ResetAttrs();
$keu_akun4_list->RenderRow();
while ($keu_akun4_list->RecCnt < $keu_akun4_list->StopRec) {
	$keu_akun4_list->RecCnt++;
	if (intval($keu_akun4_list->RecCnt) >= intval($keu_akun4_list->StartRec)) {
		$keu_akun4_list->RowCnt++;

		// Set up key count
		$keu_akun4_list->KeyCount = $keu_akun4_list->RowIndex;
		...
		if ($keu_akun4->CurrentAction == "gridadd") {
			...
			$keu_akun4_list->LoadRowValues($keu_akun4_list->Recordset); // Load row values
		}
		$keu_akun4->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$keu_akun4->RowAttrs = array_merge($keu_akun4->RowAttrs, array('data-rowindex'=>$keu_akun4_list->RowCnt, 'id'=>'r' . $keu_akun4_list->RowCnt . '_keu_akun4', 'data-rowtype'=>$keu_akun4->RowType));
...
$keu_akun4_list->ListOptions->Render("body", "left", $keu_akun4_list->RowCnt);
?>
	<?php if ($keu_akun4->kd_akun->Visible) { // kd_akun ?>
		<td data-name="kd_akun"<?php echo $keu_akun4->kd_akun->CellAttributes() ?>>
<span id="el<?php echo $keu_akun4_list->RowCnt ?>_keu_akun4_kd_akun" class="keu_akun4_kd_akun">
<span<?php echo $keu_akun4->kd_akun->ViewAttributes() ?>>
<?php echo $keu_akun4->kd_akun->ListViewValue() ?></span>
</span>
<a id="<?php echo $keu_akun4_list->PageObjName . "_row_" . $keu_akun4_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($keu_akun4->kel4->Visible) { // kel4 ?>
		<td data-name="kel4"<?php echo $keu_akun4->kel4->CellAttributes() ?>>
<span id="el<?php echo $keu_akun4_list->RowCnt ?>_keu_akun4_kel4" class="keu_akun4_kel4">
...
<span id="el<?php echo $keu_akun4_list->RowCnt ?>_keu_akun4_nmkel4" class="keu_akun4_nmkel4">
...
$keu_akun4_list->ListOptions->Render("body", "right", $keu_akun4_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($keu_akun4->CurrentAction <> "gridadd")
		$keu_akun4_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($keu_akun4->CurrentAction == "") { ?>
...
// Close recordset
if ($keu_akun4_list->Recordset)
	$keu_akun4_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($keu_akun4->CurrentAction <> "gridadd" && $keu_akun4->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($keu_akun4_list->Pager)) $keu_akun4_list->Pager = new cPrevNextPager($keu_akun4_list->StartRec, $keu_akun4_list->DisplayRecs, $keu_akun4_list->TotalRecs) ?>
<?php if ($keu_akun4_list->Pager->RecordCount > 0 && $keu_akun4_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($keu_akun4_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $keu_akun4_list->PageUrl() ?>start=<?php echo $keu_akun4_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($keu_akun4_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $keu_akun4_list->PageUrl() ?>start=<?php echo $keu_akun4_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $keu_akun4_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($keu_akun4_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $keu_akun4_list->PageUrl() ?>start=<?php echo $keu_akun4_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($keu_akun4_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $keu_akun4_list->PageUrl() ?>start=<?php echo $keu_akun4_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $keu_akun4_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $keu_akun4_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $keu_akun4_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $keu_akun4_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($keu_akun4_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $keu_akun4_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="keu_akun4">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($keu_akun4_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($keu_akun4_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($keu_akun4_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($keu_akun4_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($keu_akun4_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($keu_akun4_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($keu_akun4_list->TotalRecs == 0 && $keu_akun4->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($keu_akun4_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fkeu_akun4listsrch.FilterList = <?php echo $keu_akun4_list->GetFilterList() ?>;
fkeu_akun4listsrch.Init();
fkeu_akun4list.Init();
</script>
<?php
$keu_akun4_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
