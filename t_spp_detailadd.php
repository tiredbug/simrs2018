...
$t_spp_detail_add = NULL; // Initialize page object first

class ct_spp_detail_add extends ct_spp_detail {
	...
	// Page object name
	var $PageObjName = 't_spp_detail_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_spp_detailview.php")
...
// Create page object
if (!isset($t_spp_detail_add)) $t_spp_detail_add = new ct_spp_detail_add();

// Page init
$t_spp_detail_add->Page_Init();

// Page main
$t_spp_detail_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_spp_detail_add->Page_Render();
?>
...
var CurrentForm = ft_spp_detailadd = new ew_Form("ft_spp_detailadd", "add");

// Validate form
ft_spp_detailadd.Validate = function() {
...
ft_spp_detailadd.Form_CustomValidate = 
...
ft_spp_detailadd.ValidateRequired = true;
<?php } else { ?>
ft_spp_detailadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_spp_detailadd.Lists["x_kd_rekening_belanja"] = {"LinkField":"x_kd_akun","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_akun","x_kd_akun","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"keu_akun5"};
...
<?php if (!$t_spp_detail_add->IsModal) { ?>
<?php } ?>
<?php $t_spp_detail_add->ShowPageHeader(); ?>
<?php
$t_spp_detail_add->ShowMessage();
?>
<form name="ft_spp_detailadd" id="ft_spp_detailadd" class="<?php echo $t_spp_detail_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_spp_detail_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_spp_detail_add->Token ?>">
<?php } ?>
...
<?php if (strval($t_spp_detail->id_spp->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_id_spp" id="x_id_spp" value="<?php echo ew_HtmlEncode(strval($t_spp_detail->id_spp->getSessionValue())) ?>">
<?php } ?>
<?php if (strval($t_spp_detail->id_jenis_spp->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_id_jenis_spp" id="x_id_jenis_spp" value="<?php echo ew_HtmlEncode(strval($t_spp_detail->id_jenis_spp->getSessionValue())) ?>">
<?php } ?>
<?php if (strval($t_spp_detail->detail_jenis_spp->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_detail_jenis_spp" id="x_detail_jenis_spp" value="<?php echo ew_HtmlEncode(strval($t_spp_detail->detail_jenis_spp->getSessionValue())) ?>">
<?php } ?>
<?php if (strval($t_spp_detail->no_spp->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_no_spp" id="x_no_spp" value="<?php echo ew_HtmlEncode(strval($t_spp_detail->no_spp->getSessionValue())) ?>">
<?php } ?>
<?php if (strval($t_spp_detail->program->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_program" id="x_program" value="<?php echo ew_HtmlEncode(strval($t_spp_detail->program->getSessionValue())) ?>">
<?php } ?>
<?php if (strval($t_spp_detail->kegiatan->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_kegiatan" id="x_kegiatan" value="<?php echo ew_HtmlEncode(strval($t_spp_detail->kegiatan->getSessionValue())) ?>">
<?php } ?>
<?php if (strval($t_spp_detail->tahun_anggaran->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_tahun_anggaran" id="x_tahun_anggaran" value="<?php echo ew_HtmlEncode(strval($t_spp_detail->tahun_anggaran->getSessionValue())) ?>">
<?php } ?>
<?php if (!$t_spp_detail_add->IsModal) { ?>
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_spp_detail_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft_spp_detailadd.Init();
</script>
<?php
$t_spp_detail_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_spp_detail_add->Page_Terminate();
?>
