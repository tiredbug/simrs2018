...
$m_kegiatan_edit = NULL; // Initialize page object first

class cm_kegiatan_edit extends cm_kegiatan {
	...
	// Page object name
	var $PageObjName = 'm_kegiatan_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("m_kegiatanlist.php"); // Invalid key, return to list
		}
	...
	function ValidateForm() {
		...
		if (in_array("m_sub_kegiatan", $DetailTblVar) && $GLOBALS["m_sub_kegiatan"]->DetailEdit) {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("m_sub_kegiatan", $DetailTblVar) && $GLOBALS["m_sub_kegiatan"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["m_sub_kegiatan_grid"]->GridUpdate();
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("m_sub_kegiatan", $DetailTblVar)) {
				...
				if ($GLOBALS["m_sub_kegiatan_grid"]->DetailEdit) {
					$GLOBALS["m_sub_kegiatan_grid"]->CurrentMode = "edit";
					$GLOBALS["m_sub_kegiatan_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($m_kegiatan_edit)) $m_kegiatan_edit = new cm_kegiatan_edit();

// Page init
$m_kegiatan_edit->Page_Init();

// Page main
$m_kegiatan_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$m_kegiatan_edit->Page_Render();
?>
...
var CurrentForm = fm_kegiatanedit = new ew_Form("fm_kegiatanedit", "edit");

// Validate form
fm_kegiatanedit.Validate = function() {
...
fm_kegiatanedit.Form_CustomValidate = 
...
fm_kegiatanedit.ValidateRequired = true;
<?php } else { ?>
fm_kegiatanedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$m_kegiatan_edit->IsModal) { ?>
<?php } ?>
<?php $m_kegiatan_edit->ShowPageHeader(); ?>
<?php
$m_kegiatan_edit->ShowMessage();
?>
<form name="fm_kegiatanedit" id="fm_kegiatanedit" class="<?php echo $m_kegiatan_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($m_kegiatan_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $m_kegiatan_edit->Token ?>">
<?php } ?>
...
<?php if ($m_kegiatan->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_m_kegiatan_id" class="col-sm-2 control-label ewLabel"><?php echo $m_kegiatan->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $m_kegiatan->id->CellAttributes() ?>>
...
<input type="hidden" data-table="m_kegiatan" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($m_kegiatan->id->CurrentValue) ?>">
<?php echo $m_kegiatan->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php
	if (in_array("m_sub_kegiatan", explode(",", $m_kegiatan->getCurrentDetailTable())) && $m_sub_kegiatan->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $m_kegiatan_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fm_kegiatanedit.Init();
</script>
<?php
$m_kegiatan_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$m_kegiatan_edit->Page_Terminate();
?>
