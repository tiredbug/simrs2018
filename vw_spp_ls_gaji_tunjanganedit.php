...
$vw_spp_ls_gaji_tunjangan_edit = NULL; // Initialize page object first

class cvw_spp_ls_gaji_tunjangan_edit extends cvw_spp_ls_gaji_tunjangan {
	...
	// Page object name
	var $PageObjName = 'vw_spp_ls_gaji_tunjangan_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("vw_spp_ls_gaji_tunjanganlist.php"); // Invalid key, return to list
		}
	...
	function ValidateForm() {
		...
		if (in_array("t_spp_detail", $DetailTblVar) && $GLOBALS["t_spp_detail"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// tgl_spp
			$this->tgl_spp->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tgl_spp->CurrentValue, 0), NULL, $this->tgl_spp->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("t_spp_detail", $DetailTblVar) && $GLOBALS["t_spp_detail"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["t_spp_detail_grid"]->GridUpdate();
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("t_spp_detail", $DetailTblVar)) {
				...
				if ($GLOBALS["t_spp_detail_grid"]->DetailEdit) {
					$GLOBALS["t_spp_detail_grid"]->CurrentMode = "edit";
					$GLOBALS["t_spp_detail_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($vw_spp_ls_gaji_tunjangan_edit)) $vw_spp_ls_gaji_tunjangan_edit = new cvw_spp_ls_gaji_tunjangan_edit();

// Page init
$vw_spp_ls_gaji_tunjangan_edit->Page_Init();

// Page main
$vw_spp_ls_gaji_tunjangan_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_spp_ls_gaji_tunjangan_edit->Page_Render();
?>
...
var CurrentForm = fvw_spp_ls_gaji_tunjanganedit = new ew_Form("fvw_spp_ls_gaji_tunjanganedit", "edit");

// Validate form
fvw_spp_ls_gaji_tunjanganedit.Validate = function() {
...
fvw_spp_ls_gaji_tunjanganedit.Form_CustomValidate = 
...
fvw_spp_ls_gaji_tunjanganedit.ValidateRequired = true;
<?php } else { ?>
fvw_spp_ls_gaji_tunjanganedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$vw_spp_ls_gaji_tunjangan_edit->IsModal) { ?>
<?php } ?>
<?php $vw_spp_ls_gaji_tunjangan_edit->ShowPageHeader(); ?>
<?php
$vw_spp_ls_gaji_tunjangan_edit->ShowMessage();
?>
<form name="fvw_spp_ls_gaji_tunjanganedit" id="fvw_spp_ls_gaji_tunjanganedit" class="<?php echo $vw_spp_ls_gaji_tunjangan_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_spp_ls_gaji_tunjangan_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_spp_ls_gaji_tunjangan_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<div>
<?php if ($vw_spp_ls_gaji_tunjangan->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_vw_spp_ls_gaji_tunjangan_id" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_ls_gaji_tunjangan->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_ls_gaji_tunjangan->id->CellAttributes() ?>>
<span id="el_vw_spp_ls_gaji_tunjangan_id">
<span<?php echo $vw_spp_ls_gaji_tunjangan->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_spp_ls_gaji_tunjangan->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="vw_spp_ls_gaji_tunjangan" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vw_spp_ls_gaji_tunjangan->id->CurrentValue) ?>">
<?php echo $vw_spp_ls_gaji_tunjangan->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php
	if (in_array("t_spp_detail", explode(",", $vw_spp_ls_gaji_tunjangan->getCurrentDetailTable())) && $t_spp_detail->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_spp_ls_gaji_tunjangan_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvw_spp_ls_gaji_tunjanganedit.Init();
</script>
<?php
$vw_spp_ls_gaji_tunjangan_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_spp_ls_gaji_tunjangan_edit->Page_Terminate();
?>
