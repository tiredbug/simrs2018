...
$vw_spp_gu_list_edit = NULL; // Initialize page object first

class cvw_spp_gu_list_edit extends cvw_spp_gu_list {
	...
	// Page object name
	var $PageObjName = 'vw_spp_gu_list_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("vw_spp_gu_listlist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("vw_spp_gu_listlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "vw_spp_gu_listlist.php")
	...
	function EditRow() {
		...
		if ($this->id_spj->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`id_spj` = " . ew_AdjustSql($this->id_spj->CurrentValue, $this->DBID) . ")";
		...
		if ($rs->EOF) {
			...
			// id_spd
			$this->id_spd->SetDbValueDef($rsnew, $this->id_spd->CurrentValue, NULL, $this->id_spd->ReadOnly);
...
// Create page object
if (!isset($vw_spp_gu_list_edit)) $vw_spp_gu_list_edit = new cvw_spp_gu_list_edit();

// Page init
$vw_spp_gu_list_edit->Page_Init();

// Page main
$vw_spp_gu_list_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_spp_gu_list_edit->Page_Render();
?>
...
var CurrentForm = fvw_spp_gu_listedit = new ew_Form("fvw_spp_gu_listedit", "edit");

// Validate form
fvw_spp_gu_listedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckEuroDate(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_spp_gu_list->tgl_spp->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_tahun_anggaran");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_spp_gu_list->tahun_anggaran->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_id_spj");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_spp_gu_list->id_spj->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_id_spd");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_spp_gu_list->id_spd->FldErrMsg()) ?>");
...
fvw_spp_gu_listedit.Form_CustomValidate = 
...
fvw_spp_gu_listedit.ValidateRequired = true;
<?php } else { ?>
fvw_spp_gu_listedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_spp_gu_listedit.Lists["x_id_jenis_spp"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_jenis_spp","","",""],"ParentFields":[],"ChildFields":["x_detail_jenis_spp"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"l_jenis_spp"};
fvw_spp_gu_listedit.Lists["x_detail_jenis_spp"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_detail_jenis_spp","","",""],"ParentFields":["x_id_jenis_spp"],"ChildFields":[],"FilterFields":["x_id_jenis"],"Options":[],"Template":"","LinkTable":"l_jenis_detail_spp"};
fvw_spp_gu_listedit.Lists["x_status_spp"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvw_spp_gu_listedit.Lists["x_status_spp"].Options = <?php echo json_encode($vw_spp_gu_list->status_spp->Options()) ?>;
fvw_spp_gu_listedit.Lists["x_nama_bendahara"] = {"LinkField":"x_nama","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"m_pejabat_keuangan"};
fvw_spp_gu_listedit.Lists["x_nama_pptk"] = {"LinkField":"x_nama","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"m_pejabat_keuangan"};
fvw_spp_gu_listedit.Lists["x_kode_program"] = {"LinkField":"x_kode_program","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_program","","",""],"ParentFields":[],"ChildFields":["x_kode_kegiatan","x_kode_sub_kegiatan"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"m_program"};
fvw_spp_gu_listedit.Lists["x_kode_kegiatan"] = {"LinkField":"x_kode_kegiatan","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_kegiatan","","",""],"ParentFields":["x_kode_program"],"ChildFields":["x_kode_sub_kegiatan"],"FilterFields":["x_kode_program"],"Options":[],"Template":"","LinkTable":"m_kegiatan"};
fvw_spp_gu_listedit.Lists["x_kode_sub_kegiatan"] = {"LinkField":"x_kode_sub_kegiatan","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_sub_kegiatan","","",""],"ParentFields":["x_kode_kegiatan","x_kode_program"],"ChildFields":[],"FilterFields":["x_kode_kegiatan","x_kode_program"],"Options":[],"Template":"","LinkTable":"m_sub_kegiatan"};
fvw_spp_gu_listedit.Lists["x_id_spj"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_no_spj","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_spj"};
fvw_spp_gu_listedit.Lists["x_id_spd"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_no_spd","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_spd"};
...
<?php if (!$vw_spp_gu_list_edit->IsModal) { ?>
<?php } ?>
<?php $vw_spp_gu_list_edit->ShowPageHeader(); ?>
<?php
$vw_spp_gu_list_edit->ShowMessage();
?>
<form name="fvw_spp_gu_listedit" id="fvw_spp_gu_listedit" class="<?php echo $vw_spp_gu_list_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_spp_gu_list_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_spp_gu_list_edit->Token ?>">
<?php } ?>
...
<?php if ($vw_spp_gu_list->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_vw_spp_gu_list_id" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->id->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_id">
<span<?php echo $vw_spp_gu_list->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_spp_gu_list->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="vw_spp_gu_list" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vw_spp_gu_list->id->CurrentValue) ?>">
<?php echo $vw_spp_gu_list->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->id_jenis_spp->Visible) { // id_jenis_spp ?>
	<div id="r_id_jenis_spp" class="form-group">
		<label id="elh_vw_spp_gu_list_id_jenis_spp" for="x_id_jenis_spp" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->id_jenis_spp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->id_jenis_spp->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_id_jenis_spp">
<?php $vw_spp_gu_list->id_jenis_spp->EditAttrs["onchange"] = "ew_UpdateOpt.call(this); " . @$vw_spp_gu_list->id_jenis_spp->EditAttrs["onchange"]; ?>
<select data-table="vw_spp_gu_list" data-field="x_id_jenis_spp" data-value-separator="<?php echo $vw_spp_gu_list->id_jenis_spp->DisplayValueSeparatorAttribute() ?>" id="x_id_jenis_spp" name="x_id_jenis_spp"<?php echo $vw_spp_gu_list->id_jenis_spp->EditAttributes() ?>>
<?php echo $vw_spp_gu_list->id_jenis_spp->SelectOptionListHtml("x_id_jenis_spp") ?>
</select>
<input type="hidden" name="s_x_id_jenis_spp" id="s_x_id_jenis_spp" value="<?php echo $vw_spp_gu_list->id_jenis_spp->LookupFilterQuery() ?>">
</span>
<?php echo $vw_spp_gu_list->id_jenis_spp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->detail_jenis_spp->Visible) { // detail_jenis_spp ?>
	<div id="r_detail_jenis_spp" class="form-group">
		<label id="elh_vw_spp_gu_list_detail_jenis_spp" for="x_detail_jenis_spp" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->detail_jenis_spp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->detail_jenis_spp->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_detail_jenis_spp">
<select data-table="vw_spp_gu_list" data-field="x_detail_jenis_spp" data-value-separator="<?php echo $vw_spp_gu_list->detail_jenis_spp->DisplayValueSeparatorAttribute() ?>" id="x_detail_jenis_spp" name="x_detail_jenis_spp"<?php echo $vw_spp_gu_list->detail_jenis_spp->EditAttributes() ?>>
<?php echo $vw_spp_gu_list->detail_jenis_spp->SelectOptionListHtml("x_detail_jenis_spp") ?>
</select>
<input type="hidden" name="s_x_detail_jenis_spp" id="s_x_detail_jenis_spp" value="<?php echo $vw_spp_gu_list->detail_jenis_spp->LookupFilterQuery() ?>">
</span>
<?php echo $vw_spp_gu_list->detail_jenis_spp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->status_spp->Visible) { // status_spp ?>
	<div id="r_status_spp" class="form-group">
		<label id="elh_vw_spp_gu_list_status_spp" for="x_status_spp" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->status_spp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->status_spp->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_status_spp">
<select data-table="vw_spp_gu_list" data-field="x_status_spp" data-value-separator="<?php echo $vw_spp_gu_list->status_spp->DisplayValueSeparatorAttribute() ?>" id="x_status_spp" name="x_status_spp"<?php echo $vw_spp_gu_list->status_spp->EditAttributes() ?>>
<?php echo $vw_spp_gu_list->status_spp->SelectOptionListHtml("x_status_spp") ?>
</select>
</span>
<?php echo $vw_spp_gu_list->status_spp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->no_spp->Visible) { // no_spp ?>
	<div id="r_no_spp" class="form-group">
		<label id="elh_vw_spp_gu_list_no_spp" for="x_no_spp" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->no_spp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->no_spp->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_no_spp">
<input type="text" data-table="vw_spp_gu_list" data-field="x_no_spp" name="x_no_spp" id="x_no_spp" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->no_spp->getPlaceHolder()) ?>" value="<?php echo $vw_spp_gu_list->no_spp->EditValue ?>"<?php echo $vw_spp_gu_list->no_spp->EditAttributes() ?>>
</span>
<?php echo $vw_spp_gu_list->no_spp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->tgl_spp->Visible) { // tgl_spp ?>
	<div id="r_tgl_spp" class="form-group">
		<label id="elh_vw_spp_gu_list_tgl_spp" for="x_tgl_spp" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->tgl_spp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->tgl_spp->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_tgl_spp">
<input type="text" data-table="vw_spp_gu_list" data-field="x_tgl_spp" data-format="7" name="x_tgl_spp" id="x_tgl_spp" placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->tgl_spp->getPlaceHolder()) ?>" value="<?php echo $vw_spp_gu_list->tgl_spp->EditValue ?>"<?php echo $vw_spp_gu_list->tgl_spp->EditAttributes() ?>>
<?php if (!$vw_spp_gu_list->tgl_spp->ReadOnly && !$vw_spp_gu_list->tgl_spp->Disabled && !isset($vw_spp_gu_list->tgl_spp->EditAttrs["readonly"]) && !isset($vw_spp_gu_list->tgl_spp->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateCalendar("fvw_spp_gu_listedit", "x_tgl_spp", 7);
</script>
<?php } ?>
</span>
<?php echo $vw_spp_gu_list->tgl_spp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->keterangan->Visible) { // keterangan ?>
	<div id="r_keterangan" class="form-group">
		<label id="elh_vw_spp_gu_list_keterangan" for="x_keterangan" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->keterangan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->keterangan->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_keterangan">
<input type="text" data-table="vw_spp_gu_list" data-field="x_keterangan" name="x_keterangan" id="x_keterangan" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->keterangan->getPlaceHolder()) ?>" value="<?php echo $vw_spp_gu_list->keterangan->EditValue ?>"<?php echo $vw_spp_gu_list->keterangan->EditAttributes() ?>>
</span>
<?php echo $vw_spp_gu_list->keterangan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->nama_bendahara->Visible) { // nama_bendahara ?>
	<div id="r_nama_bendahara" class="form-group">
		<label id="elh_vw_spp_gu_list_nama_bendahara" for="x_nama_bendahara" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->nama_bendahara->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->nama_bendahara->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_nama_bendahara">
<?php $vw_spp_gu_list->nama_bendahara->EditAttrs["onchange"] = "ew_AutoFill(this); " . @$vw_spp_gu_list->nama_bendahara->EditAttrs["onchange"]; ?>
<select data-table="vw_spp_gu_list" data-field="x_nama_bendahara" data-value-separator="<?php echo $vw_spp_gu_list->nama_bendahara->DisplayValueSeparatorAttribute() ?>" id="x_nama_bendahara" name="x_nama_bendahara"<?php echo $vw_spp_gu_list->nama_bendahara->EditAttributes() ?>>
<?php echo $vw_spp_gu_list->nama_bendahara->SelectOptionListHtml("x_nama_bendahara") ?>
</select>
<input type="hidden" name="s_x_nama_bendahara" id="s_x_nama_bendahara" value="<?php echo $vw_spp_gu_list->nama_bendahara->LookupFilterQuery() ?>">
<input type="hidden" name="ln_x_nama_bendahara" id="ln_x_nama_bendahara" value="x_nip_bendahara">
</span>
<?php echo $vw_spp_gu_list->nama_bendahara->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->nip_bendahara->Visible) { // nip_bendahara ?>
	<div id="r_nip_bendahara" class="form-group">
		<label id="elh_vw_spp_gu_list_nip_bendahara" for="x_nip_bendahara" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->nip_bendahara->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->nip_bendahara->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_nip_bendahara">
<input type="text" data-table="vw_spp_gu_list" data-field="x_nip_bendahara" name="x_nip_bendahara" id="x_nip_bendahara" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->nip_bendahara->getPlaceHolder()) ?>" value="<?php echo $vw_spp_gu_list->nip_bendahara->EditValue ?>"<?php echo $vw_spp_gu_list->nip_bendahara->EditAttributes() ?>>
</span>
<?php echo $vw_spp_gu_list->nip_bendahara->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->nama_pptk->Visible) { // nama_pptk ?>
	<div id="r_nama_pptk" class="form-group">
		<label id="elh_vw_spp_gu_list_nama_pptk" for="x_nama_pptk" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->nama_pptk->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->nama_pptk->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_nama_pptk">
<?php $vw_spp_gu_list->nama_pptk->EditAttrs["onchange"] = "ew_AutoFill(this); " . @$vw_spp_gu_list->nama_pptk->EditAttrs["onchange"]; ?>
<select data-table="vw_spp_gu_list" data-field="x_nama_pptk" data-value-separator="<?php echo $vw_spp_gu_list->nama_pptk->DisplayValueSeparatorAttribute() ?>" id="x_nama_pptk" name="x_nama_pptk"<?php echo $vw_spp_gu_list->nama_pptk->EditAttributes() ?>>
<?php echo $vw_spp_gu_list->nama_pptk->SelectOptionListHtml("x_nama_pptk") ?>
</select>
<input type="hidden" name="s_x_nama_pptk" id="s_x_nama_pptk" value="<?php echo $vw_spp_gu_list->nama_pptk->LookupFilterQuery() ?>">
<input type="hidden" name="ln_x_nama_pptk" id="ln_x_nama_pptk" value="x_nip_pptk">
</span>
<?php echo $vw_spp_gu_list->nama_pptk->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->nip_pptk->Visible) { // nip_pptk ?>
	<div id="r_nip_pptk" class="form-group">
		<label id="elh_vw_spp_gu_list_nip_pptk" for="x_nip_pptk" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->nip_pptk->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->nip_pptk->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_nip_pptk">
<input type="text" data-table="vw_spp_gu_list" data-field="x_nip_pptk" name="x_nip_pptk" id="x_nip_pptk" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->nip_pptk->getPlaceHolder()) ?>" value="<?php echo $vw_spp_gu_list->nip_pptk->EditValue ?>"<?php echo $vw_spp_gu_list->nip_pptk->EditAttributes() ?>>
</span>
<?php echo $vw_spp_gu_list->nip_pptk->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->kode_program->Visible) { // kode_program ?>
	<div id="r_kode_program" class="form-group">
		<label id="elh_vw_spp_gu_list_kode_program" for="x_kode_program" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->kode_program->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->kode_program->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_kode_program">
<?php $vw_spp_gu_list->kode_program->EditAttrs["onchange"] = "ew_UpdateOpt.call(this); " . @$vw_spp_gu_list->kode_program->EditAttrs["onchange"]; ?>
<select data-table="vw_spp_gu_list" data-field="x_kode_program" data-value-separator="<?php echo $vw_spp_gu_list->kode_program->DisplayValueSeparatorAttribute() ?>" id="x_kode_program" name="x_kode_program"<?php echo $vw_spp_gu_list->kode_program->EditAttributes() ?>>
<?php echo $vw_spp_gu_list->kode_program->SelectOptionListHtml("x_kode_program") ?>
</select>
<input type="hidden" name="s_x_kode_program" id="s_x_kode_program" value="<?php echo $vw_spp_gu_list->kode_program->LookupFilterQuery() ?>">
</span>
<?php echo $vw_spp_gu_list->kode_program->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->kode_kegiatan->Visible) { // kode_kegiatan ?>
	<div id="r_kode_kegiatan" class="form-group">
		<label id="elh_vw_spp_gu_list_kode_kegiatan" for="x_kode_kegiatan" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->kode_kegiatan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->kode_kegiatan->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_kode_kegiatan">
<?php $vw_spp_gu_list->kode_kegiatan->EditAttrs["onchange"] = "ew_UpdateOpt.call(this); " . @$vw_spp_gu_list->kode_kegiatan->EditAttrs["onchange"]; ?>
<select data-table="vw_spp_gu_list" data-field="x_kode_kegiatan" data-value-separator="<?php echo $vw_spp_gu_list->kode_kegiatan->DisplayValueSeparatorAttribute() ?>" id="x_kode_kegiatan" name="x_kode_kegiatan"<?php echo $vw_spp_gu_list->kode_kegiatan->EditAttributes() ?>>
<?php echo $vw_spp_gu_list->kode_kegiatan->SelectOptionListHtml("x_kode_kegiatan") ?>
</select>
<input type="hidden" name="s_x_kode_kegiatan" id="s_x_kode_kegiatan" value="<?php echo $vw_spp_gu_list->kode_kegiatan->LookupFilterQuery() ?>">
</span>
<?php echo $vw_spp_gu_list->kode_kegiatan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->kode_sub_kegiatan->Visible) { // kode_sub_kegiatan ?>
	<div id="r_kode_sub_kegiatan" class="form-group">
		<label id="elh_vw_spp_gu_list_kode_sub_kegiatan" for="x_kode_sub_kegiatan" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->kode_sub_kegiatan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->kode_sub_kegiatan->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_kode_sub_kegiatan">
<select data-table="vw_spp_gu_list" data-field="x_kode_sub_kegiatan" data-value-separator="<?php echo $vw_spp_gu_list->kode_sub_kegiatan->DisplayValueSeparatorAttribute() ?>" id="x_kode_sub_kegiatan" name="x_kode_sub_kegiatan"<?php echo $vw_spp_gu_list->kode_sub_kegiatan->EditAttributes() ?>>
<?php echo $vw_spp_gu_list->kode_sub_kegiatan->SelectOptionListHtml("x_kode_sub_kegiatan") ?>
</select>
<input type="hidden" name="s_x_kode_sub_kegiatan" id="s_x_kode_sub_kegiatan" value="<?php echo $vw_spp_gu_list->kode_sub_kegiatan->LookupFilterQuery() ?>">
</span>
<?php echo $vw_spp_gu_list->kode_sub_kegiatan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->tahun_anggaran->Visible) { // tahun_anggaran ?>
	<div id="r_tahun_anggaran" class="form-group">
		<label id="elh_vw_spp_gu_list_tahun_anggaran" for="x_tahun_anggaran" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->tahun_anggaran->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->tahun_anggaran->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_tahun_anggaran">
<input type="text" data-table="vw_spp_gu_list" data-field="x_tahun_anggaran" name="x_tahun_anggaran" id="x_tahun_anggaran" size="30" placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->tahun_anggaran->getPlaceHolder()) ?>" value="<?php echo $vw_spp_gu_list->tahun_anggaran->EditValue ?>"<?php echo $vw_spp_gu_list->tahun_anggaran->EditAttributes() ?>>
</span>
<?php echo $vw_spp_gu_list->tahun_anggaran->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->id_spj->Visible) { // id_spj ?>
	<div id="r_id_spj" class="form-group">
		<label id="elh_vw_spp_gu_list_id_spj" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->id_spj->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->id_spj->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_id_spj">
<?php
$wrkonchange = trim(" " . @$vw_spp_gu_list->id_spj->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$vw_spp_gu_list->id_spj->EditAttrs["onchange"] = "";
?>
<span id="as_x_id_spj" style="white-space: nowrap; z-index: 8840">
	<input type="text" name="sv_x_id_spj" id="sv_x_id_spj" value="<?php echo $vw_spp_gu_list->id_spj->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->id_spj->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->id_spj->getPlaceHolder()) ?>"<?php echo $vw_spp_gu_list->id_spj->EditAttributes() ?>>
</span>
<input type="hidden" data-table="vw_spp_gu_list" data-field="x_id_spj" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $vw_spp_gu_list->id_spj->DisplayValueSeparatorAttribute() ?>" name="x_id_spj" id="x_id_spj" value="<?php echo ew_HtmlEncode($vw_spp_gu_list->id_spj->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x_id_spj" id="q_x_id_spj" value="<?php echo $vw_spp_gu_list->id_spj->LookupFilterQuery(true) ?>">
<script type="text/javascript">
fvw_spp_gu_listedit.CreateAutoSuggest({"id":"x_id_spj","forceSelect":false});
</script>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($vw_spp_gu_list->id_spj->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_id_spj',m:0,n:10,srch:true});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" name="s_x_id_spj" id="s_x_id_spj" value="<?php echo $vw_spp_gu_list->id_spj->LookupFilterQuery(false) ?>">
</span>
<?php echo $vw_spp_gu_list->id_spj->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_spp_gu_list->id_spd->Visible) { // id_spd ?>
	<div id="r_id_spd" class="form-group">
		<label id="elh_vw_spp_gu_list_id_spd" class="col-sm-2 control-label ewLabel"><?php echo $vw_spp_gu_list->id_spd->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_spp_gu_list->id_spd->CellAttributes() ?>>
<span id="el_vw_spp_gu_list_id_spd">
<?php
$wrkonchange = trim(" " . @$vw_spp_gu_list->id_spd->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$vw_spp_gu_list->id_spd->EditAttrs["onchange"] = "";
?>
<span id="as_x_id_spd" style="white-space: nowrap; z-index: 8830">
	<input type="text" name="sv_x_id_spd" id="sv_x_id_spd" value="<?php echo $vw_spp_gu_list->id_spd->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->id_spd->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($vw_spp_gu_list->id_spd->getPlaceHolder()) ?>"<?php echo $vw_spp_gu_list->id_spd->EditAttributes() ?>>
</span>
<input type="hidden" data-table="vw_spp_gu_list" data-field="x_id_spd" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $vw_spp_gu_list->id_spd->DisplayValueSeparatorAttribute() ?>" name="x_id_spd" id="x_id_spd" value="<?php echo ew_HtmlEncode($vw_spp_gu_list->id_spd->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x_id_spd" id="q_x_id_spd" value="<?php echo $vw_spp_gu_list->id_spd->LookupFilterQuery(true) ?>">
<script type="text/javascript">
fvw_spp_gu_listedit.CreateAutoSuggest({"id":"x_id_spd","forceSelect":true});
</script>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($vw_spp_gu_list->id_spd->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_id_spd',m:0,n:10,srch:false});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" name="s_x_id_spd" id="s_x_id_spd" value="<?php echo $vw_spp_gu_list->id_spd->LookupFilterQuery(false) ?>">
</span>
<?php echo $vw_spp_gu_list->id_spd->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$vw_spp_gu_list_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_spp_gu_list_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvw_spp_gu_listedit.Init();
</script>
<?php
$vw_spp_gu_list_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_spp_gu_list_edit->Page_Terminate();
?>
