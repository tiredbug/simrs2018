...
$vw_spp_ls_barang_jasa_detail_delete = NULL; // Initialize page object first

class cvw_spp_ls_barang_jasa_detail_delete extends cvw_spp_ls_barang_jasa_detail {
	...
	// Page object name
	var $PageObjName = 'vw_spp_ls_barang_jasa_detail_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("vw_spp_ls_barang_jasa_detaillist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in vw_spp_ls_barang_jasa_detail class, vw_spp_ls_barang_jasa_detailinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("vw_spp_ls_barang_jasa_detaillist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($vw_spp_ls_barang_jasa_detail_delete)) $vw_spp_ls_barang_jasa_detail_delete = new cvw_spp_ls_barang_jasa_detail_delete();

// Page init
$vw_spp_ls_barang_jasa_detail_delete->Page_Init();

// Page main
$vw_spp_ls_barang_jasa_detail_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_spp_ls_barang_jasa_detail_delete->Page_Render();
?>
...
var CurrentForm = fvw_spp_ls_barang_jasa_detaildelete = new ew_Form("fvw_spp_ls_barang_jasa_detaildelete", "delete");

// Form_CustomValidate event
fvw_spp_ls_barang_jasa_detaildelete.Form_CustomValidate = 
...
fvw_spp_ls_barang_jasa_detaildelete.ValidateRequired = true;
<?php } else { ?>
fvw_spp_ls_barang_jasa_detaildelete.ValidateRequired = false; 
<?php } ?>
...
<?php $vw_spp_ls_barang_jasa_detail_delete->ShowPageHeader(); ?>
<?php
$vw_spp_ls_barang_jasa_detail_delete->ShowMessage();
?>
<form name="fvw_spp_ls_barang_jasa_detaildelete" id="fvw_spp_ls_barang_jasa_detaildelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_spp_ls_barang_jasa_detail_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_spp_ls_barang_jasa_detail_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_spp_ls_barang_jasa_detail">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($vw_spp_ls_barang_jasa_detail_delete->RecKeys as $key) { ?>
...
<?php if ($vw_spp_ls_barang_jasa_detail->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
		<th><span id="elh_vw_spp_ls_barang_jasa_detail_kd_rekening_belanja" class="vw_spp_ls_barang_jasa_detail_kd_rekening_belanja"><?php echo $vw_spp_ls_barang_jasa_detail->kd_rekening_belanja->FldCaption() ?></span></th>
<?php } ?>
<?php if ($vw_spp_ls_barang_jasa_detail->jumlah_belanja->Visible) { // jumlah_belanja ?>
		<th><span id="elh_vw_spp_ls_barang_jasa_detail_jumlah_belanja" class="vw_spp_ls_barang_jasa_detail_jumlah_belanja"><?php echo $vw_spp_ls_barang_jasa_detail->jumlah_belanja->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$vw_spp_ls_barang_jasa_detail_delete->RecCnt = 0;
$i = 0;
while (!$vw_spp_ls_barang_jasa_detail_delete->Recordset->EOF) {
	$vw_spp_ls_barang_jasa_detail_delete->RecCnt++;
	$vw_spp_ls_barang_jasa_detail_delete->RowCnt++;

	// Set row properties
	$vw_spp_ls_barang_jasa_detail->ResetAttrs();
	$vw_spp_ls_barang_jasa_detail->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$vw_spp_ls_barang_jasa_detail_delete->LoadRowValues($vw_spp_ls_barang_jasa_detail_delete->Recordset);

	// Render row
	$vw_spp_ls_barang_jasa_detail_delete->RenderRow();
?>
	<tr<?php echo $vw_spp_ls_barang_jasa_detail->RowAttributes() ?>>
<?php if ($vw_spp_ls_barang_jasa_detail->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
		<td<?php echo $vw_spp_ls_barang_jasa_detail->kd_rekening_belanja->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_ls_barang_jasa_detail_delete->RowCnt ?>_vw_spp_ls_barang_jasa_detail_kd_rekening_belanja" class="vw_spp_ls_barang_jasa_detail_kd_rekening_belanja">
...
<?php } ?>
<?php if ($vw_spp_ls_barang_jasa_detail->jumlah_belanja->Visible) { // jumlah_belanja ?>
		<td<?php echo $vw_spp_ls_barang_jasa_detail->jumlah_belanja->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_ls_barang_jasa_detail_delete->RowCnt ?>_vw_spp_ls_barang_jasa_detail_jumlah_belanja" class="vw_spp_ls_barang_jasa_detail_jumlah_belanja">
...
<?php
	$vw_spp_ls_barang_jasa_detail_delete->Recordset->MoveNext();
}
$vw_spp_ls_barang_jasa_detail_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_spp_ls_barang_jasa_detail_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fvw_spp_ls_barang_jasa_detaildelete.Init();
</script>
<?php
$vw_spp_ls_barang_jasa_detail_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_spp_ls_barang_jasa_detail_delete->Page_Terminate();
?>
