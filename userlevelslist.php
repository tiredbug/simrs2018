...
class cuserlevels_list extends cuserlevels {
	...
	function RenderListOptions() {
		...
		if ($this->userlevelid->CurrentValue < 0 && $this->userlevelid->CurrentValue <> -2) {
			...
			$oListOpt->Body = "<a class=\"btn btn-xs btn-primary\" title=\"" . ew_HtmlTitle($Language->Phrase("Permission")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("Permission")) . "\" href=\"" . ew_HtmlEncode("userpriv.php?userlevelid=" . $this->userlevelid->CurrentValue) . "\">" . $Language->Phrase("Permission") . "</a>";
		}

		// "checkbox"
		$oListOpt = &$this->ListOptions->Items["checkbox"];
		$oListOpt->Body = "<label><input class=\"magic-checkbox ewPointer\" type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->userlevelid->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'><span></span></label>";
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fuserlevelslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
<form name="fuserlevelslistsrch" id="fuserlevelslistsrch" class="form-horizontal ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($userlevels_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fuserlevelslistsrch_SearchPanel" class="box box-default collapse<?php echo $SearchPanelClass ?>">
...
<div class="panel panel-default ewGrid userlevels">
...
<?php } ?>
<input type="hidden" name="t" value="userlevels">
<div id="gmp_userlevels" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>box box-primary">
<?php if ($userlevels_list->TotalRecs > 0 || $userlevels->CurrentAction == "gridedit") { ?>
<table id="tbl_userlevelslist" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
<?php if ($userlevels->userlevelid->Visible) { // userlevelid ?>
	...
	<?php } else { ?>
		<th data-name="userlevelid"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $userlevels->SortUrl($userlevels->userlevelid) ?>',2);"><div id="elh_userlevels_userlevelid" class="userlevels_userlevelid">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $userlevels->userlevelid->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($userlevels->userlevelid->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($userlevels->userlevelid->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($userlevels->userlevelname->Visible) { // userlevelname ?>
	...
	<?php } else { ?>
		<th data-name="userlevelname"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $userlevels->SortUrl($userlevels->userlevelname) ?>',2);"><div id="elh_userlevels_userlevelname" class="userlevels_userlevelname">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $userlevels->userlevelname->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($userlevels->userlevelname->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($userlevels->userlevelname->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $userlevels_list->PageUrl() ?>start=<?php echo $userlevels_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $userlevels_list->PageUrl() ?>start=<?php echo $userlevels_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--next page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $userlevels_list->PageUrl() ?>start=<?php echo $userlevels_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
...
<!--last page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $userlevels_list->PageUrl() ?>start=<?php echo $userlevels_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<?php } ?>
<?php if ($userlevels_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $userlevels_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="userlevels">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($userlevels_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
