...
$t_spp_detail_delete = NULL; // Initialize page object first

class ct_spp_detail_delete extends ct_spp_detail {
	...
	// Page object name
	var $PageObjName = 't_spp_detail_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_spp_detaillist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_spp_detail class, t_spp_detailinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_spp_detaillist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_spp_detail_delete)) $t_spp_detail_delete = new ct_spp_detail_delete();

// Page init
$t_spp_detail_delete->Page_Init();

// Page main
$t_spp_detail_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_spp_detail_delete->Page_Render();
?>
...
var CurrentForm = ft_spp_detaildelete = new ew_Form("ft_spp_detaildelete", "delete");

// Form_CustomValidate event
ft_spp_detaildelete.Form_CustomValidate = 
...
ft_spp_detaildelete.ValidateRequired = true;
<?php } else { ?>
ft_spp_detaildelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_spp_detaildelete.Lists["x_kd_rekening_belanja"] = {"LinkField":"x_kd_akun","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_akun","x_kd_akun","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"keu_akun5"};
...
<?php $t_spp_detail_delete->ShowPageHeader(); ?>
<?php
$t_spp_detail_delete->ShowMessage();
?>
<form name="ft_spp_detaildelete" id="ft_spp_detaildelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_spp_detail_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_spp_detail_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_spp_detail">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_spp_detail_delete->RecKeys as $key) { ?>
...
<?php if ($t_spp_detail->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
		<th><span id="elh_t_spp_detail_kd_rekening_belanja" class="t_spp_detail_kd_rekening_belanja"><?php echo $t_spp_detail->kd_rekening_belanja->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_spp_detail->jumlah_belanja->Visible) { // jumlah_belanja ?>
		<th><span id="elh_t_spp_detail_jumlah_belanja" class="t_spp_detail_jumlah_belanja"><?php echo $t_spp_detail->jumlah_belanja->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_spp_detail_delete->RecCnt = 0;
$i = 0;
while (!$t_spp_detail_delete->Recordset->EOF) {
	$t_spp_detail_delete->RecCnt++;
	$t_spp_detail_delete->RowCnt++;

	// Set row properties
	$t_spp_detail->ResetAttrs();
	$t_spp_detail->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_spp_detail_delete->LoadRowValues($t_spp_detail_delete->Recordset);

	// Render row
	$t_spp_detail_delete->RenderRow();
?>
	<tr<?php echo $t_spp_detail->RowAttributes() ?>>
<?php if ($t_spp_detail->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
		<td<?php echo $t_spp_detail->kd_rekening_belanja->CellAttributes() ?>>
<span id="el<?php echo $t_spp_detail_delete->RowCnt ?>_t_spp_detail_kd_rekening_belanja" class="t_spp_detail_kd_rekening_belanja">
...
<?php } ?>
<?php if ($t_spp_detail->jumlah_belanja->Visible) { // jumlah_belanja ?>
		<td<?php echo $t_spp_detail->jumlah_belanja->CellAttributes() ?>>
<span id="el<?php echo $t_spp_detail_delete->RowCnt ?>_t_spp_detail_jumlah_belanja" class="t_spp_detail_jumlah_belanja">
...
<?php
	$t_spp_detail_delete->Recordset->MoveNext();
}
$t_spp_detail_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_spp_detail_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_spp_detaildelete.Init();
</script>
<?php
$t_spp_detail_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_spp_detail_delete->Page_Terminate();
?>
