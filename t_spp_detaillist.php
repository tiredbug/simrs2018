...
$t_spp_detail_list = NULL; // Initialize page object first

class ct_spp_detail_list extends ct_spp_detail {
	...
	// Page object name
	var $PageObjName = 't_spp_detail_list';

	// Grid form hidden field names
	var $FormName = 'ft_spp_detaillist';
	...
	function __construct() {
		...
		$this->AddUrl = "t_spp_detailadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "t_spp_detaildelete.php";
		$this->MultiUpdateUrl = "t_spp_detailupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft_spp_detaillistsrch";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "vw_spp_up_list") {
			global $vw_spp_up_list;
			$rsmaster = $vw_spp_up_list->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("vw_spp_up_listlist.php"); // Return to master page
			} else {
				$vw_spp_up_list->LoadListRowValues($rsmaster);
				$vw_spp_up_list->RowType = EW_ROWTYPE_MASTER; // Master row
				$vw_spp_up_list->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft_spp_detaillistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft_spp_detaillistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft_spp_detaillist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
// Create page object
if (!isset($t_spp_detail_list)) $t_spp_detail_list = new ct_spp_detail_list();

// Page init
$t_spp_detail_list->Page_Init();

// Page main
$t_spp_detail_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_spp_detail_list->Page_Render();
?>
...
var CurrentForm = ft_spp_detaillist = new ew_Form("ft_spp_detaillist", "list");
ft_spp_detaillist.FormKeyCountName = '<?php echo $t_spp_detail_list->FormKeyCountName ?>';

// Form_CustomValidate event
ft_spp_detaillist.Form_CustomValidate = 
...
ft_spp_detaillist.ValidateRequired = true;
<?php } else { ?>
ft_spp_detaillist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_spp_detaillist.Lists["x_kd_rekening_belanja"] = {"LinkField":"x_kd_akun","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_akun","x_kd_akun","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"keu_akun5"};
...
<?php if ($t_spp_detail_list->TotalRecs > 0 && $t_spp_detail_list->ExportOptions->Visible()) { ?>
<?php $t_spp_detail_list->ExportOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php if (($t_spp_detail->Export == "") || (EW_EXPORT_MASTER_RECORD && $t_spp_detail->Export == "print")) { ?>
<?php
if ($t_spp_detail_list->DbMasterFilter <> "" && $t_spp_detail->getCurrentMasterTable() == "vw_spp_up_list") {
	if ($t_spp_detail_list->MasterRecordExists) {
?>
<?php include_once "vw_spp_up_listmaster.php" ?>
<?php
	}
}
?>
<?php } ?>
<?php
	$bSelectLimit = $t_spp_detail_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($t_spp_detail_list->TotalRecs <= 0)
			$t_spp_detail_list->TotalRecs = $t_spp_detail->SelectRecordCount();
	} else {
		if (!$t_spp_detail_list->Recordset && ($t_spp_detail_list->Recordset = $t_spp_detail_list->LoadRecordset()))
			$t_spp_detail_list->TotalRecs = $t_spp_detail_list->Recordset->RecordCount();
	}
	$t_spp_detail_list->StartRec = 1;
	if ($t_spp_detail_list->DisplayRecs <= 0 || ($t_spp_detail->Export <> "" && $t_spp_detail->ExportAll)) // Display all records
		$t_spp_detail_list->DisplayRecs = $t_spp_detail_list->TotalRecs;
	if (!($t_spp_detail->Export <> "" && $t_spp_detail->ExportAll))
		$t_spp_detail_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t_spp_detail_list->Recordset = $t_spp_detail_list->LoadRecordset($t_spp_detail_list->StartRec-1, $t_spp_detail_list->DisplayRecs);

	// Set no record found message
	if ($t_spp_detail->CurrentAction == "" && $t_spp_detail_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$t_spp_detail_list->setWarningMessage(ew_DeniedMsg());
		if ($t_spp_detail_list->SearchWhere == "0=101")
			$t_spp_detail_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$t_spp_detail_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$t_spp_detail_list->RenderOtherOptions();
?>
<?php $t_spp_detail_list->ShowPageHeader(); ?>
<?php
$t_spp_detail_list->ShowMessage();
?>
<?php if ($t_spp_detail_list->TotalRecs > 0 || $t_spp_detail->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid t_spp_detail">
<form name="ft_spp_detaillist" id="ft_spp_detaillist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_spp_detail_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_spp_detail_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_spp_detail">
<?php if ($t_spp_detail->getCurrentMasterTable() == "vw_spp_up_list" && $t_spp_detail->CurrentAction <> "") { ?>
...
<?php if ($t_spp_detail_list->TotalRecs > 0 || $t_spp_detail->CurrentAction == "gridedit") { ?>
<table id="tbl_t_spp_detaillist" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$t_spp_detail_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$t_spp_detail_list->RenderListOptions();

// Render list options (header, left)
$t_spp_detail_list->ListOptions->Render("header", "left");
?>
<?php if ($t_spp_detail->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
	...
	<?php } else { ?>
		<th data-name="kd_rekening_belanja"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t_spp_detail->SortUrl($t_spp_detail->kd_rekening_belanja) ?>',2);"><div id="elh_t_spp_detail_kd_rekening_belanja" class="t_spp_detail_kd_rekening_belanja">
...
<?php if ($t_spp_detail->jumlah_belanja->Visible) { // jumlah_belanja ?>
	...
	<?php } else { ?>
		<th data-name="jumlah_belanja"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t_spp_detail->SortUrl($t_spp_detail->jumlah_belanja) ?>',2);"><div id="elh_t_spp_detail_jumlah_belanja" class="t_spp_detail_jumlah_belanja">
...
$t_spp_detail_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($t_spp_detail->ExportAll && $t_spp_detail->Export <> "") {
	$t_spp_detail_list->StopRec = $t_spp_detail_list->TotalRecs;
} else {

	// Set the last record to display
	if ($t_spp_detail_list->TotalRecs > $t_spp_detail_list->StartRec + $t_spp_detail_list->DisplayRecs - 1)
		$t_spp_detail_list->StopRec = $t_spp_detail_list->StartRec + $t_spp_detail_list->DisplayRecs - 1;
	else
		$t_spp_detail_list->StopRec = $t_spp_detail_list->TotalRecs;
}
$t_spp_detail_list->RecCnt = $t_spp_detail_list->StartRec - 1;
if ($t_spp_detail_list->Recordset && !$t_spp_detail_list->Recordset->EOF) {
	$t_spp_detail_list->Recordset->MoveFirst();
	$bSelectLimit = $t_spp_detail_list->UseSelectLimit;
	if (!$bSelectLimit && $t_spp_detail_list->StartRec > 1)
		$t_spp_detail_list->Recordset->Move($t_spp_detail_list->StartRec - 1);
} elseif (!$t_spp_detail->AllowAddDeleteRow && $t_spp_detail_list->StopRec == 0) {
	$t_spp_detail_list->StopRec = $t_spp_detail->GridAddRowCount;
}

// Initialize aggregate
$t_spp_detail->RowType = EW_ROWTYPE_AGGREGATEINIT;
$t_spp_detail->ResetAttrs();
$t_spp_detail_list->RenderRow();
while ($t_spp_detail_list->RecCnt < $t_spp_detail_list->StopRec) {
	$t_spp_detail_list->RecCnt++;
	if (intval($t_spp_detail_list->RecCnt) >= intval($t_spp_detail_list->StartRec)) {
		$t_spp_detail_list->RowCnt++;

		// Set up key count
		$t_spp_detail_list->KeyCount = $t_spp_detail_list->RowIndex;
		...
		if ($t_spp_detail->CurrentAction == "gridadd") {
			...
			$t_spp_detail_list->LoadRowValues($t_spp_detail_list->Recordset); // Load row values
		}
		$t_spp_detail->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$t_spp_detail->RowAttrs = array_merge($t_spp_detail->RowAttrs, array('data-rowindex'=>$t_spp_detail_list->RowCnt, 'id'=>'r' . $t_spp_detail_list->RowCnt . '_t_spp_detail', 'data-rowtype'=>$t_spp_detail->RowType));
...
$t_spp_detail_list->ListOptions->Render("body", "left", $t_spp_detail_list->RowCnt);
?>
	<?php if ($t_spp_detail->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
		<td data-name="kd_rekening_belanja"<?php echo $t_spp_detail->kd_rekening_belanja->CellAttributes() ?>>
<span id="el<?php echo $t_spp_detail_list->RowCnt ?>_t_spp_detail_kd_rekening_belanja" class="t_spp_detail_kd_rekening_belanja">
...
<a id="<?php echo $t_spp_detail_list->PageObjName . "_row_" . $t_spp_detail_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($t_spp_detail->jumlah_belanja->Visible) { // jumlah_belanja ?>
		<td data-name="jumlah_belanja"<?php echo $t_spp_detail->jumlah_belanja->CellAttributes() ?>>
<span id="el<?php echo $t_spp_detail_list->RowCnt ?>_t_spp_detail_jumlah_belanja" class="t_spp_detail_jumlah_belanja">
...
$t_spp_detail_list->ListOptions->Render("body", "right", $t_spp_detail_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($t_spp_detail->CurrentAction <> "gridadd")
		$t_spp_detail_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($t_spp_detail->CurrentAction == "") { ?>
...
// Close recordset
if ($t_spp_detail_list->Recordset)
	$t_spp_detail_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($t_spp_detail->CurrentAction <> "gridadd" && $t_spp_detail->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t_spp_detail_list->Pager)) $t_spp_detail_list->Pager = new cPrevNextPager($t_spp_detail_list->StartRec, $t_spp_detail_list->DisplayRecs, $t_spp_detail_list->TotalRecs) ?>
<?php if ($t_spp_detail_list->Pager->RecordCount > 0 && $t_spp_detail_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t_spp_detail_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t_spp_detail_list->PageUrl() ?>start=<?php echo $t_spp_detail_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($t_spp_detail_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t_spp_detail_list->PageUrl() ?>start=<?php echo $t_spp_detail_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_spp_detail_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t_spp_detail_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t_spp_detail_list->PageUrl() ?>start=<?php echo $t_spp_detail_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_spp_detail_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t_spp_detail_list->PageUrl() ?>start=<?php echo $t_spp_detail_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_spp_detail_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $t_spp_detail_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $t_spp_detail_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $t_spp_detail_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($t_spp_detail_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $t_spp_detail_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="t_spp_detail">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($t_spp_detail_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($t_spp_detail_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($t_spp_detail_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($t_spp_detail_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($t_spp_detail_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($t_spp_detail_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($t_spp_detail_list->TotalRecs == 0 && $t_spp_detail->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($t_spp_detail_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
ft_spp_detaillist.Init();
</script>
<?php
$t_spp_detail_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_spp_detail_list->Page_Terminate();
?>
