...
$keu_akun5_list = NULL; // Initialize page object first

class ckeu_akun5_list extends ckeu_akun5 {
	...
	// Page object name
	var $PageObjName = 'keu_akun5_list';

	// Grid form hidden field names
	var $FormName = 'fkeu_akun5list';
	...
	function __construct() {
		...
		$this->AddUrl = "keu_akun5add.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "keu_akun5delete.php";
		$this->MultiUpdateUrl = "keu_akun5update.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fkeu_akun5listsrch";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "keu_akun4") {
			global $keu_akun4;
			$rsmaster = $keu_akun4->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("keu_akun4list.php"); // Return to master page
			} else {
				$keu_akun4->LoadListRowValues($rsmaster);
				$keu_akun4->RowType = EW_ROWTYPE_MASTER; // Master row
				$keu_akun4->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fkeu_akun5listsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
		...
		$sFilterList = ew_Concat($sFilterList, $this->kd_akun4->AdvancedSearch->ToJSON(), ","); // Field kd_akun4
		if ($this->BasicSearch->Keyword <> "") {
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fkeu_akun5listsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field kd_akun4
		$this->kd_akun4->AdvancedSearch->SearchValue = @$filter["x_kd_akun4"];
		$this->kd_akun4->AdvancedSearch->SearchOperator = @$filter["z_kd_akun4"];
		$this->kd_akun4->AdvancedSearch->SearchCondition = @$filter["v_kd_akun4"];
		$this->kd_akun4->AdvancedSearch->SearchValue2 = @$filter["y_kd_akun4"];
		$this->kd_akun4->AdvancedSearch->SearchOperator2 = @$filter["w_kd_akun4"];
		$this->kd_akun4->AdvancedSearch->Save();
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->akun1->setSessionValue("");
				$this->akun2->setSessionValue("");
				$this->akun3->setSessionValue("");
				$this->akun4->setSessionValue("");
			}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fkeu_akun5listsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fkeu_akun5listsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fkeu_akun5list}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fkeu_akun5listsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($keu_akun5_list)) $keu_akun5_list = new ckeu_akun5_list();

// Page init
$keu_akun5_list->Page_Init();

// Page main
$keu_akun5_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$keu_akun5_list->Page_Render();
?>
...
var CurrentForm = fkeu_akun5list = new ew_Form("fkeu_akun5list", "list");
fkeu_akun5list.FormKeyCountName = '<?php echo $keu_akun5_list->FormKeyCountName ?>';

// Form_CustomValidate event
fkeu_akun5list.Form_CustomValidate = 
...
fkeu_akun5list.ValidateRequired = true;
<?php } else { ?>
fkeu_akun5list.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var CurrentSearchForm = fkeu_akun5listsrch = new ew_Form("fkeu_akun5listsrch");
</script>
<script type="text/javascript">

// Write your client script here, no need to add script tags.
</script>
<?php if ($keu_akun5_list->TotalRecs > 0 && $keu_akun5_list->ExportOptions->Visible()) { ?>
<?php $keu_akun5_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($keu_akun5_list->SearchOptions->Visible()) { ?>
<?php $keu_akun5_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($keu_akun5_list->FilterOptions->Visible()) { ?>
<?php $keu_akun5_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php if (($keu_akun5->Export == "") || (EW_EXPORT_MASTER_RECORD && $keu_akun5->Export == "print")) { ?>
<?php
if ($keu_akun5_list->DbMasterFilter <> "" && $keu_akun5->getCurrentMasterTable() == "keu_akun4") {
	if ($keu_akun5_list->MasterRecordExists) {
?>
<?php include_once "keu_akun4master.php" ?>
<?php
	}
}
?>
<?php } ?>
<?php
	$bSelectLimit = $keu_akun5_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($keu_akun5_list->TotalRecs <= 0)
			$keu_akun5_list->TotalRecs = $keu_akun5->SelectRecordCount();
	} else {
		if (!$keu_akun5_list->Recordset && ($keu_akun5_list->Recordset = $keu_akun5_list->LoadRecordset()))
			$keu_akun5_list->TotalRecs = $keu_akun5_list->Recordset->RecordCount();
	}
	$keu_akun5_list->StartRec = 1;
	if ($keu_akun5_list->DisplayRecs <= 0 || ($keu_akun5->Export <> "" && $keu_akun5->ExportAll)) // Display all records
		$keu_akun5_list->DisplayRecs = $keu_akun5_list->TotalRecs;
	if (!($keu_akun5->Export <> "" && $keu_akun5->ExportAll))
		$keu_akun5_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$keu_akun5_list->Recordset = $keu_akun5_list->LoadRecordset($keu_akun5_list->StartRec-1, $keu_akun5_list->DisplayRecs);

	// Set no record found message
	if ($keu_akun5->CurrentAction == "" && $keu_akun5_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$keu_akun5_list->setWarningMessage(ew_DeniedMsg());
		if ($keu_akun5_list->SearchWhere == "0=101")
			$keu_akun5_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$keu_akun5_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$keu_akun5_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($keu_akun5->Export == "" && $keu_akun5->CurrentAction == "") { ?>
<form name="fkeu_akun5listsrch" id="fkeu_akun5listsrch" class="form-horizontal ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($keu_akun5_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fkeu_akun5listsrch_SearchPanel" class="box box-default collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="keu_akun5">
<div class="box-body">
<div id="xsr_1">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($keu_akun5_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($keu_akun5_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $keu_akun5_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($keu_akun5_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($keu_akun5_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($keu_akun5_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($keu_akun5_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $keu_akun5_list->ShowPageHeader(); ?>
<?php
$keu_akun5_list->ShowMessage();
?>
<?php if ($keu_akun5_list->TotalRecs > 0 || $keu_akun5->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid keu_akun5">
<form name="fkeu_akun5list" id="fkeu_akun5list" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($keu_akun5_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $keu_akun5_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="keu_akun5">
<?php if ($keu_akun5->getCurrentMasterTable() == "keu_akun4" && $keu_akun5->CurrentAction <> "") { ?>
...
<?php if ($keu_akun5_list->TotalRecs > 0 || $keu_akun5->CurrentAction == "gridedit") { ?>
<table id="tbl_keu_akun5list" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$keu_akun5_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$keu_akun5_list->RenderListOptions();

// Render list options (header, left)
$keu_akun5_list->ListOptions->Render("header", "left");
?>
<?php if ($keu_akun5->kd_akun->Visible) { // kd_akun ?>
	...
	<?php } else { ?>
		<th data-name="kd_akun"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $keu_akun5->SortUrl($keu_akun5->kd_akun) ?>',2);"><div id="elh_keu_akun5_kd_akun" class="keu_akun5_kd_akun">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $keu_akun5->kd_akun->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($keu_akun5->kd_akun->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($keu_akun5->kd_akun->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($keu_akun5->akun5->Visible) { // akun5 ?>
	...
	<?php } else { ?>
		<th data-name="akun5"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $keu_akun5->SortUrl($keu_akun5->akun5) ?>',2);"><div id="elh_keu_akun5_akun5" class="keu_akun5_akun5">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $keu_akun5->akun5->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($keu_akun5->akun5->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($keu_akun5->akun5->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($keu_akun5->nama_akun->Visible) { // nama_akun ?>
	...
	<?php } else { ?>
		<th data-name="nama_akun"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $keu_akun5->SortUrl($keu_akun5->nama_akun) ?>',2);"><div id="elh_keu_akun5_nama_akun" class="keu_akun5_nama_akun">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $keu_akun5->nama_akun->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($keu_akun5->nama_akun->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($keu_akun5->nama_akun->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
$keu_akun5_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($keu_akun5->ExportAll && $keu_akun5->Export <> "") {
	$keu_akun5_list->StopRec = $keu_akun5_list->TotalRecs;
} else {

	// Set the last record to display
	if ($keu_akun5_list->TotalRecs > $keu_akun5_list->StartRec + $keu_akun5_list->DisplayRecs - 1)
		$keu_akun5_list->StopRec = $keu_akun5_list->StartRec + $keu_akun5_list->DisplayRecs - 1;
	else
		$keu_akun5_list->StopRec = $keu_akun5_list->TotalRecs;
}
$keu_akun5_list->RecCnt = $keu_akun5_list->StartRec - 1;
if ($keu_akun5_list->Recordset && !$keu_akun5_list->Recordset->EOF) {
	$keu_akun5_list->Recordset->MoveFirst();
	$bSelectLimit = $keu_akun5_list->UseSelectLimit;
	if (!$bSelectLimit && $keu_akun5_list->StartRec > 1)
		$keu_akun5_list->Recordset->Move($keu_akun5_list->StartRec - 1);
} elseif (!$keu_akun5->AllowAddDeleteRow && $keu_akun5_list->StopRec == 0) {
	$keu_akun5_list->StopRec = $keu_akun5->GridAddRowCount;
}

// Initialize aggregate
$keu_akun5->RowType = EW_ROWTYPE_AGGREGATEINIT;
$keu_akun5->ResetAttrs();
$keu_akun5_list->RenderRow();
while ($keu_akun5_list->RecCnt < $keu_akun5_list->StopRec) {
	$keu_akun5_list->RecCnt++;
	if (intval($keu_akun5_list->RecCnt) >= intval($keu_akun5_list->StartRec)) {
		$keu_akun5_list->RowCnt++;

		// Set up key count
		$keu_akun5_list->KeyCount = $keu_akun5_list->RowIndex;
		...
		if ($keu_akun5->CurrentAction == "gridadd") {
			...
			$keu_akun5_list->LoadRowValues($keu_akun5_list->Recordset); // Load row values
		}
		$keu_akun5->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$keu_akun5->RowAttrs = array_merge($keu_akun5->RowAttrs, array('data-rowindex'=>$keu_akun5_list->RowCnt, 'id'=>'r' . $keu_akun5_list->RowCnt . '_keu_akun5', 'data-rowtype'=>$keu_akun5->RowType));
...
$keu_akun5_list->ListOptions->Render("body", "left", $keu_akun5_list->RowCnt);
?>
	<?php if ($keu_akun5->kd_akun->Visible) { // kd_akun ?>
		<td data-name="kd_akun"<?php echo $keu_akun5->kd_akun->CellAttributes() ?>>
<span id="el<?php echo $keu_akun5_list->RowCnt ?>_keu_akun5_kd_akun" class="keu_akun5_kd_akun">
<span<?php echo $keu_akun5->kd_akun->ViewAttributes() ?>>
<?php echo $keu_akun5->kd_akun->ListViewValue() ?></span>
</span>
<a id="<?php echo $keu_akun5_list->PageObjName . "_row_" . $keu_akun5_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($keu_akun5->akun5->Visible) { // akun5 ?>
		<td data-name="akun5"<?php echo $keu_akun5->akun5->CellAttributes() ?>>
<span id="el<?php echo $keu_akun5_list->RowCnt ?>_keu_akun5_akun5" class="keu_akun5_akun5">
...
<span id="el<?php echo $keu_akun5_list->RowCnt ?>_keu_akun5_nama_akun" class="keu_akun5_nama_akun">
...
$keu_akun5_list->ListOptions->Render("body", "right", $keu_akun5_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($keu_akun5->CurrentAction <> "gridadd")
		$keu_akun5_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($keu_akun5->CurrentAction == "") { ?>
...
// Close recordset
if ($keu_akun5_list->Recordset)
	$keu_akun5_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($keu_akun5->CurrentAction <> "gridadd" && $keu_akun5->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($keu_akun5_list->Pager)) $keu_akun5_list->Pager = new cPrevNextPager($keu_akun5_list->StartRec, $keu_akun5_list->DisplayRecs, $keu_akun5_list->TotalRecs) ?>
<?php if ($keu_akun5_list->Pager->RecordCount > 0 && $keu_akun5_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($keu_akun5_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $keu_akun5_list->PageUrl() ?>start=<?php echo $keu_akun5_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($keu_akun5_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $keu_akun5_list->PageUrl() ?>start=<?php echo $keu_akun5_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $keu_akun5_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($keu_akun5_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $keu_akun5_list->PageUrl() ?>start=<?php echo $keu_akun5_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($keu_akun5_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $keu_akun5_list->PageUrl() ?>start=<?php echo $keu_akun5_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $keu_akun5_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $keu_akun5_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $keu_akun5_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $keu_akun5_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($keu_akun5_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $keu_akun5_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="keu_akun5">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($keu_akun5_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($keu_akun5_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($keu_akun5_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($keu_akun5_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($keu_akun5_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($keu_akun5_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($keu_akun5_list->TotalRecs == 0 && $keu_akun5->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($keu_akun5_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fkeu_akun5listsrch.FilterList = <?php echo $keu_akun5_list->GetFilterList() ?>;
fkeu_akun5listsrch.Init();
fkeu_akun5list.Init();
</script>
<?php
$keu_akun5_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
