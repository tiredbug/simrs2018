...
$akun4_view = NULL; // Initialize page object first

class cakun4_view extends cakun4 {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["kel5"] <> "") {
			$this->RecKey["kel5"] = $_GET["kel5"];
			$KeyUrl .= "&amp;kel5=" . urlencode($this->RecKey["kel5"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["kel5"] <> "") {
				...
				$this->RecKey["kel5"] = $this->kel5->QueryStringValue;
			} elseif (@$_POST["kel5"] <> "") {
				$this->kel5->setFormValue($_POST["kel5"]);
				$this->RecKey["kel5"] = $this->kel5->FormValue;
			} else {
				$sReturnUrl = "akun4list.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "akun4list.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "akun4list.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($akun4_view)) $akun4_view = new cakun4_view();

// Page init
$akun4_view->Page_Init();

// Page main
$akun4_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$akun4_view->Page_Render();
?>
...
var CurrentForm = fakun4view = new ew_Form("fakun4view", "view");

// Form_CustomValidate event
fakun4view.Form_CustomValidate = 
...
fakun4view.ValidateRequired = true;
<?php } else { ?>
fakun4view.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$akun4_view->IsModal) { ?>
<?php } ?>
<?php $akun4_view->ExportOptions->Render("body") ?>
<?php
	foreach ($akun4_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$akun4_view->IsModal) { ?>
...
<?php $akun4_view->ShowPageHeader(); ?>
<?php
$akun4_view->ShowMessage();
?>
<form name="fakun4view" id="fakun4view" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($akun4_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $akun4_view->Token ?>">
<?php } ?>
