...
$akun5_view = NULL; // Initialize page object first

class cakun5_view extends cakun5 {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["kel6"] <> "") {
			$this->RecKey["kel6"] = $_GET["kel6"];
			$KeyUrl .= "&amp;kel6=" . urlencode($this->RecKey["kel6"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["kel6"] <> "") {
				...
				$this->RecKey["kel6"] = $this->kel6->QueryStringValue;
			} elseif (@$_POST["kel6"] <> "") {
				$this->kel6->setFormValue($_POST["kel6"]);
				$this->RecKey["kel6"] = $this->kel6->FormValue;
			} else {
				$sReturnUrl = "akun5list.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "akun5list.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "akun5list.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($akun5_view)) $akun5_view = new cakun5_view();

// Page init
$akun5_view->Page_Init();

// Page main
$akun5_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$akun5_view->Page_Render();
?>
...
var CurrentForm = fakun5view = new ew_Form("fakun5view", "view");

// Form_CustomValidate event
fakun5view.Form_CustomValidate = 
...
fakun5view.ValidateRequired = true;
<?php } else { ?>
fakun5view.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$akun5_view->IsModal) { ?>
<?php } ?>
<?php $akun5_view->ExportOptions->Render("body") ?>
<?php
	foreach ($akun5_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$akun5_view->IsModal) { ?>
...
<?php $akun5_view->ShowPageHeader(); ?>
<?php
$akun5_view->ShowMessage();
?>
<form name="fakun5view" id="fakun5view" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($akun5_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $akun5_view->Token ?>">
<?php } ?>
