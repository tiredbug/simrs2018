...
$m_sub_kegiatan_edit = NULL; // Initialize page object first

class cm_sub_kegiatan_edit extends cm_sub_kegiatan {
	...
	// Page object name
	var $PageObjName = 'm_sub_kegiatan_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("m_sub_kegiatanlist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("m_sub_kegiatanlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "m_sub_kegiatanlist.php")
	...
	function LoadFormValues() {
		...
		if (!$this->nama_sub_kegiatan->FldIsDetailKey) {
			$this->nama_sub_kegiatan->setFormValue($objForm->GetValue("x_nama_sub_kegiatan"));
		}
	}
	...
	function RestoreFormValues() {
		...
		$this->nama_sub_kegiatan->CurrentValue = $this->nama_sub_kegiatan->FormValue;
	}
	...
	function RenderRow() {
		...
		$this->nama_sub_kegiatan->ViewCustomAttributes = "";
			...
			if ($this->kode_kegiatan->getSessionValue() <> "") {
				$this->kode_kegiatan->CurrentValue = $this->kode_kegiatan->getSessionValue();
			$this->kode_kegiatan->ViewValue = $this->kode_kegiatan->CurrentValue;
			$this->kode_kegiatan->ViewCustomAttributes = "";
			} else {
			...
			// nama_sub_kegiatan
			$this->nama_sub_kegiatan->EditAttrs["class"] = "form-control";
			$this->nama_sub_kegiatan->EditCustomAttributes = "";
			$this->nama_sub_kegiatan->EditValue = ew_HtmlEncode($this->nama_sub_kegiatan->CurrentValue);
			$this->nama_sub_kegiatan->PlaceHolder = ew_RemoveHtml($this->nama_sub_kegiatan->FldCaption());
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// nama_sub_kegiatan
			$this->nama_sub_kegiatan->SetDbValueDef($rsnew, $this->nama_sub_kegiatan->CurrentValue, NULL, $this->nama_sub_kegiatan->ReadOnly);

			// Check referential integrity for master table 'm_kegiatan'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_m_kegiatan();
			$KeyValue = isset($rsnew['kode_program']) ? $rsnew['kode_program'] : $rsold['kode_program'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@kode_program@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
				$bValidMasterRecord = FALSE;
			}
			$KeyValue = isset($rsnew['kode_kegiatan']) ? $rsnew['kode_kegiatan'] : $rsold['kode_kegiatan'];
			if (strval($KeyValue) <> "") {
			...
			if ($bValidMasterRecord) {
				if (!isset($GLOBALS["m_kegiatan"])) $GLOBALS["m_kegiatan"] = new cm_kegiatan();
				$rsmaster = $GLOBALS["m_kegiatan"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "m_kegiatan", $Language->Phrase("RelatedRecordRequired"));
...
// Create page object
if (!isset($m_sub_kegiatan_edit)) $m_sub_kegiatan_edit = new cm_sub_kegiatan_edit();

// Page init
$m_sub_kegiatan_edit->Page_Init();

// Page main
$m_sub_kegiatan_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$m_sub_kegiatan_edit->Page_Render();
?>
...
var CurrentForm = fm_sub_kegiatanedit = new ew_Form("fm_sub_kegiatanedit", "edit");

// Validate form
fm_sub_kegiatanedit.Validate = function() {
...
fm_sub_kegiatanedit.Form_CustomValidate = 
...
fm_sub_kegiatanedit.ValidateRequired = true;
<?php } else { ?>
fm_sub_kegiatanedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$m_sub_kegiatan_edit->IsModal) { ?>
<?php } ?>
<?php $m_sub_kegiatan_edit->ShowPageHeader(); ?>
<?php
$m_sub_kegiatan_edit->ShowMessage();
?>
<form name="fm_sub_kegiatanedit" id="fm_sub_kegiatanedit" class="<?php echo $m_sub_kegiatan_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($m_sub_kegiatan_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $m_sub_kegiatan_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($m_sub_kegiatan->getCurrentMasterTable() == "m_kegiatan") { ?>
...
<?php if ($m_sub_kegiatan->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_m_sub_kegiatan_id" class="col-sm-2 control-label ewLabel"><?php echo $m_sub_kegiatan->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $m_sub_kegiatan->id->CellAttributes() ?>>
<span id="el_m_sub_kegiatan_id">
<span<?php echo $m_sub_kegiatan->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $m_sub_kegiatan->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="m_sub_kegiatan" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($m_sub_kegiatan->id->CurrentValue) ?>">
<?php echo $m_sub_kegiatan->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($m_sub_kegiatan->kode_program->Visible) { // kode_program ?>
	<div id="r_kode_program" class="form-group">
		<label id="elh_m_sub_kegiatan_kode_program" for="x_kode_program" class="col-sm-2 control-label ewLabel"><?php echo $m_sub_kegiatan->kode_program->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $m_sub_kegiatan->kode_program->CellAttributes() ?>>
<?php if ($m_sub_kegiatan->kode_program->getSessionValue() <> "") { ?>
<span id="el_m_sub_kegiatan_kode_program">
...
<input type="hidden" id="x_kode_program" name="x_kode_program" value="<?php echo ew_HtmlEncode($m_sub_kegiatan->kode_program->CurrentValue) ?>">
<?php } else { ?>
<span id="el_m_sub_kegiatan_kode_program">
<input type="text" data-table="m_sub_kegiatan" data-field="x_kode_program" name="x_kode_program" id="x_kode_program" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($m_sub_kegiatan->kode_program->getPlaceHolder()) ?>" value="<?php echo $m_sub_kegiatan->kode_program->EditValue ?>"<?php echo $m_sub_kegiatan->kode_program->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $m_sub_kegiatan->kode_program->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($m_sub_kegiatan->kode_kegiatan->Visible) { // kode_kegiatan ?>
	<div id="r_kode_kegiatan" class="form-group">
		<label id="elh_m_sub_kegiatan_kode_kegiatan" for="x_kode_kegiatan" class="col-sm-2 control-label ewLabel"><?php echo $m_sub_kegiatan->kode_kegiatan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $m_sub_kegiatan->kode_kegiatan->CellAttributes() ?>>
<?php if ($m_sub_kegiatan->kode_kegiatan->getSessionValue() <> "") { ?>
<span id="el_m_sub_kegiatan_kode_kegiatan">
...
<input type="hidden" id="x_kode_kegiatan" name="x_kode_kegiatan" value="<?php echo ew_HtmlEncode($m_sub_kegiatan->kode_kegiatan->CurrentValue) ?>">
<?php } else { ?>
<span id="el_m_sub_kegiatan_kode_kegiatan">
<input type="text" data-table="m_sub_kegiatan" data-field="x_kode_kegiatan" name="x_kode_kegiatan" id="x_kode_kegiatan" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($m_sub_kegiatan->kode_kegiatan->getPlaceHolder()) ?>" value="<?php echo $m_sub_kegiatan->kode_kegiatan->EditValue ?>"<?php echo $m_sub_kegiatan->kode_kegiatan->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $m_sub_kegiatan->kode_kegiatan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($m_sub_kegiatan->kode_sub_kegiatan->Visible) { // kode_sub_kegiatan ?>
	<div id="r_kode_sub_kegiatan" class="form-group">
		<label id="elh_m_sub_kegiatan_kode_sub_kegiatan" for="x_kode_sub_kegiatan" class="col-sm-2 control-label ewLabel"><?php echo $m_sub_kegiatan->kode_sub_kegiatan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $m_sub_kegiatan->kode_sub_kegiatan->CellAttributes() ?>>
<span id="el_m_sub_kegiatan_kode_sub_kegiatan">
<input type="text" data-table="m_sub_kegiatan" data-field="x_kode_sub_kegiatan" name="x_kode_sub_kegiatan" id="x_kode_sub_kegiatan" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($m_sub_kegiatan->kode_sub_kegiatan->getPlaceHolder()) ?>" value="<?php echo $m_sub_kegiatan->kode_sub_kegiatan->EditValue ?>"<?php echo $m_sub_kegiatan->kode_sub_kegiatan->EditAttributes() ?>>
</span>
<?php echo $m_sub_kegiatan->kode_sub_kegiatan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($m_sub_kegiatan->nama_sub_kegiatan->Visible) { // nama_sub_kegiatan ?>
	<div id="r_nama_sub_kegiatan" class="form-group">
		<label id="elh_m_sub_kegiatan_nama_sub_kegiatan" for="x_nama_sub_kegiatan" class="col-sm-2 control-label ewLabel"><?php echo $m_sub_kegiatan->nama_sub_kegiatan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $m_sub_kegiatan->nama_sub_kegiatan->CellAttributes() ?>>
<span id="el_m_sub_kegiatan_nama_sub_kegiatan">
<input type="text" data-table="m_sub_kegiatan" data-field="x_nama_sub_kegiatan" name="x_nama_sub_kegiatan" id="x_nama_sub_kegiatan" size="30" maxlength="250" placeholder="<?php echo ew_HtmlEncode($m_sub_kegiatan->nama_sub_kegiatan->getPlaceHolder()) ?>" value="<?php echo $m_sub_kegiatan->nama_sub_kegiatan->EditValue ?>"<?php echo $m_sub_kegiatan->nama_sub_kegiatan->EditAttributes() ?>>
</span>
<?php echo $m_sub_kegiatan->nama_sub_kegiatan->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$m_sub_kegiatan_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $m_sub_kegiatan_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fm_sub_kegiatanedit.Init();
</script>
<?php
$m_sub_kegiatan_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$m_sub_kegiatan_edit->Page_Terminate();
?>
