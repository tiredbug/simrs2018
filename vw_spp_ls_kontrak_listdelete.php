...
$vw_spp_ls_kontrak_list_delete = NULL; // Initialize page object first

class cvw_spp_ls_kontrak_list_delete extends cvw_spp_ls_kontrak_list {
	...
	// Page object name
	var $PageObjName = 'vw_spp_ls_kontrak_list_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("vw_spp_ls_kontrak_listlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in vw_spp_ls_kontrak_list class, vw_spp_ls_kontrak_listinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("vw_spp_ls_kontrak_listlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($vw_spp_ls_kontrak_list_delete)) $vw_spp_ls_kontrak_list_delete = new cvw_spp_ls_kontrak_list_delete();

// Page init
$vw_spp_ls_kontrak_list_delete->Page_Init();

// Page main
$vw_spp_ls_kontrak_list_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_spp_ls_kontrak_list_delete->Page_Render();
?>
...
var CurrentForm = fvw_spp_ls_kontrak_listdelete = new ew_Form("fvw_spp_ls_kontrak_listdelete", "delete");

// Form_CustomValidate event
fvw_spp_ls_kontrak_listdelete.Form_CustomValidate = 
...
fvw_spp_ls_kontrak_listdelete.ValidateRequired = true;
<?php } else { ?>
fvw_spp_ls_kontrak_listdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_spp_ls_kontrak_listdelete.Lists["x_status_spp"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvw_spp_ls_kontrak_listdelete.Lists["x_status_spp"].Options = <?php echo json_encode($vw_spp_ls_kontrak_list->status_spp->Options()) ?>;
...
<?php $vw_spp_ls_kontrak_list_delete->ShowPageHeader(); ?>
<?php
$vw_spp_ls_kontrak_list_delete->ShowMessage();
?>
<form name="fvw_spp_ls_kontrak_listdelete" id="fvw_spp_ls_kontrak_listdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_spp_ls_kontrak_list_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_spp_ls_kontrak_list_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_spp_ls_kontrak_list">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($vw_spp_ls_kontrak_list_delete->RecKeys as $key) { ?>
...
<?php if ($vw_spp_ls_kontrak_list->id->Visible) { // id ?>
		<th><span id="elh_vw_spp_ls_kontrak_list_id" class="vw_spp_ls_kontrak_list_id"><?php echo $vw_spp_ls_kontrak_list->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($vw_spp_ls_kontrak_list->status_spp->Visible) { // status_spp ?>
		<th><span id="elh_vw_spp_ls_kontrak_list_status_spp" class="vw_spp_ls_kontrak_list_status_spp"><?php echo $vw_spp_ls_kontrak_list->status_spp->FldCaption() ?></span></th>
<?php } ?>
<?php if ($vw_spp_ls_kontrak_list->no_spp->Visible) { // no_spp ?>
		<th><span id="elh_vw_spp_ls_kontrak_list_no_spp" class="vw_spp_ls_kontrak_list_no_spp"><?php echo $vw_spp_ls_kontrak_list->no_spp->FldCaption() ?></span></th>
<?php } ?>
<?php if ($vw_spp_ls_kontrak_list->tgl_spp->Visible) { // tgl_spp ?>
		<th><span id="elh_vw_spp_ls_kontrak_list_tgl_spp" class="vw_spp_ls_kontrak_list_tgl_spp"><?php echo $vw_spp_ls_kontrak_list->tgl_spp->FldCaption() ?></span></th>
<?php } ?>
<?php if ($vw_spp_ls_kontrak_list->keterangan->Visible) { // keterangan ?>
		<th><span id="elh_vw_spp_ls_kontrak_list_keterangan" class="vw_spp_ls_kontrak_list_keterangan"><?php echo $vw_spp_ls_kontrak_list->keterangan->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$vw_spp_ls_kontrak_list_delete->RecCnt = 0;
$i = 0;
while (!$vw_spp_ls_kontrak_list_delete->Recordset->EOF) {
	$vw_spp_ls_kontrak_list_delete->RecCnt++;
	$vw_spp_ls_kontrak_list_delete->RowCnt++;

	// Set row properties
	$vw_spp_ls_kontrak_list->ResetAttrs();
	$vw_spp_ls_kontrak_list->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$vw_spp_ls_kontrak_list_delete->LoadRowValues($vw_spp_ls_kontrak_list_delete->Recordset);

	// Render row
	$vw_spp_ls_kontrak_list_delete->RenderRow();
?>
	<tr<?php echo $vw_spp_ls_kontrak_list->RowAttributes() ?>>
<?php if ($vw_spp_ls_kontrak_list->id->Visible) { // id ?>
		<td<?php echo $vw_spp_ls_kontrak_list->id->CellAttributes() ?>>
<div id="orig<?php echo $vw_spp_ls_kontrak_list_delete->RowCnt ?>_vw_spp_ls_kontrak_list_id" class="hide">
<span id="el<?php echo $vw_spp_ls_kontrak_list_delete->RowCnt ?>_vw_spp_ls_kontrak_list_id" class="vw_spp_ls_kontrak_list_id">
...
<?php } ?>
<?php if ($vw_spp_ls_kontrak_list->status_spp->Visible) { // status_spp ?>
		<td<?php echo $vw_spp_ls_kontrak_list->status_spp->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_ls_kontrak_list_delete->RowCnt ?>_vw_spp_ls_kontrak_list_status_spp" class="vw_spp_ls_kontrak_list_status_spp">
...
<?php } ?>
<?php if ($vw_spp_ls_kontrak_list->no_spp->Visible) { // no_spp ?>
		<td<?php echo $vw_spp_ls_kontrak_list->no_spp->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_ls_kontrak_list_delete->RowCnt ?>_vw_spp_ls_kontrak_list_no_spp" class="vw_spp_ls_kontrak_list_no_spp">
...
<?php } ?>
<?php if ($vw_spp_ls_kontrak_list->tgl_spp->Visible) { // tgl_spp ?>
		<td<?php echo $vw_spp_ls_kontrak_list->tgl_spp->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_ls_kontrak_list_delete->RowCnt ?>_vw_spp_ls_kontrak_list_tgl_spp" class="vw_spp_ls_kontrak_list_tgl_spp">
...
<?php } ?>
<?php if ($vw_spp_ls_kontrak_list->keterangan->Visible) { // keterangan ?>
		<td<?php echo $vw_spp_ls_kontrak_list->keterangan->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_ls_kontrak_list_delete->RowCnt ?>_vw_spp_ls_kontrak_list_keterangan" class="vw_spp_ls_kontrak_list_keterangan">
...
<?php
	$vw_spp_ls_kontrak_list_delete->Recordset->MoveNext();
}
$vw_spp_ls_kontrak_list_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_spp_ls_kontrak_list_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fvw_spp_ls_kontrak_listdelete.Init();
</script>
<?php
$vw_spp_ls_kontrak_list_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_spp_ls_kontrak_list_delete->Page_Terminate();
?>
