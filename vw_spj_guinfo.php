...
//
// Table class for vw_spj_gu
//
class cvw_spj_gu extends cTable {
	var $id;
	var $no_spj;
	var $tgl_spj;
	var $tipe;
	...
	function __construct() {
		...
		$this->TableVar = 'vw_spj_gu';
		$this->TableName = 'vw_spj_gu';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vw_spj_gu`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('vw_spj_gu', 'vw_spj_gu', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// no_spj
		$this->no_spj = new cField('vw_spj_gu', 'vw_spj_gu', 'x_no_spj', 'no_spj', '`no_spj`', '`no_spj`', 200, -1, FALSE, '`no_spj`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->no_spj->Sortable = TRUE; // Allow sort
		$this->fields['no_spj'] = &$this->no_spj;

		// tgl_spj
		$this->tgl_spj = new cField('vw_spj_gu', 'vw_spj_gu', 'x_tgl_spj', 'tgl_spj', '`tgl_spj`', ew_CastDateFieldForLike('`tgl_spj`', 7, "DB"), 135, 7, FALSE, '`tgl_spj`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tipe
		$this->tipe = new cField('vw_spj_gu', 'vw_spj_gu', 'x_tipe', 'tipe', '`tipe`', '`tipe`', 3, -1, FALSE, '`tipe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tipe_sbp
		$this->tipe_sbp = new cField('vw_spj_gu', 'vw_spj_gu', 'x_tipe_sbp', 'tipe_sbp', '`tipe_sbp`', '`tipe_sbp`', 3, -1, FALSE, '`tipe_sbp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// no_sbp
		$this->no_sbp = new cField('vw_spj_gu', 'vw_spj_gu', 'x_no_sbp', 'no_sbp', '`no_sbp`', '`no_sbp`', 200, -1, FALSE, '`no_sbp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->no_sbp->Sortable = TRUE; // Allow sort
		$this->fields['no_sbp'] = &$this->no_sbp;

		// tgl_sbp
		$this->tgl_sbp = new cField('vw_spj_gu', 'vw_spj_gu', 'x_tgl_sbp', 'tgl_sbp', '`tgl_sbp`', ew_CastDateFieldForLike('`tgl_sbp`', 0, "DB"), 135, 0, FALSE, '`tgl_sbp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tgl_sbp->Sortable = TRUE; // Allow sort
		$this->tgl_sbp->FldDefaultErrMsg = str_replace("%s", $GLOBALS["EW_DATE_FORMAT"], $Language->Phrase("IncorrectDate"));
		$this->fields['tgl_sbp'] = &$this->tgl_sbp;

		// uraian
		$this->uraian = new cField('vw_spj_gu', 'vw_spj_gu', 'x_uraian', 'uraian', '`uraian`', '`uraian`', 201, -1, FALSE, '`uraian`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->uraian->Sortable = TRUE; // Allow sort
		$this->fields['uraian'] = &$this->uraian;

		// nama_penerima
		$this->nama_penerima = new cField('vw_spj_gu', 'vw_spj_gu', 'x_nama_penerima', 'nama_penerima', '`nama_penerima`', '`nama_penerima`', 201, -1, FALSE, '`nama_penerima`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`vw_spj_gu`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("vw_spj_guview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("vw_spj_guview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "vw_spj_guadd.php?" . $this->UrlParm($parm);
		else
			$url = "vw_spj_guadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("vw_spj_guedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("vw_spj_guadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("vw_spj_gudelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		if (strval($this->tipe->CurrentValue) <> "") {
			$this->tipe->EditValue = $this->tipe->OptionCaption($this->tipe->CurrentValue);
		} else {
			$this->tipe->EditValue = NULL;
		}
		$this->tipe->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->tgl_sbp->Exportable) $Doc->ExportCaption($this->tgl_sbp);
					if ($this->uraian->Exportable) $Doc->ExportCaption($this->uraian);
					if ($this->nama_penerima->Exportable) $Doc->ExportCaption($this->nama_penerima);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->no_spj->Exportable) $Doc->ExportCaption($this->no_spj);
					if ($this->tgl_spj->Exportable) $Doc->ExportCaption($this->tgl_spj);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->tgl_sbp->Exportable) $Doc->ExportField($this->tgl_sbp);
						if ($this->uraian->Exportable) $Doc->ExportField($this->uraian);
						if ($this->nama_penerima->Exportable) $Doc->ExportField($this->nama_penerima);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->no_spj->Exportable) $Doc->ExportField($this->no_spj);
						if ($this->tgl_spj->Exportable) $Doc->ExportField($this->tgl_spj);
