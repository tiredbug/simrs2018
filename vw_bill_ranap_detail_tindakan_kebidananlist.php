...
<?php include_once "vw_bill_ranap_detail_tindakan_kebidananinfo.php" ?>
...
$vw_bill_ranap_detail_tindakan_kebidanan_list = NULL; // Initialize page object first

class cvw_bill_ranap_detail_tindakan_kebidanan_list extends cvw_bill_ranap_detail_tindakan_kebidanan {
	...
	// Table name
	var $TableName = 'vw_bill_ranap_detail_tindakan_kebidanan';

	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_tindakan_kebidanan_list';

	// Grid form hidden field names
	var $FormName = 'fvw_bill_ranap_detail_tindakan_kebidananlist';
	...
	function __construct() {
		...
		// Table object (vw_bill_ranap_detail_tindakan_kebidanan)
		if (!isset($GLOBALS["vw_bill_ranap_detail_tindakan_kebidanan"]) || get_class($GLOBALS["vw_bill_ranap_detail_tindakan_kebidanan"]) == "cvw_bill_ranap_detail_tindakan_kebidanan") {
			$GLOBALS["vw_bill_ranap_detail_tindakan_kebidanan"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["vw_bill_ranap_detail_tindakan_kebidanan"];
		}
		...
		$this->AddUrl = "vw_bill_ranap_detail_tindakan_kebidananadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "vw_bill_ranap_detail_tindakan_kebidanandelete.php";
		$this->MultiUpdateUrl = "vw_bill_ranap_detail_tindakan_kebidananupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'vw_bill_ranap_detail_tindakan_kebidanan', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fvw_bill_ranap_detail_tindakan_kebidananlistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $vw_bill_ranap_detail_tindakan_kebidanan;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($vw_bill_ranap_detail_tindakan_kebidanan);
				$doc->Text = $sContent;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fvw_bill_ranap_detail_tindakan_kebidananlistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fvw_bill_ranap_detail_tindakan_kebidananlistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fvw_bill_ranap_detail_tindakan_kebidananlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fvw_bill_ranap_detail_tindakan_kebidananlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "gridedit") { // Not grid add/edit mode
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fvw_bill_ranap_detail_tindakan_kebidananlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fvw_bill_ranap_detail_tindakan_kebidananlistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		$lookuptblfilter = "`kelompok_tindakan`='3'";
	...
	function ValidateForm() {
		...
		if (!$this->bhp->FldIsDetailKey && !is_null($this->bhp->FormValue) && $this->bhp->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->bhp->FldCaption(), $this->bhp->ReqErrMsg));
		}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->statusbayar->getSessionValue() <> "") {
			$rsnew['statusbayar'] = $this->statusbayar->getSessionValue();
		}
		...
		if ($this->no_ruang->getSessionValue() <> "") {
			$rsnew['no_ruang'] = $this->no_ruang->getSessionValue();
		}
...
// Create page object
if (!isset($vw_bill_ranap_detail_tindakan_kebidanan_list)) $vw_bill_ranap_detail_tindakan_kebidanan_list = new cvw_bill_ranap_detail_tindakan_kebidanan_list();

// Page init
$vw_bill_ranap_detail_tindakan_kebidanan_list->Page_Init();

// Page main
$vw_bill_ranap_detail_tindakan_kebidanan_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_bill_ranap_detail_tindakan_kebidanan_list->Page_Render();
?>
...
var CurrentForm = fvw_bill_ranap_detail_tindakan_kebidananlist = new ew_Form("fvw_bill_ranap_detail_tindakan_kebidananlist", "list");
fvw_bill_ranap_detail_tindakan_kebidananlist.FormKeyCountName = '<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->FormKeyCountName ?>';

// Validate form
fvw_bill_ranap_detail_tindakan_kebidananlist.Validate = function() {
...
// Check empty row
fvw_bill_ranap_detail_tindakan_kebidananlist.EmptyRow = function(infix) {
	var fobj = this.Form;
...
fvw_bill_ranap_detail_tindakan_kebidananlist.Form_CustomValidate = 
...
fvw_bill_ranap_detail_tindakan_kebidananlist.ValidateRequired = true;
<?php } else { ?>
fvw_bill_ranap_detail_tindakan_kebidananlist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_bill_ranap_detail_tindakan_kebidananlist.Lists["x_kode_tindakan"] = {"LinkField":"x_kode","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama_tindakan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_bill_ranap_data_tarif_tindakan"};

// Form object for search
var CurrentSearchForm = fvw_bill_ranap_detail_tindakan_kebidananlistsrch = new ew_Form("fvw_bill_ranap_detail_tindakan_kebidananlistsrch");
</script>
<script type="text/javascript">

// Write your client script here, no need to add script tags.
</script>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs > 0 && $vw_bill_ranap_detail_tindakan_kebidanan_list->ExportOptions->Visible()) { ?>
<?php $vw_bill_ranap_detail_tindakan_kebidanan_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->SearchOptions->Visible()) { ?>
<?php $vw_bill_ranap_detail_tindakan_kebidanan_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->FilterOptions->Visible()) { ?>
<?php $vw_bill_ranap_detail_tindakan_kebidanan_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php if (($vw_bill_ranap_detail_tindakan_kebidanan->Export == "") || (EW_EXPORT_MASTER_RECORD && $vw_bill_ranap_detail_tindakan_kebidanan->Export == "print")) { ?>
<?php
if ($vw_bill_ranap_detail_tindakan_kebidanan_list->DbMasterFilter <> "" && $vw_bill_ranap_detail_tindakan_kebidanan->getCurrentMasterTable() == "vw_bill_ranap") {
	if ($vw_bill_ranap_detail_tindakan_kebidanan_list->MasterRecordExists) {
?>
...
if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd") {
	...
	$vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec = 1;
	$vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs = $vw_bill_ranap_detail_tindakan_kebidanan->GridAddRowCount;
	$vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs = $vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs;
	$vw_bill_ranap_detail_tindakan_kebidanan_list->StopRec = $vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs;
} else {
	$bSelectLimit = $vw_bill_ranap_detail_tindakan_kebidanan_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs <= 0)
			$vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs = $vw_bill_ranap_detail_tindakan_kebidanan->SelectRecordCount();
	} else {
		if (!$vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset && ($vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset = $vw_bill_ranap_detail_tindakan_kebidanan_list->LoadRecordset()))
			$vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs = $vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset->RecordCount();
	}
	$vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec = 1;
	if ($vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs <= 0 || ($vw_bill_ranap_detail_tindakan_kebidanan->Export <> "" && $vw_bill_ranap_detail_tindakan_kebidanan->ExportAll)) // Display all records
		$vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs = $vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs;
	if (!($vw_bill_ranap_detail_tindakan_kebidanan->Export <> "" && $vw_bill_ranap_detail_tindakan_kebidanan->ExportAll))
		$vw_bill_ranap_detail_tindakan_kebidanan_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset = $vw_bill_ranap_detail_tindakan_kebidanan_list->LoadRecordset($vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec-1, $vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs);

	// Set no record found message
	if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "" && $vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$vw_bill_ranap_detail_tindakan_kebidanan_list->setWarningMessage(ew_DeniedMsg());
		if ($vw_bill_ranap_detail_tindakan_kebidanan_list->SearchWhere == "0=101")
			$vw_bill_ranap_detail_tindakan_kebidanan_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$vw_bill_ranap_detail_tindakan_kebidanan_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($vw_bill_ranap_detail_tindakan_kebidanan_list->AuditTrailOnSearch && $vw_bill_ranap_detail_tindakan_kebidanan_list->Command == "search" && !$vw_bill_ranap_detail_tindakan_kebidanan_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $vw_bill_ranap_detail_tindakan_kebidanan_list->getSessionWhere();
		$vw_bill_ranap_detail_tindakan_kebidanan_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
}
$vw_bill_ranap_detail_tindakan_kebidanan_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->Export == "" && $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "") { ?>
<form name="fvw_bill_ranap_detail_tindakan_kebidananlistsrch" id="fvw_bill_ranap_detail_tindakan_kebidananlistsrch" class="form-horizontal ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($vw_bill_ranap_detail_tindakan_kebidanan_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fvw_bill_ranap_detail_tindakan_kebidananlistsrch_SearchPanel" class="box box-default collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="vw_bill_ranap_detail_tindakan_kebidanan">
<div class="box-body">
<div id="xsr_1">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $vw_bill_ranap_detail_tindakan_kebidanan_list->ShowPageHeader(); ?>
<?php
$vw_bill_ranap_detail_tindakan_kebidanan_list->ShowMessage();
?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs > 0 || $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid vw_bill_ranap_detail_tindakan_kebidanan">
<form name="fvw_bill_ranap_detail_tindakan_kebidananlist" id="fvw_bill_ranap_detail_tindakan_kebidananlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_bill_ranap_detail_tindakan_kebidanan">
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->getCurrentMasterTable() == "vw_bill_ranap" && $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction <> "") { ?>
<input type="hidden" name="<?php echo EW_TABLE_SHOW_MASTER ?>" value="vw_bill_ranap">
<input type="hidden" name="fk_id_admission" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->id_admission->getSessionValue() ?>">
<input type="hidden" name="fk_nomr" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->nomr->getSessionValue() ?>">
<input type="hidden" name="fk_statusbayar" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->statusbayar->getSessionValue() ?>">
<input type="hidden" name="fk_KELASPERAWATAN_ID" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->kelas->getSessionValue() ?>">
<input type="hidden" name="fk_noruang" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->no_ruang->getSessionValue() ?>">
<?php } ?>
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs > 0 || $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridedit") { ?>
<table id="tbl_vw_bill_ranap_detail_tindakan_kebidananlist" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$vw_bill_ranap_detail_tindakan_kebidanan_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$vw_bill_ranap_detail_tindakan_kebidanan_list->RenderListOptions();

// Render list options (header, left)
$vw_bill_ranap_detail_tindakan_kebidanan_list->ListOptions->Render("header", "left");
?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->tanggal->Visible) { // tanggal ?>
	...
	<?php } else { ?>
		<th data-name="tanggal"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->SortUrl($vw_bill_ranap_detail_tindakan_kebidanan->tanggal) ?>',2);"><div id="elh_vw_bill_ranap_detail_tindakan_kebidanan_tanggal" class="vw_bill_ranap_detail_tindakan_kebidanan_tanggal">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan->Visible) { // kode_tindakan ?>
	...
	<?php } else { ?>
		<th data-name="kode_tindakan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->SortUrl($vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan) ?>',2);"><div id="elh_vw_bill_ranap_detail_tindakan_kebidanan_kode_tindakan" class="vw_bill_ranap_detail_tindakan_kebidanan_kode_tindakan">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->qty->Visible) { // qty ?>
	...
	<?php } else { ?>
		<th data-name="qty"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->SortUrl($vw_bill_ranap_detail_tindakan_kebidanan->qty) ?>',2);"><div id="elh_vw_bill_ranap_detail_tindakan_kebidanan_qty" class="vw_bill_ranap_detail_tindakan_kebidanan_qty">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->tarif->Visible) { // tarif ?>
	...
	<?php } else { ?>
		<th data-name="tarif"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->SortUrl($vw_bill_ranap_detail_tindakan_kebidanan->tarif) ?>',2);"><div id="elh_vw_bill_ranap_detail_tindakan_kebidanan_tarif" class="vw_bill_ranap_detail_tindakan_kebidanan_tarif">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->bhp->Visible) { // bhp ?>
	...
	<?php } else { ?>
		<th data-name="bhp"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->SortUrl($vw_bill_ranap_detail_tindakan_kebidanan->bhp) ?>',2);"><div id="elh_vw_bill_ranap_detail_tindakan_kebidanan_bhp" class="vw_bill_ranap_detail_tindakan_kebidanan_bhp">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->user->Visible) { // user ?>
	...
	<?php } else { ?>
		<th data-name="user"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->SortUrl($vw_bill_ranap_detail_tindakan_kebidanan->user) ?>',2);"><div id="elh_vw_bill_ranap_detail_tindakan_kebidanan_user" class="vw_bill_ranap_detail_tindakan_kebidanan_user">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_bill_ranap_detail_tindakan_kebidanan->user->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($vw_bill_ranap_detail_tindakan_kebidanan->user->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_bill_ranap_detail_tindakan_kebidanan->user->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
$vw_bill_ranap_detail_tindakan_kebidanan_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($vw_bill_ranap_detail_tindakan_kebidanan->ExportAll && $vw_bill_ranap_detail_tindakan_kebidanan->Export <> "") {
	$vw_bill_ranap_detail_tindakan_kebidanan_list->StopRec = $vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs;
} else {

	// Set the last record to display
	if ($vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs > $vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec + $vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs - 1)
		$vw_bill_ranap_detail_tindakan_kebidanan_list->StopRec = $vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec + $vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs - 1;
	else
		$vw_bill_ranap_detail_tindakan_kebidanan_list->StopRec = $vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs;
}
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($vw_bill_ranap_detail_tindakan_kebidanan_list->FormKeyCountName) && ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd" || $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridedit" || $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "F")) {
		$vw_bill_ranap_detail_tindakan_kebidanan_list->KeyCount = $objForm->GetValue($vw_bill_ranap_detail_tindakan_kebidanan_list->FormKeyCountName);
		$vw_bill_ranap_detail_tindakan_kebidanan_list->StopRec = $vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec + $vw_bill_ranap_detail_tindakan_kebidanan_list->KeyCount - 1;
	}
}
$vw_bill_ranap_detail_tindakan_kebidanan_list->RecCnt = $vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec - 1;
if ($vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset && !$vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset->EOF) {
	$vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset->MoveFirst();
	$bSelectLimit = $vw_bill_ranap_detail_tindakan_kebidanan_list->UseSelectLimit;
	if (!$bSelectLimit && $vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec > 1)
		$vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset->Move($vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec - 1);
} elseif (!$vw_bill_ranap_detail_tindakan_kebidanan->AllowAddDeleteRow && $vw_bill_ranap_detail_tindakan_kebidanan_list->StopRec == 0) {
	$vw_bill_ranap_detail_tindakan_kebidanan_list->StopRec = $vw_bill_ranap_detail_tindakan_kebidanan->GridAddRowCount;
}
...
$vw_bill_ranap_detail_tindakan_kebidanan_list->RenderRow();
if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd")
	$vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex = 0;
if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridedit")
	$vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex = 0;
while ($vw_bill_ranap_detail_tindakan_kebidanan_list->RecCnt < $vw_bill_ranap_detail_tindakan_kebidanan_list->StopRec) {
	$vw_bill_ranap_detail_tindakan_kebidanan_list->RecCnt++;
	if (intval($vw_bill_ranap_detail_tindakan_kebidanan_list->RecCnt) >= intval($vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec)) {
		$vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt++;
		if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd" || $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridedit" || $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "F") {
			$vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex++;
			$objForm->Index = $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex;
			if ($objForm->HasValue($vw_bill_ranap_detail_tindakan_kebidanan_list->FormActionName))
				$vw_bill_ranap_detail_tindakan_kebidanan_list->RowAction = strval($objForm->GetValue($vw_bill_ranap_detail_tindakan_kebidanan_list->FormActionName));
			elseif ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd")
				$vw_bill_ranap_detail_tindakan_kebidanan_list->RowAction = "insert";
			else
				$vw_bill_ranap_detail_tindakan_kebidanan_list->RowAction = "";
		}

		// Set up key count
		$vw_bill_ranap_detail_tindakan_kebidanan_list->KeyCount = $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex;

		// Init row class and style
		$vw_bill_ranap_detail_tindakan_kebidanan->ResetAttrs();
		$vw_bill_ranap_detail_tindakan_kebidanan->CssClass = "";
		if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd") {
			$vw_bill_ranap_detail_tindakan_kebidanan_list->LoadDefaultValues(); // Load default values
		} else {
			$vw_bill_ranap_detail_tindakan_kebidanan_list->LoadRowValues($vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset); // Load row values
		}
		...
		if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd" && $vw_bill_ranap_detail_tindakan_kebidanan->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$vw_bill_ranap_detail_tindakan_kebidanan_list->RestoreCurrentRowFormValues($vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex); // Restore form values
		if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridedit") { // Grid edit
			...
			if ($vw_bill_ranap_detail_tindakan_kebidanan_list->RowAction == "insert")
				$vw_bill_ranap_detail_tindakan_kebidanan->RowType = EW_ROWTYPE_ADD; // Render add
			else
		...
		if ($vw_bill_ranap_detail_tindakan_kebidanan_list->RowAction <> "delete" && $vw_bill_ranap_detail_tindakan_kebidanan_list->RowAction <> "insertdelete" && !($vw_bill_ranap_detail_tindakan_kebidanan_list->RowAction == "insert" && $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "F" && $vw_bill_ranap_detail_tindakan_kebidanan_list->EmptyRow())) {
?>
	<tr<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->RowAttributes() ?>>
<?php

// Render list options (body, left)
$vw_bill_ranap_detail_tindakan_kebidanan_list->ListOptions->Render("body", "left", $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt);
?>
	<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->tanggal->Visible) { // tanggal ?>
...
<script type="text/javascript">
ew_CreateCalendar("fvw_bill_ranap_detail_tindakan_kebidananlist", "x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tanggal", 7);
</script>
<?php } ?>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_tanggal" name="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tanggal" id="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tanggal" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->tanggal->OldValue) ?>">
<?php } ?>
...
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_tanggal" class="vw_bill_ranap_detail_tindakan_kebidanan_tanggal">
...
<?php } ?>
<a id="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->PageObjName . "_row_" . $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_id" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_id" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_id" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->id->CurrentValue) ?>">
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_id" name="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_id" id="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_id" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->id->OldValue) ?>">
<?php } ?>
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_kode_tindakan" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_kode_tindakan">
...
<select data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_kode_tindakan" data-value-separator="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan->EditAttributes() ?>>
<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan->SelectOptionListHtml("x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan") ?>
</select>
<input type="hidden" name="s_x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan" id="s_x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan->LookupFilterQuery() ?>">
<input type="hidden" name="ln_x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan" id="ln_x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan" value="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tarif,x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_bhp">
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_kode_tindakan" name="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan" id="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_kode_tindakan" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan->OldValue) ?>">
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_kode_tindakan" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_kode_tindakan">
<?php $vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan->EditAttrs["onchange"] = "ew_AutoFill(this); " . @$vw_bill_ranap_detail_tindakan_kebidanan->kode_tindakan->EditAttrs["onchange"]; ?>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_kode_tindakan" class="vw_bill_ranap_detail_tindakan_kebidanan_kode_tindakan">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_qty" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_qty">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_qty" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_qty" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_qty" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->qty->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->qty->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->qty->EditAttributes() ?>>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_qty" name="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_qty" id="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_qty" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->qty->OldValue) ?>">
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_qty" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_qty">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_qty" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_qty" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_qty" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->qty->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->qty->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->qty->EditAttributes() ?>>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_qty" class="vw_bill_ranap_detail_tindakan_kebidanan_qty">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_tarif" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_tarif">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_tarif" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tarif" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tarif" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->tarif->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->tarif->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->tarif->EditAttributes() ?>>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_tarif" name="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tarif" id="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tarif" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->tarif->OldValue) ?>">
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_tarif" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_tarif">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_tarif" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tarif" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_tarif" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->tarif->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->tarif->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->tarif->EditAttributes() ?>>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_tarif" class="vw_bill_ranap_detail_tindakan_kebidanan_tarif">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_bhp" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_bhp">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_bhp" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_bhp" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_bhp" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->bhp->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->bhp->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->bhp->EditAttributes() ?>>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_bhp" name="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_bhp" id="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_bhp" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->bhp->OldValue) ?>">
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_bhp" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_bhp">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_bhp" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_bhp" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_bhp" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->bhp->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->bhp->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->bhp->EditAttributes() ?>>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_bhp" class="vw_bill_ranap_detail_tindakan_kebidanan_bhp">
...
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_user" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_user">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_user" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_user" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_user" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->user->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->user->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->user->EditAttributes() ?>>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_user" name="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_user" id="o<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_user" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->user->OldValue) ?>">
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_user" class="form-group vw_bill_ranap_detail_tindakan_kebidanan_user">
<input type="text" data-table="vw_bill_ranap_detail_tindakan_kebidanan" data-field="x_user" name="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_user" id="x<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>_user" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tindakan_kebidanan->user->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->user->EditValue ?>"<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->user->EditAttributes() ?>>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt ?>_vw_bill_ranap_detail_tindakan_kebidanan_user" class="vw_bill_ranap_detail_tindakan_kebidanan_user">
...
$vw_bill_ranap_detail_tindakan_kebidanan_list->ListOptions->Render("body", "right", $vw_bill_ranap_detail_tindakan_kebidanan_list->RowCnt);
?>
	</tr>
...
<script type="text/javascript">
fvw_bill_ranap_detail_tindakan_kebidananlist.UpdateOpts(<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex ?>);
</script>
<?php } ?>
<?php
	}
	...
	if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction <> "gridadd")
		if (!$vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset->EOF) $vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset->MoveNext();
}
?>
<?php
	if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd" || $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridedit") {
		$vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex = '$rowindex$';
		$vw_bill_ranap_detail_tindakan_kebidanan_list->LoadDefaultValues();

		// Set row properties
		$vw_bill_ranap_detail_tindakan_kebidanan->ResetAttrs();
		$vw_bill_ranap_detail_tindakan_kebidanan->RowAttrs = array_merge($vw_bill_ranap_detail_tindakan_kebidanan->RowAttrs, array('data-rowindex'=>$vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex, 'id'=>'r0_vw_bill_ranap_detail_tindakan_kebidanan', 'data-rowtype'=>EW_ROWTYPE_ADD));
		...
		// Render list options
		$vw_bill_ranap_detail_tindakan_kebidanan_list->RenderListOptions();
		$vw_bill_ranap_detail_tindakan_kebidanan_list->StartRowCnt = 0;
?>
	<tr<?php echo $vw_bill_ranap_detail_tindakan_kebidanan->RowAttributes() ?>>
<?php

// Render list options (body, left)
$vw_bill_ranap_detail_tindakan_kebidanan_list->ListOptions->Render("body", "left", $vw_bill_ranap_detail_tindakan_kebidanan_list->RowIndex);
?>
	<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->tanggal->Visible) { // tanggal ?>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->FormKeyCountName ?>" id="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->FormKeyCountName ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->KeyCount ?>">
<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->MultiSelectKey ?>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "gridedit") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridupdate">
<input type="hidden" name="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->FormKeyCountName ?>" id="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->FormKeyCountName ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->KeyCount ?>">
<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->MultiSelectKey ?>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "") { ?>
...
// Close recordset
if ($vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset)
	$vw_bill_ranap_detail_tindakan_kebidanan_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction <> "gridadd" && $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($vw_bill_ranap_detail_tindakan_kebidanan_list->Pager)) $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager = new cPrevNextPager($vw_bill_ranap_detail_tindakan_kebidanan_list->StartRec, $vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs, $vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs) ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->RecordCount > 0 && $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->PageUrl() ?>start=<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->PageUrl() ?>start=<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->PageUrl() ?>start=<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->PageUrl() ?>start=<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $vw_bill_ranap_detail_tindakan_kebidanan_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="vw_bill_ranap_detail_tindakan_kebidanan">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($vw_bill_ranap_detail_tindakan_kebidanan_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tindakan_kebidanan_list->TotalRecs == 0 && $vw_bill_ranap_detail_tindakan_kebidanan->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($vw_bill_ranap_detail_tindakan_kebidanan_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fvw_bill_ranap_detail_tindakan_kebidananlistsrch.FilterList = <?php echo $vw_bill_ranap_detail_tindakan_kebidanan_list->GetFilterList() ?>;
fvw_bill_ranap_detail_tindakan_kebidananlistsrch.Init();
fvw_bill_ranap_detail_tindakan_kebidananlist.Init();
</script>
<?php
$vw_bill_ranap_detail_tindakan_kebidanan_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_bill_ranap_detail_tindakan_kebidanan_list->Page_Terminate();
?>
