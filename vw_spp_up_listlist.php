...
$vw_spp_up_list_list = NULL; // Initialize page object first

class cvw_spp_up_list_list extends cvw_spp_up_list {
	...
	// Page object name
	var $PageObjName = 'vw_spp_up_list_list';

	// Grid form hidden field names
	var $FormName = 'fvw_spp_up_listlist';
	...
	function __construct() {
		...
		$this->AddUrl = "vw_spp_up_listadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "vw_spp_up_listdelete.php";
		$this->MultiUpdateUrl = "vw_spp_up_listupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fvw_spp_up_listlistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fvw_spp_up_listlistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fvw_spp_up_listlistsrch", $filters);
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList(CurrentProjectID() . 't_spp_detail')) {
			...
			$body = "<a class=\"btn btn-default ewRowLink ewDetail btn-xs line-2139\" data-action=\"list\" href=\"" . ew_HtmlEncode("t_spp_detaillist.php?" . EW_TABLE_SHOW_MASTER . "=vw_spp_up_list&fk_id=" . urlencode(strval($this->id->CurrentValue)) . "&fk_id_jenis_spp=" . urlencode(strval($this->id_jenis_spp->CurrentValue)) . "&fk_detail_jenis_spp=" . urlencode(strval($this->detail_jenis_spp->CurrentValue)) . "&fk_no_spp=" . urlencode(strval($this->no_spp->CurrentValue)) . "&fk_kode_program=" . urlencode(strval($this->kode_program->CurrentValue)) . "&fk_kode_kegiatan=" . urlencode(strval($this->kode_kegiatan->CurrentValue)) . "&fk_tahun_anggaran=" . urlencode(strval($this->tahun_anggaran->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($links <> "") {
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fvw_spp_up_listlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fvw_spp_up_listlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fvw_spp_up_listlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fvw_spp_up_listlistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($vw_spp_up_list_list)) $vw_spp_up_list_list = new cvw_spp_up_list_list();

// Page init
$vw_spp_up_list_list->Page_Init();

// Page main
$vw_spp_up_list_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_spp_up_list_list->Page_Render();
?>
...
var CurrentForm = fvw_spp_up_listlist = new ew_Form("fvw_spp_up_listlist", "list");
fvw_spp_up_listlist.FormKeyCountName = '<?php echo $vw_spp_up_list_list->FormKeyCountName ?>';

// Form_CustomValidate event
fvw_spp_up_listlist.Form_CustomValidate = 
...
fvw_spp_up_listlist.ValidateRequired = true;
<?php } else { ?>
fvw_spp_up_listlist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_spp_up_listlist.Lists["x_status_spp"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvw_spp_up_listlist.Lists["x_status_spp"].Options = <?php echo json_encode($vw_spp_up_list->status_spp->Options()) ?>;

// Form object for search
var CurrentSearchForm = fvw_spp_up_listlistsrch = new ew_Form("fvw_spp_up_listlistsrch");
</script>
<script type="text/javascript">

// Write your client script here, no need to add script tags.
</script>
<?php if ($vw_spp_up_list_list->TotalRecs > 0 && $vw_spp_up_list_list->ExportOptions->Visible()) { ?>
<?php $vw_spp_up_list_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_spp_up_list_list->SearchOptions->Visible()) { ?>
<?php $vw_spp_up_list_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_spp_up_list_list->FilterOptions->Visible()) { ?>
<?php $vw_spp_up_list_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php
	$bSelectLimit = $vw_spp_up_list_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($vw_spp_up_list_list->TotalRecs <= 0)
			$vw_spp_up_list_list->TotalRecs = $vw_spp_up_list->SelectRecordCount();
	} else {
		if (!$vw_spp_up_list_list->Recordset && ($vw_spp_up_list_list->Recordset = $vw_spp_up_list_list->LoadRecordset()))
			$vw_spp_up_list_list->TotalRecs = $vw_spp_up_list_list->Recordset->RecordCount();
	}
	$vw_spp_up_list_list->StartRec = 1;
	if ($vw_spp_up_list_list->DisplayRecs <= 0 || ($vw_spp_up_list->Export <> "" && $vw_spp_up_list->ExportAll)) // Display all records
		$vw_spp_up_list_list->DisplayRecs = $vw_spp_up_list_list->TotalRecs;
	if (!($vw_spp_up_list->Export <> "" && $vw_spp_up_list->ExportAll))
		$vw_spp_up_list_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$vw_spp_up_list_list->Recordset = $vw_spp_up_list_list->LoadRecordset($vw_spp_up_list_list->StartRec-1, $vw_spp_up_list_list->DisplayRecs);

	// Set no record found message
	if ($vw_spp_up_list->CurrentAction == "" && $vw_spp_up_list_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$vw_spp_up_list_list->setWarningMessage(ew_DeniedMsg());
		if ($vw_spp_up_list_list->SearchWhere == "0=101")
			$vw_spp_up_list_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$vw_spp_up_list_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$vw_spp_up_list_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($vw_spp_up_list->Export == "" && $vw_spp_up_list->CurrentAction == "") { ?>
<form name="fvw_spp_up_listlistsrch" id="fvw_spp_up_listlistsrch" class="form-horizontal ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($vw_spp_up_list_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fvw_spp_up_listlistsrch_SearchPanel" class="box box-default collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="vw_spp_up_list">
<div class="box-body">
<div id="xsr_1">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($vw_spp_up_list_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($vw_spp_up_list_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $vw_spp_up_list_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($vw_spp_up_list_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($vw_spp_up_list_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($vw_spp_up_list_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($vw_spp_up_list_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $vw_spp_up_list_list->ShowPageHeader(); ?>
<?php
$vw_spp_up_list_list->ShowMessage();
?>
<?php if ($vw_spp_up_list_list->TotalRecs > 0 || $vw_spp_up_list->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid vw_spp_up_list">
<form name="fvw_spp_up_listlist" id="fvw_spp_up_listlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_spp_up_list_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_spp_up_list_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_spp_up_list">
<div id="gmp_vw_spp_up_list" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>box box-primary">
<?php if ($vw_spp_up_list_list->TotalRecs > 0 || $vw_spp_up_list->CurrentAction == "gridedit") { ?>
<table id="tbl_vw_spp_up_listlist" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$vw_spp_up_list_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$vw_spp_up_list_list->RenderListOptions();

// Render list options (header, left)
$vw_spp_up_list_list->ListOptions->Render("header", "left");
?>
<?php if ($vw_spp_up_list->id->Visible) { // id ?>
	<?php if ($vw_spp_up_list->SortUrl($vw_spp_up_list->id) == "") { ?>
		<th data-name="id"><div id="elh_vw_spp_up_list_id" class="vw_spp_up_list_id"><div class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_up_list->SortUrl($vw_spp_up_list->id) ?>',2);"><div id="elh_vw_spp_up_list_id" class="vw_spp_up_list_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_spp_up_list->id->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_spp_up_list->id->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_spp_up_list->status_spp->Visible) { // status_spp ?>
	<?php if ($vw_spp_up_list->SortUrl($vw_spp_up_list->status_spp) == "") { ?>
		<th data-name="status_spp"><div id="elh_vw_spp_up_list_status_spp" class="vw_spp_up_list_status_spp"><div class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->status_spp->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status_spp"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_up_list->SortUrl($vw_spp_up_list->status_spp) ?>',2);"><div id="elh_vw_spp_up_list_status_spp" class="vw_spp_up_list_status_spp">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->status_spp->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_spp_up_list->status_spp->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_spp_up_list->status_spp->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_spp_up_list->no_spp->Visible) { // no_spp ?>
	<?php if ($vw_spp_up_list->SortUrl($vw_spp_up_list->no_spp) == "") { ?>
		<th data-name="no_spp"><div id="elh_vw_spp_up_list_no_spp" class="vw_spp_up_list_no_spp"><div class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->no_spp->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="no_spp"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_up_list->SortUrl($vw_spp_up_list->no_spp) ?>',2);"><div id="elh_vw_spp_up_list_no_spp" class="vw_spp_up_list_no_spp">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->no_spp->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($vw_spp_up_list->no_spp->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_spp_up_list->no_spp->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_spp_up_list->tgl_spp->Visible) { // tgl_spp ?>
	<?php if ($vw_spp_up_list->SortUrl($vw_spp_up_list->tgl_spp) == "") { ?>
		<th data-name="tgl_spp"><div id="elh_vw_spp_up_list_tgl_spp" class="vw_spp_up_list_tgl_spp"><div class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->tgl_spp->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tgl_spp"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_up_list->SortUrl($vw_spp_up_list->tgl_spp) ?>',2);"><div id="elh_vw_spp_up_list_tgl_spp" class="vw_spp_up_list_tgl_spp">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->tgl_spp->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_spp_up_list->tgl_spp->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_spp_up_list->tgl_spp->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_spp_up_list->keterangan->Visible) { // keterangan ?>
	<?php if ($vw_spp_up_list->SortUrl($vw_spp_up_list->keterangan) == "") { ?>
		<th data-name="keterangan"><div id="elh_vw_spp_up_list_keterangan" class="vw_spp_up_list_keterangan"><div class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->keterangan->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="keterangan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_up_list->SortUrl($vw_spp_up_list->keterangan) ?>',2);"><div id="elh_vw_spp_up_list_keterangan" class="vw_spp_up_list_keterangan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->keterangan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($vw_spp_up_list->keterangan->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_spp_up_list->keterangan->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_spp_up_list->tahun_anggaran->Visible) { // tahun_anggaran ?>
	<?php if ($vw_spp_up_list->SortUrl($vw_spp_up_list->tahun_anggaran) == "") { ?>
		<th data-name="tahun_anggaran"><div id="elh_vw_spp_up_list_tahun_anggaran" class="vw_spp_up_list_tahun_anggaran"><div class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->tahun_anggaran->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tahun_anggaran"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_up_list->SortUrl($vw_spp_up_list->tahun_anggaran) ?>',2);"><div id="elh_vw_spp_up_list_tahun_anggaran" class="vw_spp_up_list_tahun_anggaran">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_spp_up_list->tahun_anggaran->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_spp_up_list->tahun_anggaran->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_spp_up_list->tahun_anggaran->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
$vw_spp_up_list_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($vw_spp_up_list->ExportAll && $vw_spp_up_list->Export <> "") {
	$vw_spp_up_list_list->StopRec = $vw_spp_up_list_list->TotalRecs;
} else {

	// Set the last record to display
	if ($vw_spp_up_list_list->TotalRecs > $vw_spp_up_list_list->StartRec + $vw_spp_up_list_list->DisplayRecs - 1)
		$vw_spp_up_list_list->StopRec = $vw_spp_up_list_list->StartRec + $vw_spp_up_list_list->DisplayRecs - 1;
	else
		$vw_spp_up_list_list->StopRec = $vw_spp_up_list_list->TotalRecs;
}
$vw_spp_up_list_list->RecCnt = $vw_spp_up_list_list->StartRec - 1;
if ($vw_spp_up_list_list->Recordset && !$vw_spp_up_list_list->Recordset->EOF) {
	$vw_spp_up_list_list->Recordset->MoveFirst();
	$bSelectLimit = $vw_spp_up_list_list->UseSelectLimit;
	if (!$bSelectLimit && $vw_spp_up_list_list->StartRec > 1)
		$vw_spp_up_list_list->Recordset->Move($vw_spp_up_list_list->StartRec - 1);
} elseif (!$vw_spp_up_list->AllowAddDeleteRow && $vw_spp_up_list_list->StopRec == 0) {
	$vw_spp_up_list_list->StopRec = $vw_spp_up_list->GridAddRowCount;
}

// Initialize aggregate
$vw_spp_up_list->RowType = EW_ROWTYPE_AGGREGATEINIT;
$vw_spp_up_list->ResetAttrs();
$vw_spp_up_list_list->RenderRow();
while ($vw_spp_up_list_list->RecCnt < $vw_spp_up_list_list->StopRec) {
	$vw_spp_up_list_list->RecCnt++;
	if (intval($vw_spp_up_list_list->RecCnt) >= intval($vw_spp_up_list_list->StartRec)) {
		$vw_spp_up_list_list->RowCnt++;

		// Set up key count
		$vw_spp_up_list_list->KeyCount = $vw_spp_up_list_list->RowIndex;

		// Init row class and style
		$vw_spp_up_list->ResetAttrs();
		$vw_spp_up_list->CssClass = "";
		if ($vw_spp_up_list->CurrentAction == "gridadd") {
		} else {
			$vw_spp_up_list_list->LoadRowValues($vw_spp_up_list_list->Recordset); // Load row values
		}
		$vw_spp_up_list->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$vw_spp_up_list->RowAttrs = array_merge($vw_spp_up_list->RowAttrs, array('data-rowindex'=>$vw_spp_up_list_list->RowCnt, 'id'=>'r' . $vw_spp_up_list_list->RowCnt . '_vw_spp_up_list', 'data-rowtype'=>$vw_spp_up_list->RowType));
		...
		$vw_spp_up_list_list->RenderListOptions();
?>
	<tr<?php echo $vw_spp_up_list->RowAttributes() ?>>
<?php

// Render list options (body, left)
$vw_spp_up_list_list->ListOptions->Render("body", "left", $vw_spp_up_list_list->RowCnt);
?>
	<?php if ($vw_spp_up_list->id->Visible) { // id ?>
		<td data-name="id"<?php echo $vw_spp_up_list->id->CellAttributes() ?>>
<div id="orig<?php echo $vw_spp_up_list_list->RowCnt ?>_vw_spp_up_list_id" class="hide">
<span id="el<?php echo $vw_spp_up_list_list->RowCnt ?>_vw_spp_up_list_id" class="vw_spp_up_list_id">
...
<a id="<?php echo $vw_spp_up_list_list->PageObjName . "_row_" . $vw_spp_up_list_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($vw_spp_up_list->status_spp->Visible) { // status_spp ?>
		<td data-name="status_spp"<?php echo $vw_spp_up_list->status_spp->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_up_list_list->RowCnt ?>_vw_spp_up_list_status_spp" class="vw_spp_up_list_status_spp">
...
<?php echo $vw_spp_up_list->status_spp->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($vw_spp_up_list->no_spp->Visible) { // no_spp ?>
		<td data-name="no_spp"<?php echo $vw_spp_up_list->no_spp->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_up_list_list->RowCnt ?>_vw_spp_up_list_no_spp" class="vw_spp_up_list_no_spp">
...
<?php echo $vw_spp_up_list->no_spp->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($vw_spp_up_list->tgl_spp->Visible) { // tgl_spp ?>
		<td data-name="tgl_spp"<?php echo $vw_spp_up_list->tgl_spp->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_up_list_list->RowCnt ?>_vw_spp_up_list_tgl_spp" class="vw_spp_up_list_tgl_spp">
...
<?php echo $vw_spp_up_list->tgl_spp->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($vw_spp_up_list->keterangan->Visible) { // keterangan ?>
		<td data-name="keterangan"<?php echo $vw_spp_up_list->keterangan->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_up_list_list->RowCnt ?>_vw_spp_up_list_keterangan" class="vw_spp_up_list_keterangan">
...
<?php echo $vw_spp_up_list->keterangan->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($vw_spp_up_list->tahun_anggaran->Visible) { // tahun_anggaran ?>
		<td data-name="tahun_anggaran"<?php echo $vw_spp_up_list->tahun_anggaran->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_up_list_list->RowCnt ?>_vw_spp_up_list_tahun_anggaran" class="vw_spp_up_list_tahun_anggaran">
...
$vw_spp_up_list_list->ListOptions->Render("body", "right", $vw_spp_up_list_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($vw_spp_up_list->CurrentAction <> "gridadd")
		$vw_spp_up_list_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($vw_spp_up_list->CurrentAction == "") { ?>
...
// Close recordset
if ($vw_spp_up_list_list->Recordset)
	$vw_spp_up_list_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($vw_spp_up_list->CurrentAction <> "gridadd" && $vw_spp_up_list->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($vw_spp_up_list_list->Pager)) $vw_spp_up_list_list->Pager = new cPrevNextPager($vw_spp_up_list_list->StartRec, $vw_spp_up_list_list->DisplayRecs, $vw_spp_up_list_list->TotalRecs) ?>
<?php if ($vw_spp_up_list_list->Pager->RecordCount > 0 && $vw_spp_up_list_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($vw_spp_up_list_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $vw_spp_up_list_list->PageUrl() ?>start=<?php echo $vw_spp_up_list_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($vw_spp_up_list_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $vw_spp_up_list_list->PageUrl() ?>start=<?php echo $vw_spp_up_list_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $vw_spp_up_list_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($vw_spp_up_list_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $vw_spp_up_list_list->PageUrl() ?>start=<?php echo $vw_spp_up_list_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($vw_spp_up_list_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $vw_spp_up_list_list->PageUrl() ?>start=<?php echo $vw_spp_up_list_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $vw_spp_up_list_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $vw_spp_up_list_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $vw_spp_up_list_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $vw_spp_up_list_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($vw_spp_up_list_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $vw_spp_up_list_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="vw_spp_up_list">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($vw_spp_up_list_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($vw_spp_up_list_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($vw_spp_up_list_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($vw_spp_up_list_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($vw_spp_up_list_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($vw_spp_up_list_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($vw_spp_up_list_list->TotalRecs == 0 && $vw_spp_up_list->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($vw_spp_up_list_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fvw_spp_up_listlistsrch.FilterList = <?php echo $vw_spp_up_list_list->GetFilterList() ?>;
fvw_spp_up_listlistsrch.Init();
fvw_spp_up_listlist.Init();
</script>
<?php
$vw_spp_up_list_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_spp_up_list_list->Page_Terminate();
?>
