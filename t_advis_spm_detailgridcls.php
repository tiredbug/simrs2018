...
$t_advis_spm_detail_grid = NULL; // Initialize page object first

class ct_advis_spm_detail_grid extends ct_advis_spm_detail {
	...
	// Page object name
	var $PageObjName = 't_advis_spm_detail_grid';

	// Grid form hidden field names
	var $FormName = 'ft_advis_spm_detailgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_advis_spm_detail"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_no_spm") && $objForm->HasValue("o_no_spm") && $this->no_spm->CurrentValue <> $this->no_spm->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nama_rekanan") && $objForm->HasValue("o_nama_rekanan") && $this->nama_rekanan->CurrentValue <> $this->nama_rekanan->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nama_bank") && $objForm->HasValue("o_nama_bank") && $this->nama_bank->CurrentValue <> $this->nama_bank->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// nama_rekanan
			$this->nama_rekanan->SetDbValueDef($rsnew, $this->nama_rekanan->CurrentValue, NULL, $this->nama_rekanan->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->id_advis->getSessionValue() <> "") {
			$rsnew['id_advis'] = $this->id_advis->getSessionValue();
		}
		...
		if ($this->no_advis->getSessionValue() <> "") {
			$rsnew['no_advis'] = $this->no_advis->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_advis_spm") {
			$this->id_advis->Visible = FALSE;
			if ($GLOBALS["t_advis_spm"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->no_advis->Visible = FALSE;
			if ($GLOBALS["t_advis_spm"]->EventCancelled) $this->EventCancelled = TRUE;
