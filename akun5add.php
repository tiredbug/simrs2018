...
$akun5_add = NULL; // Initialize page object first

class cakun5_add extends cakun5 {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["kel6"] != "") {
				...
				$this->setKey("kel6", $this->kel6->CurrentValue); // Set up key
			} else {
				$this->setKey("kel6", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "akun5view.php")
	...
	function LoadDefaultValues() {
		...
		$this->kel41->CurrentValue = NULL;
		$this->kel41->OldValue = $this->kel41->CurrentValue;
		...
		$this->kel6->CurrentValue = NULL;
		$this->kel6->OldValue = $this->kel6->CurrentValue;
		$this->nmkel5->CurrentValue = NULL;
		$this->nmkel5->OldValue = $this->nmkel5->CurrentValue;
	}
	...
	function RenderRow() {
		...
		$this->nmkel5->ViewCustomAttributes = "";
			...
			$this->kel6->EditValue = ew_HtmlEncode($this->kel6->CurrentValue);
			$this->kel6->PlaceHolder = ew_RemoveHtml($this->kel6->FldCaption());
	...
	function AddRow($rsold = NULL) {
		...
		// kel41
		$this->kel41->SetDbValueDef($rsnew, $this->kel41->CurrentValue, NULL, FALSE);

		// kel4
		$this->kel4->SetDbValueDef($rsnew, $this->kel4->CurrentValue, NULL, FALSE);

		// kel5
		$this->kel5->SetDbValueDef($rsnew, $this->kel5->CurrentValue, NULL, FALSE);

		// kel6
		$this->kel6->SetDbValueDef($rsnew, $this->kel6->CurrentValue, "", FALSE);

		// nmkel5
		$this->nmkel5->SetDbValueDef($rsnew, $this->nmkel5->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['kel6']) == "") {
...
// Create page object
if (!isset($akun5_add)) $akun5_add = new cakun5_add();
...
var CurrentForm = fakun5add = new ew_Form("fakun5add", "add");

// Validate form
fakun5add.Validate = function() {
...
fakun5add.ValidateRequired = true;
<?php } else { ?>
fakun5add.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$akun5_add->IsModal) { ?>
<?php } ?>
<?php $akun5_add->ShowPageHeader(); ?>
<?php
$akun5_add->ShowMessage();
?>
<form name="fakun5add" id="fakun5add" class="<?php echo $akun5_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($akun5_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $akun5_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="akun5" data-field="x_kel6" name="x_kel6" id="x_kel6" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($akun5->kel6->getPlaceHolder()) ?>" value="<?php echo $akun5->kel6->EditValue ?>"<?php echo $akun5->kel6->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $akun5_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
