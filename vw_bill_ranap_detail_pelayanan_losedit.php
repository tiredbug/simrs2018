...
$vw_bill_ranap_detail_pelayanan_los_edit = NULL; // Initialize page object first

class cvw_bill_ranap_detail_pelayanan_los_edit extends cvw_bill_ranap_detail_pelayanan_los {
	...
	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_pelayanan_los_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("vw_bill_ranap_detail_pelayanan_loslist.php"));
			else
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("vw_bill_ranap_detail_pelayanan_loslist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("vw_bill_ranap_detail_pelayanan_loslist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "vw_bill_ranap_detail_pelayanan_loslist.php")
		...
		$this->no_ruang->ViewCustomAttributes = "";
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				...
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					...
					$this->kode_tindakan->EditValue = $this->kode_tindakan->DisplayValue($arwrk);
					$rswrk->Close();
				} else {
					$this->kode_tindakan->EditValue = ew_HtmlEncode($this->kode_tindakan->CurrentValue);
				}
			} else {
				$this->kode_tindakan->EditValue = NULL;
			}
	...
	function ValidateForm() {
		...
		if (!ew_CheckInteger($this->kode_tindakan->FormValue)) {
			ew_AddMessage($gsFormError, $this->kode_tindakan->FldErrMsg());
		}
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("vw_bill_ranap_detail_pelayanan_loslist.php"), "", $this->TableVar, TRUE);
		$PageId = "edit";
	...
	function SetupAutoSuggestFilters($fld, $pageId = null) {
		...
		case "x_kode_tindakan":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `kode`, `nama_tindakan` AS `DispFld` FROM `vw_bill_ranap_data_tarif_tindakan`";
			$sWhereWrk = "`nama_tindakan` LIKE '%{query_value}%'";
			$this->kode_tindakan->LookupFilters = array();
			$lookuptblfilter = "`kelompok_tindakan`='8'";
			ew_AddFilter($sWhereWrk, $lookuptblfilter);
...
// Create page object
if (!isset($vw_bill_ranap_detail_pelayanan_los_edit)) $vw_bill_ranap_detail_pelayanan_los_edit = new cvw_bill_ranap_detail_pelayanan_los_edit();

// Page init
$vw_bill_ranap_detail_pelayanan_los_edit->Page_Init();

// Page main
$vw_bill_ranap_detail_pelayanan_los_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_bill_ranap_detail_pelayanan_los_edit->Page_Render();
?>
...
var CurrentForm = fvw_bill_ranap_detail_pelayanan_losedit = new ew_Form("fvw_bill_ranap_detail_pelayanan_losedit", "edit");

// Validate form
fvw_bill_ranap_detail_pelayanan_losedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->id_admission->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_statusbayar");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->statusbayar->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelas");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->kelas->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_tanggal");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $vw_bill_ranap_detail_pelayanan_los->tanggal->FldCaption(), $vw_bill_ranap_detail_pelayanan_los->tanggal->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_tanggal");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->tanggal->FldErrMsg()) ?>");
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->kelompok1->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok2");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->kelompok2->FldErrMsg()) ?>");
			...
			if (elm && !ew_CheckNumber(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->bhp->FldErrMsg()) ?>");
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->kelompok_tindakan->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kode_dokter");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_pelayanan_los->kode_dokter->FldErrMsg()) ?>");
...
fvw_bill_ranap_detail_pelayanan_losedit.Form_CustomValidate = 
...
fvw_bill_ranap_detail_pelayanan_losedit.ValidateRequired = true;
<?php } else { ?>
fvw_bill_ranap_detail_pelayanan_losedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_bill_ranap_detail_pelayanan_losedit.Lists["x_kode_tindakan"] = {"LinkField":"x_kode","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama_tindakan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_bill_ranap_data_tarif_tindakan"};
...
<?php if (!$vw_bill_ranap_detail_pelayanan_los_edit->IsModal) { ?>
<?php } ?>
<?php $vw_bill_ranap_detail_pelayanan_los_edit->ShowPageHeader(); ?>
<?php
$vw_bill_ranap_detail_pelayanan_los_edit->ShowMessage();
?>
<form name="fvw_bill_ranap_detail_pelayanan_losedit" id="fvw_bill_ranap_detail_pelayanan_losedit" class="<?php echo $vw_bill_ranap_detail_pelayanan_los_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_bill_ranap_detail_pelayanan_los_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->getCurrentMasterTable() == "vw_bill_ranap") { ?>
...
<?php } ?>
<div>
<?php if ($vw_bill_ranap_detail_pelayanan_los->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_id" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->id->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_id">
<span<?php echo $vw_bill_ranap_detail_pelayanan_los->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_pelayanan_los->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->id->CurrentValue) ?>">
<?php echo $vw_bill_ranap_detail_pelayanan_los->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->id_admission->Visible) { // id_admission ?>
	<div id="r_id_admission" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_id_admission" for="x_id_admission" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->id_admission->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->id_admission->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_pelayanan_los->id_admission->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_pelayanan_los_id_admission">
<span<?php echo $vw_bill_ranap_detail_pelayanan_los->id_admission->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_pelayanan_los->id_admission->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_id_admission" name="x_id_admission" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->id_admission->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_pelayanan_los_id_admission">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_id_admission" name="x_id_admission" id="x_id_admission" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->id_admission->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->id_admission->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->id_admission->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_pelayanan_los->id_admission->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->nomr->Visible) { // nomr ?>
	<div id="r_nomr" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_nomr" for="x_nomr" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->nomr->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->nomr->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_pelayanan_los->nomr->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_pelayanan_los_nomr">
<span<?php echo $vw_bill_ranap_detail_pelayanan_los->nomr->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_pelayanan_los->nomr->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_nomr" name="x_nomr" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->nomr->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_pelayanan_los_nomr">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_nomr" name="x_nomr" id="x_nomr" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->nomr->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->nomr->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->nomr->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_pelayanan_los->nomr->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->statusbayar->Visible) { // statusbayar ?>
	<div id="r_statusbayar" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_statusbayar" for="x_statusbayar" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->statusbayar->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->statusbayar->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_pelayanan_los->statusbayar->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_pelayanan_los_statusbayar">
<span<?php echo $vw_bill_ranap_detail_pelayanan_los->statusbayar->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_pelayanan_los->statusbayar->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_statusbayar" name="x_statusbayar" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->statusbayar->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_pelayanan_los_statusbayar">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_statusbayar" name="x_statusbayar" id="x_statusbayar" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->statusbayar->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->statusbayar->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->statusbayar->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_pelayanan_los->statusbayar->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->kelas->Visible) { // kelas ?>
	<div id="r_kelas" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_kelas" for="x_kelas" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->kelas->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->kelas->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_pelayanan_los->kelas->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_pelayanan_los_kelas">
<span<?php echo $vw_bill_ranap_detail_pelayanan_los->kelas->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_pelayanan_los->kelas->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_kelas" name="x_kelas" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kelas->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_pelayanan_los_kelas">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_kelas" name="x_kelas" id="x_kelas" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kelas->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->kelas->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->kelas->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_pelayanan_los->kelas->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->tanggal->Visible) { // tanggal ?>
	<div id="r_tanggal" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_tanggal" for="x_tanggal" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->tanggal->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->tanggal->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_tanggal">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_tanggal" name="x_tanggal" id="x_tanggal" size="8" maxlength="8" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->tanggal->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->tanggal->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->tanggal->EditAttributes() ?>>
<?php if (!$vw_bill_ranap_detail_pelayanan_los->tanggal->ReadOnly && !$vw_bill_ranap_detail_pelayanan_los->tanggal->Disabled && !isset($vw_bill_ranap_detail_pelayanan_los->tanggal->EditAttrs["readonly"]) && !isset($vw_bill_ranap_detail_pelayanan_los->tanggal->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateCalendar("fvw_bill_ranap_detail_pelayanan_losedit", "x_tanggal", 0);
</script>
<?php } ?>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->tanggal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->kode_tindakan->Visible) { // kode_tindakan ?>
	<div id="r_kode_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_kode_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->kode_tindakan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_kode_tindakan">
<?php
...
?>
<span id="as_x_kode_tindakan" style="white-space: nowrap; z-index: 8930">
	<input type="text" name="sv_x_kode_tindakan" id="sv_x_kode_tindakan" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_tindakan->EditValue ?>" size="55" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kode_tindakan->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kode_tindakan->getPlaceHolder()) ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_tindakan->EditAttributes() ?>>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_kode_tindakan" data-value-separator="<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_tindakan->DisplayValueSeparatorAttribute() ?>" name="x_kode_tindakan" id="x_kode_tindakan" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kode_tindakan->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x_kode_tindakan" id="q_x_kode_tindakan" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_tindakan->LookupFilterQuery(true) ?>">
<script type="text/javascript">
fvw_bill_ranap_detail_pelayanan_losedit.CreateAutoSuggest({"id":"x_kode_tindakan","forceSelect":true});
</script>
<input type="hidden" name="ln_x_kode_tindakan" id="ln_x_kode_tindakan" value="x_nama_tindakan,x_kelompok_tindakan,x_kelompok1,x_kelompok2,x_tarif,x_bhp">
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->nama_tindakan->Visible) { // nama_tindakan ?>
	<div id="r_nama_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_nama_tindakan" for="x_nama_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->nama_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->nama_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_nama_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_nama_tindakan" name="x_nama_tindakan" id="x_nama_tindakan" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->nama_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->nama_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->nama_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->nama_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->kelompok1->Visible) { // kelompok1 ?>
	<div id="r_kelompok1" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_kelompok1" for="x_kelompok1" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok1->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok1->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_kelompok1">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_kelompok1" name="x_kelompok1" id="x_kelompok1" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kelompok1->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok1->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok1->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok1->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->kelompok2->Visible) { // kelompok2 ?>
	<div id="r_kelompok2" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_kelompok2" for="x_kelompok2" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok2->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok2->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_kelompok2">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_kelompok2" name="x_kelompok2" id="x_kelompok2" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kelompok2->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok2->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok2->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok2->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->tarif->Visible) { // tarif ?>
	<div id="r_tarif" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_tarif" for="x_tarif" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->tarif->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->tarif->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_tarif">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_tarif" name="x_tarif" id="x_tarif" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->tarif->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->tarif->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->tarif->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->tarif->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->bhp->Visible) { // bhp ?>
	<div id="r_bhp" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_bhp" for="x_bhp" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->bhp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->bhp->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_bhp">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_bhp" name="x_bhp" id="x_bhp" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->bhp->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->bhp->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->bhp->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->bhp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->qty->Visible) { // qty ?>
	<div id="r_qty" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_qty" for="x_qty" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->qty->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->qty->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_qty">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_qty" name="x_qty" id="x_qty" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->qty->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->qty->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->qty->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->qty->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->user->Visible) { // user ?>
	<div id="r_user" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_user" for="x_user" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->user->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->user->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_user">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_user" name="x_user" id="x_user" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->user->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->user->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->user->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->user->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->kelompok_tindakan->Visible) { // kelompok_tindakan ?>
	<div id="r_kelompok_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_kelompok_tindakan" for="x_kelompok_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_kelompok_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_kelompok_tindakan" name="x_kelompok_tindakan" id="x_kelompok_tindakan" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kelompok_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->kelompok_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->kode_dokter->Visible) { // kode_dokter ?>
	<div id="r_kode_dokter" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_kode_dokter" for="x_kode_dokter" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->kode_dokter->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_dokter->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_kode_dokter">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_kode_dokter" name="x_kode_dokter" id="x_kode_dokter" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->kode_dokter->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_dokter->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_dokter->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->kode_dokter->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_pelayanan_los->no_ruang->Visible) { // no_ruang ?>
	<div id="r_no_ruang" class="form-group">
		<label id="elh_vw_bill_ranap_detail_pelayanan_los_no_ruang" for="x_no_ruang" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_pelayanan_los->no_ruang->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_pelayanan_los->no_ruang->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_pelayanan_los_no_ruang">
<input type="text" data-table="vw_bill_ranap_detail_pelayanan_los" data-field="x_no_ruang" name="x_no_ruang" id="x_no_ruang" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_pelayanan_los->no_ruang->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_pelayanan_los->no_ruang->EditValue ?>"<?php echo $vw_bill_ranap_detail_pelayanan_los->no_ruang->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_pelayanan_los->no_ruang->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$vw_bill_ranap_detail_pelayanan_los_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_bill_ranap_detail_pelayanan_los_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvw_bill_ranap_detail_pelayanan_losedit.Init();
</script>
<?php
$vw_bill_ranap_detail_pelayanan_los_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_bill_ranap_detail_pelayanan_los_edit->Page_Terminate();
?>
