...
$vw_spp_detail_pajak_list = NULL; // Initialize page object first

class cvw_spp_detail_pajak_list extends cvw_spp_detail_pajak {
	...
	// Page object name
	var $PageObjName = 'vw_spp_detail_pajak_list';

	// Grid form hidden field names
	var $FormName = 'fvw_spp_detail_pajaklist';
	...
	function __construct() {
		...
		$this->MultiDeleteUrl = "vw_spp_detail_pajakdelete.php";
		$this->MultiUpdateUrl = "vw_spp_detail_pajakupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fvw_spp_detail_pajaklistsrch";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fvw_spp_detail_pajaklistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fvw_spp_detail_pajaklistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fvw_spp_detail_pajaklist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
// Create page object
if (!isset($vw_spp_detail_pajak_list)) $vw_spp_detail_pajak_list = new cvw_spp_detail_pajak_list();

// Page init
$vw_spp_detail_pajak_list->Page_Init();

// Page main
$vw_spp_detail_pajak_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_spp_detail_pajak_list->Page_Render();
?>
...
var CurrentForm = fvw_spp_detail_pajaklist = new ew_Form("fvw_spp_detail_pajaklist", "list");
fvw_spp_detail_pajaklist.FormKeyCountName = '<?php echo $vw_spp_detail_pajak_list->FormKeyCountName ?>';

// Form_CustomValidate event
fvw_spp_detail_pajaklist.Form_CustomValidate = 
...
fvw_spp_detail_pajaklist.ValidateRequired = true;
<?php } else { ?>
fvw_spp_detail_pajaklist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_spp_detail_pajaklist.Lists["x_kd_rekening_belanja"] = {"LinkField":"x_kd_akun","Ajax":true,"AutoFill":false,"DisplayFields":["x_kd_akun","x_nama_akun","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"keu_akun5"};
...
<?php if ($vw_spp_detail_pajak_list->TotalRecs > 0 && $vw_spp_detail_pajak_list->ExportOptions->Visible()) { ?>
<?php $vw_spp_detail_pajak_list->ExportOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php if (($vw_spp_detail_pajak->Export == "") || (EW_EXPORT_MASTER_RECORD && $vw_spp_detail_pajak->Export == "print")) { ?>
<?php
if ($vw_spp_detail_pajak_list->DbMasterFilter <> "" && $vw_spp_detail_pajak->getCurrentMasterTable() == "vw_spp_ls_kontrak_list") {
	if ($vw_spp_detail_pajak_list->MasterRecordExists) {
?>
...
<?php } ?>
<?php
	$bSelectLimit = $vw_spp_detail_pajak_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($vw_spp_detail_pajak_list->TotalRecs <= 0)
			$vw_spp_detail_pajak_list->TotalRecs = $vw_spp_detail_pajak->SelectRecordCount();
	} else {
		if (!$vw_spp_detail_pajak_list->Recordset && ($vw_spp_detail_pajak_list->Recordset = $vw_spp_detail_pajak_list->LoadRecordset()))
			$vw_spp_detail_pajak_list->TotalRecs = $vw_spp_detail_pajak_list->Recordset->RecordCount();
	}
	$vw_spp_detail_pajak_list->StartRec = 1;
	if ($vw_spp_detail_pajak_list->DisplayRecs <= 0 || ($vw_spp_detail_pajak->Export <> "" && $vw_spp_detail_pajak->ExportAll)) // Display all records
		$vw_spp_detail_pajak_list->DisplayRecs = $vw_spp_detail_pajak_list->TotalRecs;
	if (!($vw_spp_detail_pajak->Export <> "" && $vw_spp_detail_pajak->ExportAll))
		$vw_spp_detail_pajak_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$vw_spp_detail_pajak_list->Recordset = $vw_spp_detail_pajak_list->LoadRecordset($vw_spp_detail_pajak_list->StartRec-1, $vw_spp_detail_pajak_list->DisplayRecs);

	// Set no record found message
	if ($vw_spp_detail_pajak->CurrentAction == "" && $vw_spp_detail_pajak_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$vw_spp_detail_pajak_list->setWarningMessage(ew_DeniedMsg());
		if ($vw_spp_detail_pajak_list->SearchWhere == "0=101")
			$vw_spp_detail_pajak_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$vw_spp_detail_pajak_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$vw_spp_detail_pajak_list->RenderOtherOptions();
?>
<?php $vw_spp_detail_pajak_list->ShowPageHeader(); ?>
<?php
$vw_spp_detail_pajak_list->ShowMessage();
?>
<?php if ($vw_spp_detail_pajak_list->TotalRecs > 0 || $vw_spp_detail_pajak->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid vw_spp_detail_pajak">
<form name="fvw_spp_detail_pajaklist" id="fvw_spp_detail_pajaklist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_spp_detail_pajak_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_spp_detail_pajak_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_spp_detail_pajak">
<?php if ($vw_spp_detail_pajak->getCurrentMasterTable() == "vw_spp_ls_kontrak_list" && $vw_spp_detail_pajak->CurrentAction <> "") { ?>
...
<?php if ($vw_spp_detail_pajak_list->TotalRecs > 0 || $vw_spp_detail_pajak->CurrentAction == "gridedit") { ?>
<table id="tbl_vw_spp_detail_pajaklist" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$vw_spp_detail_pajak_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$vw_spp_detail_pajak_list->RenderListOptions();

// Render list options (header, left)
$vw_spp_detail_pajak_list->ListOptions->Render("header", "left");
?>
<?php if ($vw_spp_detail_pajak->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
	...
	<?php } else { ?>
		<th data-name="kd_rekening_belanja"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_detail_pajak->SortUrl($vw_spp_detail_pajak->kd_rekening_belanja) ?>',2);"><div id="elh_vw_spp_detail_pajak_kd_rekening_belanja" class="vw_spp_detail_pajak_kd_rekening_belanja">
...
<?php if ($vw_spp_detail_pajak->jumlah_belanja->Visible) { // jumlah_belanja ?>
	...
	<?php } else { ?>
		<th data-name="jumlah_belanja"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_detail_pajak->SortUrl($vw_spp_detail_pajak->jumlah_belanja) ?>',2);"><div id="elh_vw_spp_detail_pajak_jumlah_belanja" class="vw_spp_detail_pajak_jumlah_belanja">
...
$vw_spp_detail_pajak_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($vw_spp_detail_pajak->ExportAll && $vw_spp_detail_pajak->Export <> "") {
	$vw_spp_detail_pajak_list->StopRec = $vw_spp_detail_pajak_list->TotalRecs;
} else {

	// Set the last record to display
	if ($vw_spp_detail_pajak_list->TotalRecs > $vw_spp_detail_pajak_list->StartRec + $vw_spp_detail_pajak_list->DisplayRecs - 1)
		$vw_spp_detail_pajak_list->StopRec = $vw_spp_detail_pajak_list->StartRec + $vw_spp_detail_pajak_list->DisplayRecs - 1;
	else
		$vw_spp_detail_pajak_list->StopRec = $vw_spp_detail_pajak_list->TotalRecs;
}
$vw_spp_detail_pajak_list->RecCnt = $vw_spp_detail_pajak_list->StartRec - 1;
if ($vw_spp_detail_pajak_list->Recordset && !$vw_spp_detail_pajak_list->Recordset->EOF) {
	$vw_spp_detail_pajak_list->Recordset->MoveFirst();
	$bSelectLimit = $vw_spp_detail_pajak_list->UseSelectLimit;
	if (!$bSelectLimit && $vw_spp_detail_pajak_list->StartRec > 1)
		$vw_spp_detail_pajak_list->Recordset->Move($vw_spp_detail_pajak_list->StartRec - 1);
} elseif (!$vw_spp_detail_pajak->AllowAddDeleteRow && $vw_spp_detail_pajak_list->StopRec == 0) {
	$vw_spp_detail_pajak_list->StopRec = $vw_spp_detail_pajak->GridAddRowCount;
}

// Initialize aggregate
$vw_spp_detail_pajak->RowType = EW_ROWTYPE_AGGREGATEINIT;
$vw_spp_detail_pajak->ResetAttrs();
$vw_spp_detail_pajak_list->RenderRow();
while ($vw_spp_detail_pajak_list->RecCnt < $vw_spp_detail_pajak_list->StopRec) {
	$vw_spp_detail_pajak_list->RecCnt++;
	if (intval($vw_spp_detail_pajak_list->RecCnt) >= intval($vw_spp_detail_pajak_list->StartRec)) {
		$vw_spp_detail_pajak_list->RowCnt++;

		// Set up key count
		$vw_spp_detail_pajak_list->KeyCount = $vw_spp_detail_pajak_list->RowIndex;
		...
		if ($vw_spp_detail_pajak->CurrentAction == "gridadd") {
			...
			$vw_spp_detail_pajak_list->LoadRowValues($vw_spp_detail_pajak_list->Recordset); // Load row values
		}
		$vw_spp_detail_pajak->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$vw_spp_detail_pajak->RowAttrs = array_merge($vw_spp_detail_pajak->RowAttrs, array('data-rowindex'=>$vw_spp_detail_pajak_list->RowCnt, 'id'=>'r' . $vw_spp_detail_pajak_list->RowCnt . '_vw_spp_detail_pajak', 'data-rowtype'=>$vw_spp_detail_pajak->RowType));
...
$vw_spp_detail_pajak_list->ListOptions->Render("body", "left", $vw_spp_detail_pajak_list->RowCnt);
?>
	<?php if ($vw_spp_detail_pajak->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
		<td data-name="kd_rekening_belanja"<?php echo $vw_spp_detail_pajak->kd_rekening_belanja->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_detail_pajak_list->RowCnt ?>_vw_spp_detail_pajak_kd_rekening_belanja" class="vw_spp_detail_pajak_kd_rekening_belanja">
...
<a id="<?php echo $vw_spp_detail_pajak_list->PageObjName . "_row_" . $vw_spp_detail_pajak_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($vw_spp_detail_pajak->jumlah_belanja->Visible) { // jumlah_belanja ?>
		<td data-name="jumlah_belanja"<?php echo $vw_spp_detail_pajak->jumlah_belanja->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_detail_pajak_list->RowCnt ?>_vw_spp_detail_pajak_jumlah_belanja" class="vw_spp_detail_pajak_jumlah_belanja">
...
$vw_spp_detail_pajak_list->ListOptions->Render("body", "right", $vw_spp_detail_pajak_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($vw_spp_detail_pajak->CurrentAction <> "gridadd")
		$vw_spp_detail_pajak_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($vw_spp_detail_pajak->CurrentAction == "") { ?>
...
// Close recordset
if ($vw_spp_detail_pajak_list->Recordset)
	$vw_spp_detail_pajak_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($vw_spp_detail_pajak->CurrentAction <> "gridadd" && $vw_spp_detail_pajak->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($vw_spp_detail_pajak_list->Pager)) $vw_spp_detail_pajak_list->Pager = new cPrevNextPager($vw_spp_detail_pajak_list->StartRec, $vw_spp_detail_pajak_list->DisplayRecs, $vw_spp_detail_pajak_list->TotalRecs) ?>
<?php if ($vw_spp_detail_pajak_list->Pager->RecordCount > 0 && $vw_spp_detail_pajak_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($vw_spp_detail_pajak_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $vw_spp_detail_pajak_list->PageUrl() ?>start=<?php echo $vw_spp_detail_pajak_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($vw_spp_detail_pajak_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $vw_spp_detail_pajak_list->PageUrl() ?>start=<?php echo $vw_spp_detail_pajak_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $vw_spp_detail_pajak_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($vw_spp_detail_pajak_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $vw_spp_detail_pajak_list->PageUrl() ?>start=<?php echo $vw_spp_detail_pajak_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($vw_spp_detail_pajak_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $vw_spp_detail_pajak_list->PageUrl() ?>start=<?php echo $vw_spp_detail_pajak_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $vw_spp_detail_pajak_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $vw_spp_detail_pajak_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $vw_spp_detail_pajak_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $vw_spp_detail_pajak_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($vw_spp_detail_pajak_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $vw_spp_detail_pajak_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="vw_spp_detail_pajak">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($vw_spp_detail_pajak_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($vw_spp_detail_pajak_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($vw_spp_detail_pajak_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($vw_spp_detail_pajak_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($vw_spp_detail_pajak_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($vw_spp_detail_pajak_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($vw_spp_detail_pajak_list->TotalRecs == 0 && $vw_spp_detail_pajak->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($vw_spp_detail_pajak_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fvw_spp_detail_pajaklist.Init();
</script>
<?php
$vw_spp_detail_pajak_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_spp_detail_pajak_list->Page_Terminate();
?>
