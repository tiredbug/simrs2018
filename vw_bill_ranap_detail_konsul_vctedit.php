...
$vw_bill_ranap_detail_konsul_vct_edit = NULL; // Initialize page object first

class cvw_bill_ranap_detail_konsul_vct_edit extends cvw_bill_ranap_detail_konsul_vct {
	...
	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_konsul_vct_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("vw_bill_ranap_detail_konsul_vctlist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("vw_bill_ranap_detail_konsul_vctlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "vw_bill_ranap_detail_konsul_vctlist.php")
...
// Create page object
if (!isset($vw_bill_ranap_detail_konsul_vct_edit)) $vw_bill_ranap_detail_konsul_vct_edit = new cvw_bill_ranap_detail_konsul_vct_edit();

// Page init
$vw_bill_ranap_detail_konsul_vct_edit->Page_Init();

// Page main
$vw_bill_ranap_detail_konsul_vct_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_bill_ranap_detail_konsul_vct_edit->Page_Render();
?>
...
var CurrentForm = fvw_bill_ranap_detail_konsul_vctedit = new ew_Form("fvw_bill_ranap_detail_konsul_vctedit", "edit");

// Validate form
fvw_bill_ranap_detail_konsul_vctedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_konsul_vct->id_admission->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_statusbayar");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_konsul_vct->statusbayar->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelas");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_konsul_vct->kelas->FldErrMsg()) ?>");
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_konsul_vct->kelompok_tindakan->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok1");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_konsul_vct->kelompok1->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok2");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_konsul_vct->kelompok2->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_bhp");
			if (elm && !ew_CheckNumber(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_konsul_vct->bhp->FldErrMsg()) ?>");
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_konsul_vct->no_ruang->FldErrMsg()) ?>");
...
fvw_bill_ranap_detail_konsul_vctedit.Form_CustomValidate = 
...
fvw_bill_ranap_detail_konsul_vctedit.ValidateRequired = true;
<?php } else { ?>
fvw_bill_ranap_detail_konsul_vctedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_bill_ranap_detail_konsul_vctedit.Lists["x_kode_tindakan"] = {"LinkField":"x_kode","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama_tindakan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_bill_ranap_data_tarif_tindakan"};
fvw_bill_ranap_detail_konsul_vctedit.Lists["x_kode_dokter"] = {"LinkField":"x_kd_dokter","Ajax":true,"AutoFill":false,"DisplayFields":["x_NAMADOKTER","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_dokter_jaga_ranap"};
...
<?php if (!$vw_bill_ranap_detail_konsul_vct_edit->IsModal) { ?>
<?php } ?>
<?php $vw_bill_ranap_detail_konsul_vct_edit->ShowPageHeader(); ?>
<?php
$vw_bill_ranap_detail_konsul_vct_edit->ShowMessage();
?>
<form name="fvw_bill_ranap_detail_konsul_vctedit" id="fvw_bill_ranap_detail_konsul_vctedit" class="<?php echo $vw_bill_ranap_detail_konsul_vct_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_bill_ranap_detail_konsul_vct_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->getCurrentMasterTable() == "vw_bill_ranap") { ?>
<input type="hidden" name="<?php echo EW_TABLE_SHOW_MASTER ?>" value="vw_bill_ranap">
<input type="hidden" name="fk_id_admission" value="<?php echo $vw_bill_ranap_detail_konsul_vct->id_admission->getSessionValue() ?>">
<input type="hidden" name="fk_nomr" value="<?php echo $vw_bill_ranap_detail_konsul_vct->nomr->getSessionValue() ?>">
<input type="hidden" name="fk_statusbayar" value="<?php echo $vw_bill_ranap_detail_konsul_vct->statusbayar->getSessionValue() ?>">
<input type="hidden" name="fk_KELASPERAWATAN_ID" value="<?php echo $vw_bill_ranap_detail_konsul_vct->kelas->getSessionValue() ?>">
<?php } ?>
<div>
<?php if ($vw_bill_ranap_detail_konsul_vct->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_id" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->id->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_id">
<span<?php echo $vw_bill_ranap_detail_konsul_vct->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_konsul_vct->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->id->CurrentValue) ?>">
<?php echo $vw_bill_ranap_detail_konsul_vct->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->id_admission->Visible) { // id_admission ?>
	<div id="r_id_admission" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_id_admission" for="x_id_admission" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->id_admission->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->id_admission->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_konsul_vct->id_admission->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_konsul_vct_id_admission">
<span<?php echo $vw_bill_ranap_detail_konsul_vct->id_admission->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_konsul_vct->id_admission->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_id_admission" name="x_id_admission" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->id_admission->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_konsul_vct_id_admission">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_id_admission" name="x_id_admission" id="x_id_admission" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->id_admission->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->id_admission->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->id_admission->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_konsul_vct->id_admission->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->nomr->Visible) { // nomr ?>
	<div id="r_nomr" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_nomr" for="x_nomr" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->nomr->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->nomr->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_konsul_vct->nomr->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_konsul_vct_nomr">
<span<?php echo $vw_bill_ranap_detail_konsul_vct->nomr->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_konsul_vct->nomr->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_nomr" name="x_nomr" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->nomr->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_konsul_vct_nomr">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_nomr" name="x_nomr" id="x_nomr" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->nomr->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->nomr->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->nomr->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_konsul_vct->nomr->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->statusbayar->Visible) { // statusbayar ?>
	<div id="r_statusbayar" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_statusbayar" for="x_statusbayar" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->statusbayar->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->statusbayar->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_konsul_vct->statusbayar->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_konsul_vct_statusbayar">
<span<?php echo $vw_bill_ranap_detail_konsul_vct->statusbayar->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_konsul_vct->statusbayar->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_statusbayar" name="x_statusbayar" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->statusbayar->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_konsul_vct_statusbayar">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_statusbayar" name="x_statusbayar" id="x_statusbayar" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->statusbayar->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->statusbayar->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->statusbayar->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_konsul_vct->statusbayar->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->kelas->Visible) { // kelas ?>
	<div id="r_kelas" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_kelas" for="x_kelas" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->kelas->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->kelas->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_konsul_vct->kelas->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_konsul_vct_kelas">
<span<?php echo $vw_bill_ranap_detail_konsul_vct->kelas->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_konsul_vct->kelas->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_kelas" name="x_kelas" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->kelas->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_konsul_vct_kelas">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_kelas" name="x_kelas" id="x_kelas" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->kelas->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->kelas->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->kelas->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_konsul_vct->kelas->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->tanggal->Visible) { // tanggal ?>
	<div id="r_tanggal" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_tanggal" for="x_tanggal" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->tanggal->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->tanggal->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_tanggal">
...
<script type="text/javascript">
ew_CreateCalendar("fvw_bill_ranap_detail_konsul_vctedit", "x_tanggal", 7);
</script>
<?php } ?>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->tanggal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->kode_tindakan->Visible) { // kode_tindakan ?>
	<div id="r_kode_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_kode_tindakan" for="x_kode_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->kode_tindakan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->kode_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_kode_tindakan">
...
<select data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_kode_tindakan" data-value-separator="<?php echo $vw_bill_ranap_detail_konsul_vct->kode_tindakan->DisplayValueSeparatorAttribute() ?>" id="x_kode_tindakan" name="x_kode_tindakan"<?php echo $vw_bill_ranap_detail_konsul_vct->kode_tindakan->EditAttributes() ?>>
<?php echo $vw_bill_ranap_detail_konsul_vct->kode_tindakan->SelectOptionListHtml("x_kode_tindakan") ?>
</select>
<input type="hidden" name="s_x_kode_tindakan" id="s_x_kode_tindakan" value="<?php echo $vw_bill_ranap_detail_konsul_vct->kode_tindakan->LookupFilterQuery() ?>">
<input type="hidden" name="ln_x_kode_tindakan" id="ln_x_kode_tindakan" value="x_tarif,x_bhp">
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->kode_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->kode_dokter->Visible) { // kode_dokter ?>
	<div id="r_kode_dokter" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_kode_dokter" for="x_kode_dokter" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->kode_dokter->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->kode_dokter->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_kode_dokter">
<select data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_kode_dokter" data-value-separator="<?php echo $vw_bill_ranap_detail_konsul_vct->kode_dokter->DisplayValueSeparatorAttribute() ?>" id="x_kode_dokter" name="x_kode_dokter"<?php echo $vw_bill_ranap_detail_konsul_vct->kode_dokter->EditAttributes() ?>>
<?php echo $vw_bill_ranap_detail_konsul_vct->kode_dokter->SelectOptionListHtml("x_kode_dokter") ?>
</select>
<input type="hidden" name="s_x_kode_dokter" id="s_x_kode_dokter" value="<?php echo $vw_bill_ranap_detail_konsul_vct->kode_dokter->LookupFilterQuery() ?>">
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->kode_dokter->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->tarif->Visible) { // tarif ?>
	<div id="r_tarif" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_tarif" for="x_tarif" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->tarif->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->tarif->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_tarif">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_tarif" name="x_tarif" id="x_tarif" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->tarif->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->tarif->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->tarif->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->tarif->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->qty->Visible) { // qty ?>
	<div id="r_qty" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_qty" for="x_qty" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->qty->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->qty->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_qty">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_qty" name="x_qty" id="x_qty" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->qty->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->qty->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->qty->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->qty->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->nama_tindakan->Visible) { // nama_tindakan ?>
	<div id="r_nama_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_nama_tindakan" for="x_nama_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->nama_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->nama_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_nama_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_nama_tindakan" name="x_nama_tindakan" id="x_nama_tindakan" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->nama_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->nama_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->nama_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->nama_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->kelompok_tindakan->Visible) { // kelompok_tindakan ?>
	<div id="r_kelompok_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_kelompok_tindakan" for="x_kelompok_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->kelompok_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_kelompok_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_kelompok_tindakan" name="x_kelompok_tindakan" id="x_kelompok_tindakan" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->kelompok_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->kelompok1->Visible) { // kelompok1 ?>
	<div id="r_kelompok1" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_kelompok1" for="x_kelompok1" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->kelompok1->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok1->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_kelompok1">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_kelompok1" name="x_kelompok1" id="x_kelompok1" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->kelompok1->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok1->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok1->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok1->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->kelompok2->Visible) { // kelompok2 ?>
	<div id="r_kelompok2" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_kelompok2" for="x_kelompok2" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->kelompok2->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok2->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_kelompok2">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_kelompok2" name="x_kelompok2" id="x_kelompok2" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->kelompok2->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok2->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok2->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->kelompok2->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->bhp->Visible) { // bhp ?>
	<div id="r_bhp" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_bhp" for="x_bhp" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->bhp->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->bhp->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_bhp">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_bhp" name="x_bhp" id="x_bhp" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->bhp->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->bhp->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->bhp->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->bhp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->user->Visible) { // user ?>
	<div id="r_user" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_user" for="x_user" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->user->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->user->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_user">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_user" name="x_user" id="x_user" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->user->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->user->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->user->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->user->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_konsul_vct->no_ruang->Visible) { // no_ruang ?>
	<div id="r_no_ruang" class="form-group">
		<label id="elh_vw_bill_ranap_detail_konsul_vct_no_ruang" for="x_no_ruang" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_konsul_vct->no_ruang->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_konsul_vct->no_ruang->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_konsul_vct_no_ruang">
<input type="text" data-table="vw_bill_ranap_detail_konsul_vct" data-field="x_no_ruang" name="x_no_ruang" id="x_no_ruang" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_konsul_vct->no_ruang->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_konsul_vct->no_ruang->EditValue ?>"<?php echo $vw_bill_ranap_detail_konsul_vct->no_ruang->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_konsul_vct->no_ruang->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$vw_bill_ranap_detail_konsul_vct_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_bill_ranap_detail_konsul_vct_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvw_bill_ranap_detail_konsul_vctedit.Init();
</script>
<?php
$vw_bill_ranap_detail_konsul_vct_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_bill_ranap_detail_konsul_vct_edit->Page_Terminate();
?>
