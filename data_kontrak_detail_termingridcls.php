...
$data_kontrak_detail_termin_grid = NULL; // Initialize page object first

class cdata_kontrak_detail_termin_grid extends cdata_kontrak_detail_termin {
	...
	// Page object name
	var $PageObjName = 'data_kontrak_detail_termin_grid';

	// Grid form hidden field names
	var $FormName = 'fdata_kontrak_detail_termingrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["data_kontrak_detail_termin"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "data_kontrak") {
			global $data_kontrak;
			$rsmaster = $data_kontrak->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("data_kontraklist.php"); // Return to master page
			} else {
				$data_kontrak->LoadListRowValues($rsmaster);
				$data_kontrak->RowType = EW_ROWTYPE_MASTER; // Master row
				$data_kontrak->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function ClearInlineMode() {
		$this->target_fisik->FormValue = ""; // Clear form value
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_terminke") && $objForm->HasValue("o_terminke") && $this->terminke->CurrentValue <> $this->terminke->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_target_fisik") && $objForm->HasValue("o_target_fisik") && $this->target_fisik->CurrentValue <> $this->target_fisik->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nilai") && $objForm->HasValue("o_nilai") && $this->nilai->CurrentValue <> $this->nilai->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_id_detail") && $objForm->HasValue("o_id_detail") && $this->id_detail->CurrentValue <> $this->id_detail->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->id_detail->setSessionValue("");
			}
	...
	function LoadFormValues() {
		...
		$this->terminke->setOldValue($objForm->GetValue("o_terminke"));
		if (!$this->target_fisik->FldIsDetailKey) {
			$this->target_fisik->setFormValue($objForm->GetValue("x_target_fisik"));
		}
		$this->target_fisik->setOldValue($objForm->GetValue("o_target_fisik"));
		...
		$this->id_detail->setOldValue($objForm->GetValue("o_id_detail"));
	}
	...
	function RenderRow() {
		...
		$this->id_detail->ViewCustomAttributes = "";
			...
			if (strval($this->target_fisik->EditValue) <> "" && is_numeric($this->target_fisik->EditValue)) {
			$this->target_fisik->EditValue = ew_FormatNumber($this->target_fisik->EditValue, -2, -1, -2, 0);
			$this->target_fisik->OldValue = $this->target_fisik->EditValue;
			}
			...
			$this->nilai->EditValue = ew_FormatNumber($this->nilai->EditValue, -2, -1, -2, 0);
			...
			if ($this->id_detail->getSessionValue() <> "") {
				$this->id_detail->CurrentValue = $this->id_detail->getSessionValue();
				$this->id_detail->OldValue = $this->id_detail->CurrentValue;
			$this->id_detail->ViewValue = $this->id_detail->CurrentValue;
			$this->id_detail->ViewCustomAttributes = "";
			} else {
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "data_kontrak") {
				$this->id_detail->CurrentValue = $this->id_detail->getSessionValue();
			}
		$conn = &$this->Connection();
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "data_kontrak") {
			$this->id_detail->Visible = FALSE;
			if ($GLOBALS["data_kontrak"]->EventCancelled) $this->EventCancelled = TRUE;
		}
