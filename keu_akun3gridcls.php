...
$keu_akun3_grid = NULL; // Initialize page object first

class ckeu_akun3_grid extends ckeu_akun3 {
	...
	// Page object name
	var $PageObjName = 'keu_akun3_grid';

	// Grid form hidden field names
	var $FormName = 'fkeu_akun3grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["keu_akun3"];

		}
		$this->AddUrl = "keu_akun3add.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_kd_akun") && $objForm->HasValue("o_kd_akun") && $this->kd_akun->CurrentValue <> $this->kd_akun->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_kel3") && $objForm->HasValue("o_kel3") && $this->kel3->CurrentValue <> $this->kel3->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nmkel3") && $objForm->HasValue("o_nmkel3") && $this->nmkel3->CurrentValue <> $this->nmkel3->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->kel3->setOldValue($objForm->GetValue("o_kel3"));
		if (!$this->nmkel3->FldIsDetailKey) {
			$this->nmkel3->setFormValue($objForm->GetValue("x_nmkel3"));
		}
		$this->nmkel3->setOldValue($objForm->GetValue("o_nmkel3"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "keu_akun2") {
			...
			if ($GLOBALS["keu_akun2"]->EventCancelled) $this->EventCancelled = TRUE;
