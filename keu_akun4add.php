...
$keu_akun4_add = NULL; // Initialize page object first

class ckeu_akun4_add extends ckeu_akun4 {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "keu_akun4view.php")
	...
	function LoadDefaultValues() {
		...
		$this->kode_akun3->CurrentValue = NULL;
		$this->kode_akun3->OldValue = $this->kode_akun3->CurrentValue;
	}
	...
	function ValidateForm() {
		...
		if (in_array("keu_akun5", $DetailTblVar) && $GLOBALS["keu_akun5"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// kode_akun3
		$this->kode_akun3->SetDbValueDef($rsnew, $this->kode_akun3->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("keu_akun5", $DetailTblVar) && $GLOBALS["keu_akun5"]->DetailAdd) {
				$GLOBALS["keu_akun5"]->akun1->setSessionValue($this->kel1->CurrentValue); // Set master key
				$GLOBALS["keu_akun5"]->akun2->setSessionValue($this->kel2->CurrentValue); // Set master key
				$GLOBALS["keu_akun5"]->akun3->setSessionValue($this->kel3->CurrentValue); // Set master key
				$GLOBALS["keu_akun5"]->akun4->setSessionValue($this->kel4->CurrentValue); // Set master key
				if (!isset($GLOBALS["keu_akun5_grid"])) $GLOBALS["keu_akun5_grid"] = new ckeu_akun5_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "keu_akun5"); // Load user level of detail table
				$AddRow = $GLOBALS["keu_akun5_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["keu_akun5"]->akun4->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("keu_akun5", $DetailTblVar)) {
				...
				if ($GLOBALS["keu_akun5_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["keu_akun5_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["keu_akun5_grid"]->CurrentMode = "add";
					$GLOBALS["keu_akun5_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($keu_akun4_add)) $keu_akun4_add = new ckeu_akun4_add();

// Page init
$keu_akun4_add->Page_Init();

// Page main
$keu_akun4_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$keu_akun4_add->Page_Render();
?>
...
var CurrentForm = fkeu_akun4add = new ew_Form("fkeu_akun4add", "add");

// Validate form
fkeu_akun4add.Validate = function() {
...
fkeu_akun4add.Form_CustomValidate = 
...
fkeu_akun4add.ValidateRequired = true;
<?php } else { ?>
fkeu_akun4add.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$keu_akun4_add->IsModal) { ?>
<?php } ?>
<?php $keu_akun4_add->ShowPageHeader(); ?>
<?php
$keu_akun4_add->ShowMessage();
?>
<form name="fkeu_akun4add" id="fkeu_akun4add" class="<?php echo $keu_akun4_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($keu_akun4_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $keu_akun4_add->Token ?>">
<?php } ?>
...
<?php
	if (in_array("keu_akun5", explode(",", $keu_akun4->getCurrentDetailTable())) && $keu_akun5->DetailAdd) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $keu_akun4_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
