...
$vw_bill_ranap_detail_visite_gizi_edit = NULL; // Initialize page object first

class cvw_bill_ranap_detail_visite_gizi_edit extends cvw_bill_ranap_detail_visite_gizi {
	...
	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_visite_gizi_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("vw_bill_ranap_detail_visite_gizilist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// kode_farmasi
			$this->kode_farmasi->SetDbValueDef($rsnew, $this->kode_farmasi->CurrentValue, NULL, $this->kode_farmasi->ReadOnly);
...
// Create page object
if (!isset($vw_bill_ranap_detail_visite_gizi_edit)) $vw_bill_ranap_detail_visite_gizi_edit = new cvw_bill_ranap_detail_visite_gizi_edit();

// Page init
$vw_bill_ranap_detail_visite_gizi_edit->Page_Init();

// Page main
$vw_bill_ranap_detail_visite_gizi_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_bill_ranap_detail_visite_gizi_edit->Page_Render();
?>
...
var CurrentForm = fvw_bill_ranap_detail_visite_giziedit = new ew_Form("fvw_bill_ranap_detail_visite_giziedit", "edit");

// Validate form
fvw_bill_ranap_detail_visite_giziedit.Validate = function() {
...
fvw_bill_ranap_detail_visite_giziedit.Form_CustomValidate = 
...
fvw_bill_ranap_detail_visite_giziedit.ValidateRequired = true;
<?php } else { ?>
fvw_bill_ranap_detail_visite_giziedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_bill_ranap_detail_visite_giziedit.Lists["x_kode_tindakan"] = {"LinkField":"x_kode","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama_tindakan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_bill_ranap_data_tarif_tindakan"};
...
<?php if (!$vw_bill_ranap_detail_visite_gizi_edit->IsModal) { ?>
<?php } ?>
<?php $vw_bill_ranap_detail_visite_gizi_edit->ShowPageHeader(); ?>
<?php
$vw_bill_ranap_detail_visite_gizi_edit->ShowMessage();
?>
<form name="fvw_bill_ranap_detail_visite_giziedit" id="fvw_bill_ranap_detail_visite_giziedit" class="<?php echo $vw_bill_ranap_detail_visite_gizi_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_bill_ranap_detail_visite_gizi_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_bill_ranap_detail_visite_gizi_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<div>
<?php if ($vw_bill_ranap_detail_visite_gizi->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_vw_bill_ranap_detail_visite_gizi_id" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_visite_gizi->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_visite_gizi->id->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_visite_gizi_id">
<span<?php echo $vw_bill_ranap_detail_visite_gizi->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_visite_gizi->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_visite_gizi" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_visite_gizi->id->CurrentValue) ?>">
<?php echo $vw_bill_ranap_detail_visite_gizi->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fvw_bill_ranap_detail_visite_giziedit", "x_tanggal", 7);
</script>
<?php } ?>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_bill_ranap_detail_visite_gizi_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvw_bill_ranap_detail_visite_giziedit.Init();
</script>
<?php
$vw_bill_ranap_detail_visite_gizi_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_bill_ranap_detail_visite_gizi_edit->Page_Terminate();
?>
