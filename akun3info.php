...
//
// Table class for akun3
//
class cakun3 extends cTable {
	var $kel1;
	var $kel2;
	var $kel3;
	var $kel4;
	...
	function __construct() {
		...
		$this->TableVar = 'akun3';
		$this->TableName = 'akun3';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`akun3`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kel1
		$this->kel1 = new cField('akun3', 'akun3', 'x_kel1', 'kel1', '`kel1`', '`kel1`', 200, -1, FALSE, '`kel1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel1->Sortable = TRUE; // Allow sort
		$this->fields['kel1'] = &$this->kel1;

		// kel2
		$this->kel2 = new cField('akun3', 'akun3', 'x_kel2', 'kel2', '`kel2`', '`kel2`', 200, -1, FALSE, '`kel2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel2->Sortable = TRUE; // Allow sort
		$this->fields['kel2'] = &$this->kel2;

		// kel3
		$this->kel3 = new cField('akun3', 'akun3', 'x_kel3', 'kel3', '`kel3`', '`kel3`', 200, -1, FALSE, '`kel3`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel3->Sortable = TRUE; // Allow sort
		$this->fields['kel3'] = &$this->kel3;

		// kel4
		$this->kel4 = new cField('akun3', 'akun3', 'x_kel4', 'kel4', '`kel4`', '`kel4`', 200, -1, FALSE, '`kel4`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel4->Sortable = TRUE; // Allow sort
		$this->fields['kel4'] = &$this->kel4;

		// nmkel3
		$this->nmkel3 = new cField('akun3', 'akun3', 'x_nmkel3', 'nmkel3', '`nmkel3`', '`nmkel3`', 200, -1, FALSE, '`nmkel3`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`akun3`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kel4', $rs))
				ew_AddFilter($where, ew_QuotedName('kel4', $this->DBID) . '=' . ew_QuotedValue($rs['kel4'], $this->kel4->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@kel4@", ew_AdjustSql($this->kel4->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("akun3view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("akun3view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "akun3add.php?" . $this->UrlParm($parm);
		else
			$url = "akun3add.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("akun3edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("akun3add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("akun3delete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kel4:" . ew_VarToJson($this->kel4->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kel4->CurrentValue)) {
			$sUrl .= "kel4=" . urlencode($this->kel4->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["kel4"]))
				$arKeys[] = ew_StripSlashes($_POST["kel4"]);
			elseif (isset($_GET["kel4"]))
				$arKeys[] = ew_StripSlashes($_GET["kel4"]);
			else
