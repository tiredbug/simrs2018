...
$pd_notifications_add = NULL; // Initialize page object first

class cpd_notifications_add extends cpd_notifications {
	...
	// Page object name
	var $PageObjName = 'pd_notifications_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["n_id"] != "") {
				...
				$this->setKey("n_id", $this->n_id->CurrentValue); // Set up key
			} else {
				$this->setKey("n_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "pd_notificationsview.php")
	...
	function LoadDefaultValues() {
		$this->n_datetime->CurrentValue = NULL;
		$this->n_datetime->OldValue = $this->n_datetime->CurrentValue;
		$this->n_content->CurrentValue = NULL;
		$this->n_content->OldValue = $this->n_content->CurrentValue;
		$this->n_targetlevel->CurrentValue = NULL;
		$this->n_targetlevel->OldValue = $this->n_targetlevel->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// n_datetime
		$this->n_datetime->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->n_datetime->CurrentValue, 0), NULL, FALSE);

		// n_content
		$this->n_content->SetDbValueDef($rsnew, $this->n_content->CurrentValue, NULL, FALSE);

		// n_targetlevel
		$this->n_targetlevel->SetDbValueDef($rsnew, $this->n_targetlevel->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($pd_notifications_add)) $pd_notifications_add = new cpd_notifications_add();

// Page init
$pd_notifications_add->Page_Init();

// Page main
$pd_notifications_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pd_notifications_add->Page_Render();
?>
...
var CurrentForm = fpd_notificationsadd = new ew_Form("fpd_notificationsadd", "add");

// Validate form
fpd_notificationsadd.Validate = function() {
...
fpd_notificationsadd.Form_CustomValidate = 
...
fpd_notificationsadd.ValidateRequired = true;
<?php } else { ?>
fpd_notificationsadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$pd_notifications_add->IsModal) { ?>
<?php } ?>
<?php $pd_notifications_add->ShowPageHeader(); ?>
<?php
$pd_notifications_add->ShowMessage();
?>
<form name="fpd_notificationsadd" id="fpd_notificationsadd" class="<?php echo $pd_notifications_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pd_notifications_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pd_notifications_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $pd_notifications_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fpd_notificationsadd.Init();
</script>
<?php
$pd_notifications_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$pd_notifications_add->Page_Terminate();
?>
