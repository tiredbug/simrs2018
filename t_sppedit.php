...
$t_spp_edit = NULL; // Initialize page object first

class ct_spp_edit extends ct_spp {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("t_spplist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// tgl_spm
			$this->tgl_spm->SetDbValueDef($rsnew, $this->tgl_spm->CurrentValue, NULL, $this->tgl_spm->ReadOnly);
			...
			// opd
			$this->opd->SetDbValueDef($rsnew, $this->opd->CurrentValue, NULL, $this->opd->ReadOnly);

			// urusan_pemerintahan
			$this->urusan_pemerintahan->SetDbValueDef($rsnew, $this->urusan_pemerintahan->CurrentValue, NULL, $this->urusan_pemerintahan->ReadOnly);

			// tgl_sptb
			$this->tgl_sptb->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tgl_sptb->CurrentValue, 0), NULL, $this->tgl_sptb->ReadOnly);

			// no_sptb
			$this->no_sptb->SetDbValueDef($rsnew, $this->no_sptb->CurrentValue, NULL, $this->no_sptb->ReadOnly);

			// status_advis
			$this->status_advis->SetDbValueDef($rsnew, $this->status_advis->CurrentValue, NULL, $this->status_advis->ReadOnly);
...
// Create page object
if (!isset($t_spp_edit)) $t_spp_edit = new ct_spp_edit();

// Page init
$t_spp_edit->Page_Init();

// Page main
$t_spp_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_spp_edit->Page_Render();
?>
...
var CurrentForm = ft_sppedit = new ew_Form("ft_sppedit", "edit");

// Validate form
ft_sppedit.Validate = function() {
...
ft_sppedit.Form_CustomValidate = 
...
ft_sppedit.ValidateRequired = true;
<?php } else { ?>
ft_sppedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_sppedit.Lists["x_id_jenis_spp"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_jenis_spp","","",""],"ParentFields":[],"ChildFields":["x_detail_jenis_spp"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"l_jenis_spp"};
ft_sppedit.Lists["x_detail_jenis_spp"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_detail_jenis_spp","","",""],"ParentFields":["x_id_jenis_spp"],"ChildFields":[],"FilterFields":["x_id_jenis"],"Options":[],"Template":"","LinkTable":"l_jenis_detail_spp"};
ft_sppedit.Lists["x_status_spp"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_sppedit.Lists["x_status_spp"].Options = <?php echo json_encode($t_spp->status_spp->Options()) ?>;
...
<?php if (!$t_spp_edit->IsModal) { ?>
<?php } ?>
<?php $t_spp_edit->ShowPageHeader(); ?>
<?php
$t_spp_edit->ShowMessage();
?>
<form name="ft_sppedit" id="ft_sppedit" class="<?php echo $t_spp_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_spp_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_spp_edit->Token ?>">
<?php } ?>
...
<?php if ($t_spp->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_t_spp_id" class="col-sm-2 control-label ewLabel"><?php echo $t_spp->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $t_spp->id->CellAttributes() ?>>
<span id="el_t_spp_id">
<span<?php echo $t_spp->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $t_spp->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="t_spp" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($t_spp->id->CurrentValue) ?>">
<?php echo $t_spp->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("ft_sppedit", "x_tgl_spp", 7);
</script>
<?php } ?>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_spp_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
