...
$m_kegiatan_delete = NULL; // Initialize page object first

class cm_kegiatan_delete extends cm_kegiatan {
	...
	// Page object name
	var $PageObjName = 'm_kegiatan_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("m_kegiatanlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in m_kegiatan class, m_kegiataninfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("m_kegiatanlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($m_kegiatan_delete)) $m_kegiatan_delete = new cm_kegiatan_delete();

// Page init
$m_kegiatan_delete->Page_Init();

// Page main
$m_kegiatan_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$m_kegiatan_delete->Page_Render();
?>
...
var CurrentForm = fm_kegiatandelete = new ew_Form("fm_kegiatandelete", "delete");

// Form_CustomValidate event
fm_kegiatandelete.Form_CustomValidate = 
...
fm_kegiatandelete.ValidateRequired = true;
<?php } else { ?>
fm_kegiatandelete.ValidateRequired = false; 
<?php } ?>
...
<?php $m_kegiatan_delete->ShowPageHeader(); ?>
<?php
$m_kegiatan_delete->ShowMessage();
?>
<form name="fm_kegiatandelete" id="fm_kegiatandelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($m_kegiatan_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $m_kegiatan_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="m_kegiatan">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($m_kegiatan_delete->RecKeys as $key) { ?>
...
<?php if ($m_kegiatan->id->Visible) { // id ?>
		<th><span id="elh_m_kegiatan_id" class="m_kegiatan_id"><?php echo $m_kegiatan->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($m_kegiatan->kode_program->Visible) { // kode_program ?>
		<th><span id="elh_m_kegiatan_kode_program" class="m_kegiatan_kode_program"><?php echo $m_kegiatan->kode_program->FldCaption() ?></span></th>
<?php } ?>
<?php if ($m_kegiatan->kode_kegiatan->Visible) { // kode_kegiatan ?>
		<th><span id="elh_m_kegiatan_kode_kegiatan" class="m_kegiatan_kode_kegiatan"><?php echo $m_kegiatan->kode_kegiatan->FldCaption() ?></span></th>
<?php } ?>
<?php if ($m_kegiatan->nama_kegiatan->Visible) { // nama_kegiatan ?>
		<th><span id="elh_m_kegiatan_nama_kegiatan" class="m_kegiatan_nama_kegiatan"><?php echo $m_kegiatan->nama_kegiatan->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$m_kegiatan_delete->RecCnt = 0;
$i = 0;
while (!$m_kegiatan_delete->Recordset->EOF) {
	$m_kegiatan_delete->RecCnt++;
	$m_kegiatan_delete->RowCnt++;

	// Set row properties
	$m_kegiatan->ResetAttrs();
	$m_kegiatan->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$m_kegiatan_delete->LoadRowValues($m_kegiatan_delete->Recordset);

	// Render row
	$m_kegiatan_delete->RenderRow();
?>
	<tr<?php echo $m_kegiatan->RowAttributes() ?>>
<?php if ($m_kegiatan->id->Visible) { // id ?>
		<td<?php echo $m_kegiatan->id->CellAttributes() ?>>
<span id="el<?php echo $m_kegiatan_delete->RowCnt ?>_m_kegiatan_id" class="m_kegiatan_id">
...
<span id="el<?php echo $m_kegiatan_delete->RowCnt ?>_m_kegiatan_kode_program" class="m_kegiatan_kode_program">
...
<span id="el<?php echo $m_kegiatan_delete->RowCnt ?>_m_kegiatan_kode_kegiatan" class="m_kegiatan_kode_kegiatan">
...
<span id="el<?php echo $m_kegiatan_delete->RowCnt ?>_m_kegiatan_nama_kegiatan" class="m_kegiatan_nama_kegiatan">
...
<?php
	$m_kegiatan_delete->Recordset->MoveNext();
}
$m_kegiatan_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $m_kegiatan_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fm_kegiatandelete.Init();
</script>
<?php
$m_kegiatan_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$m_kegiatan_delete->Page_Terminate();
?>
