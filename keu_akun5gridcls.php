...
$keu_akun5_grid = NULL; // Initialize page object first

class ckeu_akun5_grid extends ckeu_akun5 {
	...
	// Page object name
	var $PageObjName = 'keu_akun5_grid';

	// Grid form hidden field names
	var $FormName = 'fkeu_akun5grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["keu_akun5"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_kd_akun") && $objForm->HasValue("o_kd_akun") && $this->kd_akun->CurrentValue <> $this->kd_akun->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_akun5") && $objForm->HasValue("o_akun5") && $this->akun5->CurrentValue <> $this->akun5->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nama_akun") && $objForm->HasValue("o_nama_akun") && $this->nama_akun->CurrentValue <> $this->nama_akun->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->akun1->getSessionValue() <> "") {
			$rsnew['akun1'] = $this->akun1->getSessionValue();
		}

		// akun2
		if ($this->akun2->getSessionValue() <> "") {
			$rsnew['akun2'] = $this->akun2->getSessionValue();
		}

		// akun3
		if ($this->akun3->getSessionValue() <> "") {
			$rsnew['akun3'] = $this->akun3->getSessionValue();
		}

		// akun4
		if ($this->akun4->getSessionValue() <> "") {
			$rsnew['akun4'] = $this->akun4->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "keu_akun4") {
			$this->akun1->Visible = FALSE;
			if ($GLOBALS["keu_akun4"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->akun2->Visible = FALSE;
			if ($GLOBALS["keu_akun4"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->akun3->Visible = FALSE;
			if ($GLOBALS["keu_akun4"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->akun4->Visible = FALSE;
			if ($GLOBALS["keu_akun4"]->EventCancelled) $this->EventCancelled = TRUE;
		}
