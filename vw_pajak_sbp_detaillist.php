...
$vw_pajak_sbp_detail_list = NULL; // Initialize page object first

class cvw_pajak_sbp_detail_list extends cvw_pajak_sbp_detail {
	...
	// Page object name
	var $PageObjName = 'vw_pajak_sbp_detail_list';

	// Grid form hidden field names
	var $FormName = 'fvw_pajak_sbp_detaillist';
	...
	function __construct() {
		...
		$this->AddUrl = "vw_pajak_sbp_detailadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "vw_pajak_sbp_detaildelete.php";
		$this->MultiUpdateUrl = "vw_pajak_sbp_detailupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fvw_pajak_sbp_detaillistsrch";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fvw_pajak_sbp_detaillistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fvw_pajak_sbp_detaillistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fvw_pajak_sbp_detaillist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
// Create page object
if (!isset($vw_pajak_sbp_detail_list)) $vw_pajak_sbp_detail_list = new cvw_pajak_sbp_detail_list();

// Page init
$vw_pajak_sbp_detail_list->Page_Init();

// Page main
$vw_pajak_sbp_detail_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_pajak_sbp_detail_list->Page_Render();
?>
...
var CurrentForm = fvw_pajak_sbp_detaillist = new ew_Form("fvw_pajak_sbp_detaillist", "list");
fvw_pajak_sbp_detaillist.FormKeyCountName = '<?php echo $vw_pajak_sbp_detail_list->FormKeyCountName ?>';

// Form_CustomValidate event
fvw_pajak_sbp_detaillist.Form_CustomValidate = 
...
fvw_pajak_sbp_detaillist.ValidateRequired = true;
<?php } else { ?>
fvw_pajak_sbp_detaillist.ValidateRequired = false; 
<?php } ?>
...
<?php if ($vw_pajak_sbp_detail_list->TotalRecs > 0 && $vw_pajak_sbp_detail_list->ExportOptions->Visible()) { ?>
<?php $vw_pajak_sbp_detail_list->ExportOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php if (($vw_pajak_sbp_detail->Export == "") || (EW_EXPORT_MASTER_RECORD && $vw_pajak_sbp_detail->Export == "print")) { ?>
<?php
if ($vw_pajak_sbp_detail_list->DbMasterFilter <> "" && $vw_pajak_sbp_detail->getCurrentMasterTable() == "t_sbp") {
	if ($vw_pajak_sbp_detail_list->MasterRecordExists) {
?>
...
<?php } ?>
<?php
	$bSelectLimit = $vw_pajak_sbp_detail_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($vw_pajak_sbp_detail_list->TotalRecs <= 0)
			$vw_pajak_sbp_detail_list->TotalRecs = $vw_pajak_sbp_detail->SelectRecordCount();
	} else {
		if (!$vw_pajak_sbp_detail_list->Recordset && ($vw_pajak_sbp_detail_list->Recordset = $vw_pajak_sbp_detail_list->LoadRecordset()))
			$vw_pajak_sbp_detail_list->TotalRecs = $vw_pajak_sbp_detail_list->Recordset->RecordCount();
	}
	$vw_pajak_sbp_detail_list->StartRec = 1;
	if ($vw_pajak_sbp_detail_list->DisplayRecs <= 0 || ($vw_pajak_sbp_detail->Export <> "" && $vw_pajak_sbp_detail->ExportAll)) // Display all records
		$vw_pajak_sbp_detail_list->DisplayRecs = $vw_pajak_sbp_detail_list->TotalRecs;
	if (!($vw_pajak_sbp_detail->Export <> "" && $vw_pajak_sbp_detail->ExportAll))
		$vw_pajak_sbp_detail_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$vw_pajak_sbp_detail_list->Recordset = $vw_pajak_sbp_detail_list->LoadRecordset($vw_pajak_sbp_detail_list->StartRec-1, $vw_pajak_sbp_detail_list->DisplayRecs);

	// Set no record found message
	if ($vw_pajak_sbp_detail->CurrentAction == "" && $vw_pajak_sbp_detail_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$vw_pajak_sbp_detail_list->setWarningMessage(ew_DeniedMsg());
		if ($vw_pajak_sbp_detail_list->SearchWhere == "0=101")
			$vw_pajak_sbp_detail_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$vw_pajak_sbp_detail_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$vw_pajak_sbp_detail_list->RenderOtherOptions();
?>
<?php $vw_pajak_sbp_detail_list->ShowPageHeader(); ?>
<?php
$vw_pajak_sbp_detail_list->ShowMessage();
?>
<?php if ($vw_pajak_sbp_detail_list->TotalRecs > 0 || $vw_pajak_sbp_detail->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid vw_pajak_sbp_detail">
<form name="fvw_pajak_sbp_detaillist" id="fvw_pajak_sbp_detaillist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_pajak_sbp_detail_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_pajak_sbp_detail_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_pajak_sbp_detail">
<?php if ($vw_pajak_sbp_detail->getCurrentMasterTable() == "t_sbp" && $vw_pajak_sbp_detail->CurrentAction <> "") { ?>
...
<?php if ($vw_pajak_sbp_detail_list->TotalRecs > 0 || $vw_pajak_sbp_detail->CurrentAction == "gridedit") { ?>
<table id="tbl_vw_pajak_sbp_detaillist" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$vw_pajak_sbp_detail_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$vw_pajak_sbp_detail_list->RenderListOptions();

// Render list options (header, left)
$vw_pajak_sbp_detail_list->ListOptions->Render("header", "left");
?>
<?php if ($vw_pajak_sbp_detail->jumlah_belanja->Visible) { // jumlah_belanja ?>
	...
	<?php } else { ?>
		<th data-name="jumlah_belanja"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pajak_sbp_detail->SortUrl($vw_pajak_sbp_detail->jumlah_belanja) ?>',2);"><div id="elh_vw_pajak_sbp_detail_jumlah_belanja" class="vw_pajak_sbp_detail_jumlah_belanja">
...
<?php if ($vw_pajak_sbp_detail->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
	...
	<?php } else { ?>
		<th data-name="kd_rekening_belanja"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pajak_sbp_detail->SortUrl($vw_pajak_sbp_detail->kd_rekening_belanja) ?>',2);"><div id="elh_vw_pajak_sbp_detail_kd_rekening_belanja" class="vw_pajak_sbp_detail_kd_rekening_belanja">
...
$vw_pajak_sbp_detail_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($vw_pajak_sbp_detail->ExportAll && $vw_pajak_sbp_detail->Export <> "") {
	$vw_pajak_sbp_detail_list->StopRec = $vw_pajak_sbp_detail_list->TotalRecs;
} else {

	// Set the last record to display
	if ($vw_pajak_sbp_detail_list->TotalRecs > $vw_pajak_sbp_detail_list->StartRec + $vw_pajak_sbp_detail_list->DisplayRecs - 1)
		$vw_pajak_sbp_detail_list->StopRec = $vw_pajak_sbp_detail_list->StartRec + $vw_pajak_sbp_detail_list->DisplayRecs - 1;
	else
		$vw_pajak_sbp_detail_list->StopRec = $vw_pajak_sbp_detail_list->TotalRecs;
}
$vw_pajak_sbp_detail_list->RecCnt = $vw_pajak_sbp_detail_list->StartRec - 1;
if ($vw_pajak_sbp_detail_list->Recordset && !$vw_pajak_sbp_detail_list->Recordset->EOF) {
	$vw_pajak_sbp_detail_list->Recordset->MoveFirst();
	$bSelectLimit = $vw_pajak_sbp_detail_list->UseSelectLimit;
	if (!$bSelectLimit && $vw_pajak_sbp_detail_list->StartRec > 1)
		$vw_pajak_sbp_detail_list->Recordset->Move($vw_pajak_sbp_detail_list->StartRec - 1);
} elseif (!$vw_pajak_sbp_detail->AllowAddDeleteRow && $vw_pajak_sbp_detail_list->StopRec == 0) {
	$vw_pajak_sbp_detail_list->StopRec = $vw_pajak_sbp_detail->GridAddRowCount;
}

// Initialize aggregate
$vw_pajak_sbp_detail->RowType = EW_ROWTYPE_AGGREGATEINIT;
$vw_pajak_sbp_detail->ResetAttrs();
$vw_pajak_sbp_detail_list->RenderRow();
while ($vw_pajak_sbp_detail_list->RecCnt < $vw_pajak_sbp_detail_list->StopRec) {
	$vw_pajak_sbp_detail_list->RecCnt++;
	if (intval($vw_pajak_sbp_detail_list->RecCnt) >= intval($vw_pajak_sbp_detail_list->StartRec)) {
		$vw_pajak_sbp_detail_list->RowCnt++;

		// Set up key count
		$vw_pajak_sbp_detail_list->KeyCount = $vw_pajak_sbp_detail_list->RowIndex;
		...
		if ($vw_pajak_sbp_detail->CurrentAction == "gridadd") {
			...
			$vw_pajak_sbp_detail_list->LoadRowValues($vw_pajak_sbp_detail_list->Recordset); // Load row values
		}
		$vw_pajak_sbp_detail->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$vw_pajak_sbp_detail->RowAttrs = array_merge($vw_pajak_sbp_detail->RowAttrs, array('data-rowindex'=>$vw_pajak_sbp_detail_list->RowCnt, 'id'=>'r' . $vw_pajak_sbp_detail_list->RowCnt . '_vw_pajak_sbp_detail', 'data-rowtype'=>$vw_pajak_sbp_detail->RowType));
...
$vw_pajak_sbp_detail_list->ListOptions->Render("body", "left", $vw_pajak_sbp_detail_list->RowCnt);
?>
	<?php if ($vw_pajak_sbp_detail->jumlah_belanja->Visible) { // jumlah_belanja ?>
		<td data-name="jumlah_belanja"<?php echo $vw_pajak_sbp_detail->jumlah_belanja->CellAttributes() ?>>
<span id="el<?php echo $vw_pajak_sbp_detail_list->RowCnt ?>_vw_pajak_sbp_detail_jumlah_belanja" class="vw_pajak_sbp_detail_jumlah_belanja">
...
<a id="<?php echo $vw_pajak_sbp_detail_list->PageObjName . "_row_" . $vw_pajak_sbp_detail_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($vw_pajak_sbp_detail->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
		<td data-name="kd_rekening_belanja"<?php echo $vw_pajak_sbp_detail->kd_rekening_belanja->CellAttributes() ?>>
<span id="el<?php echo $vw_pajak_sbp_detail_list->RowCnt ?>_vw_pajak_sbp_detail_kd_rekening_belanja" class="vw_pajak_sbp_detail_kd_rekening_belanja">
...
$vw_pajak_sbp_detail_list->ListOptions->Render("body", "right", $vw_pajak_sbp_detail_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($vw_pajak_sbp_detail->CurrentAction <> "gridadd")
		$vw_pajak_sbp_detail_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($vw_pajak_sbp_detail->CurrentAction == "") { ?>
...
// Close recordset
if ($vw_pajak_sbp_detail_list->Recordset)
	$vw_pajak_sbp_detail_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($vw_pajak_sbp_detail->CurrentAction <> "gridadd" && $vw_pajak_sbp_detail->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($vw_pajak_sbp_detail_list->Pager)) $vw_pajak_sbp_detail_list->Pager = new cPrevNextPager($vw_pajak_sbp_detail_list->StartRec, $vw_pajak_sbp_detail_list->DisplayRecs, $vw_pajak_sbp_detail_list->TotalRecs) ?>
<?php if ($vw_pajak_sbp_detail_list->Pager->RecordCount > 0 && $vw_pajak_sbp_detail_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($vw_pajak_sbp_detail_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $vw_pajak_sbp_detail_list->PageUrl() ?>start=<?php echo $vw_pajak_sbp_detail_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($vw_pajak_sbp_detail_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $vw_pajak_sbp_detail_list->PageUrl() ?>start=<?php echo $vw_pajak_sbp_detail_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $vw_pajak_sbp_detail_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($vw_pajak_sbp_detail_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $vw_pajak_sbp_detail_list->PageUrl() ?>start=<?php echo $vw_pajak_sbp_detail_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($vw_pajak_sbp_detail_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $vw_pajak_sbp_detail_list->PageUrl() ?>start=<?php echo $vw_pajak_sbp_detail_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $vw_pajak_sbp_detail_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $vw_pajak_sbp_detail_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $vw_pajak_sbp_detail_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $vw_pajak_sbp_detail_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($vw_pajak_sbp_detail_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $vw_pajak_sbp_detail_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="vw_pajak_sbp_detail">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($vw_pajak_sbp_detail_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($vw_pajak_sbp_detail_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($vw_pajak_sbp_detail_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($vw_pajak_sbp_detail_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($vw_pajak_sbp_detail_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($vw_pajak_sbp_detail_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($vw_pajak_sbp_detail_list->TotalRecs == 0 && $vw_pajak_sbp_detail->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($vw_pajak_sbp_detail_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fvw_pajak_sbp_detaillist.Init();
</script>
<?php
$vw_pajak_sbp_detail_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_pajak_sbp_detail_list->Page_Terminate();
?>
