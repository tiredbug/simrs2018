...
$vw_spp_ls_gaji_list_detail_belanja_list = NULL; // Initialize page object first

class cvw_spp_ls_gaji_list_detail_belanja_list extends cvw_spp_ls_gaji_list_detail_belanja {
	...
	// Page object name
	var $PageObjName = 'vw_spp_ls_gaji_list_detail_belanja_list';

	// Grid form hidden field names
	var $FormName = 'fvw_spp_ls_gaji_list_detail_belanjalist';
	...
	function __construct() {
		...
		$this->AddUrl = "vw_spp_ls_gaji_list_detail_belanjaadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "vw_spp_ls_gaji_list_detail_belanjadelete.php";
		$this->MultiUpdateUrl = "vw_spp_ls_gaji_list_detail_belanjaupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fvw_spp_ls_gaji_list_detail_belanjalistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fvw_spp_ls_gaji_list_detail_belanjalistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fvw_spp_ls_gaji_list_detail_belanjalistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fvw_spp_ls_gaji_list_detail_belanjalistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fvw_spp_ls_gaji_list_detail_belanjalistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fvw_spp_ls_gaji_list_detail_belanjalist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fvw_spp_ls_gaji_list_detail_belanjalistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($vw_spp_ls_gaji_list_detail_belanja_list)) $vw_spp_ls_gaji_list_detail_belanja_list = new cvw_spp_ls_gaji_list_detail_belanja_list();

// Page init
$vw_spp_ls_gaji_list_detail_belanja_list->Page_Init();

// Page main
$vw_spp_ls_gaji_list_detail_belanja_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_spp_ls_gaji_list_detail_belanja_list->Page_Render();
?>
...
var CurrentForm = fvw_spp_ls_gaji_list_detail_belanjalist = new ew_Form("fvw_spp_ls_gaji_list_detail_belanjalist", "list");
fvw_spp_ls_gaji_list_detail_belanjalist.FormKeyCountName = '<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->FormKeyCountName ?>';

// Form_CustomValidate event
fvw_spp_ls_gaji_list_detail_belanjalist.Form_CustomValidate = 
...
fvw_spp_ls_gaji_list_detail_belanjalist.ValidateRequired = true;
<?php } else { ?>
fvw_spp_ls_gaji_list_detail_belanjalist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var CurrentSearchForm = fvw_spp_ls_gaji_list_detail_belanjalistsrch = new ew_Form("fvw_spp_ls_gaji_list_detail_belanjalistsrch");
</script>
<script type="text/javascript">

// Write your client script here, no need to add script tags.
</script>
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs > 0 && $vw_spp_ls_gaji_list_detail_belanja_list->ExportOptions->Visible()) { ?>
<?php $vw_spp_ls_gaji_list_detail_belanja_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->SearchOptions->Visible()) { ?>
<?php $vw_spp_ls_gaji_list_detail_belanja_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->FilterOptions->Visible()) { ?>
<?php $vw_spp_ls_gaji_list_detail_belanja_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php if (($vw_spp_ls_gaji_list_detail_belanja->Export == "") || (EW_EXPORT_MASTER_RECORD && $vw_spp_ls_gaji_list_detail_belanja->Export == "print")) { ?>
<?php
if ($vw_spp_ls_gaji_list_detail_belanja_list->DbMasterFilter <> "" && $vw_spp_ls_gaji_list_detail_belanja->getCurrentMasterTable() == "vw_spp_ls_gaji_list") {
	if ($vw_spp_ls_gaji_list_detail_belanja_list->MasterRecordExists) {
?>
...
<?php } ?>
<?php
	$bSelectLimit = $vw_spp_ls_gaji_list_detail_belanja_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs <= 0)
			$vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs = $vw_spp_ls_gaji_list_detail_belanja->SelectRecordCount();
	} else {
		if (!$vw_spp_ls_gaji_list_detail_belanja_list->Recordset && ($vw_spp_ls_gaji_list_detail_belanja_list->Recordset = $vw_spp_ls_gaji_list_detail_belanja_list->LoadRecordset()))
			$vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs = $vw_spp_ls_gaji_list_detail_belanja_list->Recordset->RecordCount();
	}
	$vw_spp_ls_gaji_list_detail_belanja_list->StartRec = 1;
	if ($vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs <= 0 || ($vw_spp_ls_gaji_list_detail_belanja->Export <> "" && $vw_spp_ls_gaji_list_detail_belanja->ExportAll)) // Display all records
		$vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs = $vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs;
	if (!($vw_spp_ls_gaji_list_detail_belanja->Export <> "" && $vw_spp_ls_gaji_list_detail_belanja->ExportAll))
		$vw_spp_ls_gaji_list_detail_belanja_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$vw_spp_ls_gaji_list_detail_belanja_list->Recordset = $vw_spp_ls_gaji_list_detail_belanja_list->LoadRecordset($vw_spp_ls_gaji_list_detail_belanja_list->StartRec-1, $vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs);

	// Set no record found message
	if ($vw_spp_ls_gaji_list_detail_belanja->CurrentAction == "" && $vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$vw_spp_ls_gaji_list_detail_belanja_list->setWarningMessage(ew_DeniedMsg());
		if ($vw_spp_ls_gaji_list_detail_belanja_list->SearchWhere == "0=101")
			$vw_spp_ls_gaji_list_detail_belanja_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$vw_spp_ls_gaji_list_detail_belanja_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$vw_spp_ls_gaji_list_detail_belanja_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja->Export == "" && $vw_spp_ls_gaji_list_detail_belanja->CurrentAction == "") { ?>
<form name="fvw_spp_ls_gaji_list_detail_belanjalistsrch" id="fvw_spp_ls_gaji_list_detail_belanjalistsrch" class="form-horizontal ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($vw_spp_ls_gaji_list_detail_belanja_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fvw_spp_ls_gaji_list_detail_belanjalistsrch_SearchPanel" class="box box-default collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="vw_spp_ls_gaji_list_detail_belanja">
<div class="box-body">
<div id="xsr_1">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($vw_spp_ls_gaji_list_detail_belanja_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($vw_spp_ls_gaji_list_detail_belanja_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $vw_spp_ls_gaji_list_detail_belanja_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $vw_spp_ls_gaji_list_detail_belanja_list->ShowPageHeader(); ?>
<?php
$vw_spp_ls_gaji_list_detail_belanja_list->ShowMessage();
?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs > 0 || $vw_spp_ls_gaji_list_detail_belanja->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid vw_spp_ls_gaji_list_detail_belanja">
<form name="fvw_spp_ls_gaji_list_detail_belanjalist" id="fvw_spp_ls_gaji_list_detail_belanjalist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_spp_ls_gaji_list_detail_belanja">
<?php if ($vw_spp_ls_gaji_list_detail_belanja->getCurrentMasterTable() == "vw_spp_ls_gaji_list" && $vw_spp_ls_gaji_list_detail_belanja->CurrentAction <> "") { ?>
...
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs > 0 || $vw_spp_ls_gaji_list_detail_belanja->CurrentAction == "gridedit") { ?>
<table id="tbl_vw_spp_ls_gaji_list_detail_belanjalist" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$vw_spp_ls_gaji_list_detail_belanja_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$vw_spp_ls_gaji_list_detail_belanja_list->RenderListOptions();

// Render list options (header, left)
$vw_spp_ls_gaji_list_detail_belanja_list->ListOptions->Render("header", "left");
?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
	...
	<?php } else { ?>
		<th data-name="kd_rekening_belanja"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_ls_gaji_list_detail_belanja->SortUrl($vw_spp_ls_gaji_list_detail_belanja->kd_rekening_belanja) ?>',2);"><div id="elh_vw_spp_ls_gaji_list_detail_belanja_kd_rekening_belanja" class="vw_spp_ls_gaji_list_detail_belanja_kd_rekening_belanja">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_spp_ls_gaji_list_detail_belanja->kd_rekening_belanja->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($vw_spp_ls_gaji_list_detail_belanja->kd_rekening_belanja->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_spp_ls_gaji_list_detail_belanja->kd_rekening_belanja->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_spp_ls_gaji_list_detail_belanja->jumlah_belanja->Visible) { // jumlah_belanja ?>
	...
	<?php } else { ?>
		<th data-name="jumlah_belanja"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_spp_ls_gaji_list_detail_belanja->SortUrl($vw_spp_ls_gaji_list_detail_belanja->jumlah_belanja) ?>',2);"><div id="elh_vw_spp_ls_gaji_list_detail_belanja_jumlah_belanja" class="vw_spp_ls_gaji_list_detail_belanja_jumlah_belanja">
...
$vw_spp_ls_gaji_list_detail_belanja_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($vw_spp_ls_gaji_list_detail_belanja->ExportAll && $vw_spp_ls_gaji_list_detail_belanja->Export <> "") {
	$vw_spp_ls_gaji_list_detail_belanja_list->StopRec = $vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs;
} else {

	// Set the last record to display
	if ($vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs > $vw_spp_ls_gaji_list_detail_belanja_list->StartRec + $vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs - 1)
		$vw_spp_ls_gaji_list_detail_belanja_list->StopRec = $vw_spp_ls_gaji_list_detail_belanja_list->StartRec + $vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs - 1;
	else
		$vw_spp_ls_gaji_list_detail_belanja_list->StopRec = $vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs;
}
$vw_spp_ls_gaji_list_detail_belanja_list->RecCnt = $vw_spp_ls_gaji_list_detail_belanja_list->StartRec - 1;
if ($vw_spp_ls_gaji_list_detail_belanja_list->Recordset && !$vw_spp_ls_gaji_list_detail_belanja_list->Recordset->EOF) {
	$vw_spp_ls_gaji_list_detail_belanja_list->Recordset->MoveFirst();
	$bSelectLimit = $vw_spp_ls_gaji_list_detail_belanja_list->UseSelectLimit;
	if (!$bSelectLimit && $vw_spp_ls_gaji_list_detail_belanja_list->StartRec > 1)
		$vw_spp_ls_gaji_list_detail_belanja_list->Recordset->Move($vw_spp_ls_gaji_list_detail_belanja_list->StartRec - 1);
} elseif (!$vw_spp_ls_gaji_list_detail_belanja->AllowAddDeleteRow && $vw_spp_ls_gaji_list_detail_belanja_list->StopRec == 0) {
	$vw_spp_ls_gaji_list_detail_belanja_list->StopRec = $vw_spp_ls_gaji_list_detail_belanja->GridAddRowCount;
}

// Initialize aggregate
$vw_spp_ls_gaji_list_detail_belanja->RowType = EW_ROWTYPE_AGGREGATEINIT;
$vw_spp_ls_gaji_list_detail_belanja->ResetAttrs();
$vw_spp_ls_gaji_list_detail_belanja_list->RenderRow();
while ($vw_spp_ls_gaji_list_detail_belanja_list->RecCnt < $vw_spp_ls_gaji_list_detail_belanja_list->StopRec) {
	$vw_spp_ls_gaji_list_detail_belanja_list->RecCnt++;
	if (intval($vw_spp_ls_gaji_list_detail_belanja_list->RecCnt) >= intval($vw_spp_ls_gaji_list_detail_belanja_list->StartRec)) {
		$vw_spp_ls_gaji_list_detail_belanja_list->RowCnt++;

		// Set up key count
		$vw_spp_ls_gaji_list_detail_belanja_list->KeyCount = $vw_spp_ls_gaji_list_detail_belanja_list->RowIndex;
		...
		if ($vw_spp_ls_gaji_list_detail_belanja->CurrentAction == "gridadd") {
			...
			$vw_spp_ls_gaji_list_detail_belanja_list->LoadRowValues($vw_spp_ls_gaji_list_detail_belanja_list->Recordset); // Load row values
		}
		$vw_spp_ls_gaji_list_detail_belanja->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$vw_spp_ls_gaji_list_detail_belanja->RowAttrs = array_merge($vw_spp_ls_gaji_list_detail_belanja->RowAttrs, array('data-rowindex'=>$vw_spp_ls_gaji_list_detail_belanja_list->RowCnt, 'id'=>'r' . $vw_spp_ls_gaji_list_detail_belanja_list->RowCnt . '_vw_spp_ls_gaji_list_detail_belanja', 'data-rowtype'=>$vw_spp_ls_gaji_list_detail_belanja->RowType));
...
$vw_spp_ls_gaji_list_detail_belanja_list->ListOptions->Render("body", "left", $vw_spp_ls_gaji_list_detail_belanja_list->RowCnt);
?>
	<?php if ($vw_spp_ls_gaji_list_detail_belanja->kd_rekening_belanja->Visible) { // kd_rekening_belanja ?>
		<td data-name="kd_rekening_belanja"<?php echo $vw_spp_ls_gaji_list_detail_belanja->kd_rekening_belanja->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->RowCnt ?>_vw_spp_ls_gaji_list_detail_belanja_kd_rekening_belanja" class="vw_spp_ls_gaji_list_detail_belanja_kd_rekening_belanja">
...
<a id="<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->PageObjName . "_row_" . $vw_spp_ls_gaji_list_detail_belanja_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($vw_spp_ls_gaji_list_detail_belanja->jumlah_belanja->Visible) { // jumlah_belanja ?>
		<td data-name="jumlah_belanja"<?php echo $vw_spp_ls_gaji_list_detail_belanja->jumlah_belanja->CellAttributes() ?>>
<span id="el<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->RowCnt ?>_vw_spp_ls_gaji_list_detail_belanja_jumlah_belanja" class="vw_spp_ls_gaji_list_detail_belanja_jumlah_belanja">
...
$vw_spp_ls_gaji_list_detail_belanja_list->ListOptions->Render("body", "right", $vw_spp_ls_gaji_list_detail_belanja_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($vw_spp_ls_gaji_list_detail_belanja->CurrentAction <> "gridadd")
		$vw_spp_ls_gaji_list_detail_belanja_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja->CurrentAction == "") { ?>
...
// Close recordset
if ($vw_spp_ls_gaji_list_detail_belanja_list->Recordset)
	$vw_spp_ls_gaji_list_detail_belanja_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($vw_spp_ls_gaji_list_detail_belanja->CurrentAction <> "gridadd" && $vw_spp_ls_gaji_list_detail_belanja->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($vw_spp_ls_gaji_list_detail_belanja_list->Pager)) $vw_spp_ls_gaji_list_detail_belanja_list->Pager = new cPrevNextPager($vw_spp_ls_gaji_list_detail_belanja_list->StartRec, $vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs, $vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs) ?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->Pager->RecordCount > 0 && $vw_spp_ls_gaji_list_detail_belanja_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->PageUrl() ?>start=<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->PageUrl() ?>start=<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->PageUrl() ?>start=<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->PageUrl() ?>start=<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $vw_spp_ls_gaji_list_detail_belanja_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $vw_spp_ls_gaji_list_detail_belanja_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="vw_spp_ls_gaji_list_detail_belanja">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($vw_spp_ls_gaji_list_detail_belanja_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($vw_spp_ls_gaji_list_detail_belanja_list->TotalRecs == 0 && $vw_spp_ls_gaji_list_detail_belanja->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($vw_spp_ls_gaji_list_detail_belanja_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fvw_spp_ls_gaji_list_detail_belanjalistsrch.FilterList = <?php echo $vw_spp_ls_gaji_list_detail_belanja_list->GetFilterList() ?>;
fvw_spp_ls_gaji_list_detail_belanjalistsrch.Init();
fvw_spp_ls_gaji_list_detail_belanjalist.Init();
</script>
<?php
$vw_spp_ls_gaji_list_detail_belanja_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_spp_ls_gaji_list_detail_belanja_list->Page_Terminate();
?>
