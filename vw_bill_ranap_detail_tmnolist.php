...
<?php include_once "vw_bill_ranap_detail_tmnoinfo.php" ?>
...
$vw_bill_ranap_detail_tmno_list = NULL; // Initialize page object first

class cvw_bill_ranap_detail_tmno_list extends cvw_bill_ranap_detail_tmno {
	...
	// Table name
	var $TableName = 'vw_bill_ranap_detail_tmno';

	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_tmno_list';

	// Grid form hidden field names
	var $FormName = 'fvw_bill_ranap_detail_tmnolist';
	...
	function __construct() {
		...
		// Table object (vw_bill_ranap_detail_tmno)
		if (!isset($GLOBALS["vw_bill_ranap_detail_tmno"]) || get_class($GLOBALS["vw_bill_ranap_detail_tmno"]) == "cvw_bill_ranap_detail_tmno") {
			$GLOBALS["vw_bill_ranap_detail_tmno"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["vw_bill_ranap_detail_tmno"];
		}
		...
		$this->AddUrl = "vw_bill_ranap_detail_tmnoadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "vw_bill_ranap_detail_tmnodelete.php";
		$this->MultiUpdateUrl = "vw_bill_ranap_detail_tmnoupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'vw_bill_ranap_detail_tmno', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fvw_bill_ranap_detail_tmnolistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $vw_bill_ranap_detail_tmno;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($vw_bill_ranap_detail_tmno);
				$doc->Text = $sContent;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fvw_bill_ranap_detail_tmnolistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fvw_bill_ranap_detail_tmnolistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fvw_bill_ranap_detail_tmnolistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fvw_bill_ranap_detail_tmnolistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"btn btn-xs btn-default line-2540\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fvw_bill_ranap_detail_tmnolist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fvw_bill_ranap_detail_tmnolistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		$lookuptblfilter = "`kelompok_tindakan`=1";
	...
	function Page_Render() {
		...
		//	$this->nama_tindakan->Visible = FALSE;
		}
	}
...
// Create page object
if (!isset($vw_bill_ranap_detail_tmno_list)) $vw_bill_ranap_detail_tmno_list = new cvw_bill_ranap_detail_tmno_list();

// Page init
$vw_bill_ranap_detail_tmno_list->Page_Init();

// Page main
$vw_bill_ranap_detail_tmno_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_bill_ranap_detail_tmno_list->Page_Render();
?>
...
var CurrentForm = fvw_bill_ranap_detail_tmnolist = new ew_Form("fvw_bill_ranap_detail_tmnolist", "list");
fvw_bill_ranap_detail_tmnolist.FormKeyCountName = '<?php echo $vw_bill_ranap_detail_tmno_list->FormKeyCountName ?>';

// Form_CustomValidate event
fvw_bill_ranap_detail_tmnolist.Form_CustomValidate = 
...
fvw_bill_ranap_detail_tmnolist.ValidateRequired = true;
<?php } else { ?>
fvw_bill_ranap_detail_tmnolist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_bill_ranap_detail_tmnolist.Lists["x_kode_tindakan"] = {"LinkField":"x_kode","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_tindakan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_bill_ranap_data_tarif_tindakan"};
fvw_bill_ranap_detail_tmnolist.Lists["x_kode_dokter"] = {"LinkField":"x_kd_dokter","Ajax":true,"AutoFill":false,"DisplayFields":["x_NAMADOKTER","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_dokter_jaga_ranap"};

// Form object for search
var CurrentSearchForm = fvw_bill_ranap_detail_tmnolistsrch = new ew_Form("fvw_bill_ranap_detail_tmnolistsrch");
</script>
<script type="text/javascript">

// Write your client script here, no need to add script tags.
</script>
<?php if ($vw_bill_ranap_detail_tmno_list->TotalRecs > 0 && $vw_bill_ranap_detail_tmno_list->ExportOptions->Visible()) { ?>
<?php $vw_bill_ranap_detail_tmno_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno_list->SearchOptions->Visible()) { ?>
<?php $vw_bill_ranap_detail_tmno_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno_list->FilterOptions->Visible()) { ?>
<?php $vw_bill_ranap_detail_tmno_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
<?php if (($vw_bill_ranap_detail_tmno->Export == "") || (EW_EXPORT_MASTER_RECORD && $vw_bill_ranap_detail_tmno->Export == "print")) { ?>
<?php
if ($vw_bill_ranap_detail_tmno_list->DbMasterFilter <> "" && $vw_bill_ranap_detail_tmno->getCurrentMasterTable() == "vw_bill_ranap") {
	if ($vw_bill_ranap_detail_tmno_list->MasterRecordExists) {
?>
...
<?php } ?>
<?php
	$bSelectLimit = $vw_bill_ranap_detail_tmno_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($vw_bill_ranap_detail_tmno_list->TotalRecs <= 0)
			$vw_bill_ranap_detail_tmno_list->TotalRecs = $vw_bill_ranap_detail_tmno->SelectRecordCount();
	} else {
		if (!$vw_bill_ranap_detail_tmno_list->Recordset && ($vw_bill_ranap_detail_tmno_list->Recordset = $vw_bill_ranap_detail_tmno_list->LoadRecordset()))
			$vw_bill_ranap_detail_tmno_list->TotalRecs = $vw_bill_ranap_detail_tmno_list->Recordset->RecordCount();
	}
	$vw_bill_ranap_detail_tmno_list->StartRec = 1;
	if ($vw_bill_ranap_detail_tmno_list->DisplayRecs <= 0 || ($vw_bill_ranap_detail_tmno->Export <> "" && $vw_bill_ranap_detail_tmno->ExportAll)) // Display all records
		$vw_bill_ranap_detail_tmno_list->DisplayRecs = $vw_bill_ranap_detail_tmno_list->TotalRecs;
	if (!($vw_bill_ranap_detail_tmno->Export <> "" && $vw_bill_ranap_detail_tmno->ExportAll))
		$vw_bill_ranap_detail_tmno_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$vw_bill_ranap_detail_tmno_list->Recordset = $vw_bill_ranap_detail_tmno_list->LoadRecordset($vw_bill_ranap_detail_tmno_list->StartRec-1, $vw_bill_ranap_detail_tmno_list->DisplayRecs);

	// Set no record found message
	if ($vw_bill_ranap_detail_tmno->CurrentAction == "" && $vw_bill_ranap_detail_tmno_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$vw_bill_ranap_detail_tmno_list->setWarningMessage(ew_DeniedMsg());
		if ($vw_bill_ranap_detail_tmno_list->SearchWhere == "0=101")
			$vw_bill_ranap_detail_tmno_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$vw_bill_ranap_detail_tmno_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($vw_bill_ranap_detail_tmno_list->AuditTrailOnSearch && $vw_bill_ranap_detail_tmno_list->Command == "search" && !$vw_bill_ranap_detail_tmno_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $vw_bill_ranap_detail_tmno_list->getSessionWhere();
		$vw_bill_ranap_detail_tmno_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$vw_bill_ranap_detail_tmno_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($vw_bill_ranap_detail_tmno->Export == "" && $vw_bill_ranap_detail_tmno->CurrentAction == "") { ?>
<form name="fvw_bill_ranap_detail_tmnolistsrch" id="fvw_bill_ranap_detail_tmnolistsrch" class="form-horizontal ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($vw_bill_ranap_detail_tmno_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fvw_bill_ranap_detail_tmnolistsrch_SearchPanel" class="box box-default collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="vw_bill_ranap_detail_tmno">
<div class="box-body">
<div id="xsr_1">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $vw_bill_ranap_detail_tmno_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($vw_bill_ranap_detail_tmno_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($vw_bill_ranap_detail_tmno_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($vw_bill_ranap_detail_tmno_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($vw_bill_ranap_detail_tmno_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $vw_bill_ranap_detail_tmno_list->ShowPageHeader(); ?>
<?php
$vw_bill_ranap_detail_tmno_list->ShowMessage();
?>
<?php if ($vw_bill_ranap_detail_tmno_list->TotalRecs > 0 || $vw_bill_ranap_detail_tmno->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid vw_bill_ranap_detail_tmno">
<form name="fvw_bill_ranap_detail_tmnolist" id="fvw_bill_ranap_detail_tmnolist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_bill_ranap_detail_tmno_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_bill_ranap_detail_tmno_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_bill_ranap_detail_tmno">
<?php if ($vw_bill_ranap_detail_tmno->getCurrentMasterTable() == "vw_bill_ranap" && $vw_bill_ranap_detail_tmno->CurrentAction <> "") { ?>
<input type="hidden" name="<?php echo EW_TABLE_SHOW_MASTER ?>" value="vw_bill_ranap">
<input type="hidden" name="fk_id_admission" value="<?php echo $vw_bill_ranap_detail_tmno->id_admission->getSessionValue() ?>">
<input type="hidden" name="fk_nomr" value="<?php echo $vw_bill_ranap_detail_tmno->nomr->getSessionValue() ?>">
<input type="hidden" name="fk_statusbayar" value="<?php echo $vw_bill_ranap_detail_tmno->statusbayar->getSessionValue() ?>">
<input type="hidden" name="fk_KELASPERAWATAN_ID" value="<?php echo $vw_bill_ranap_detail_tmno->kelas->getSessionValue() ?>">
<input type="hidden" name="fk_noruang" value="<?php echo $vw_bill_ranap_detail_tmno->no_ruang->getSessionValue() ?>">
<?php } ?>
<div id="gmp_vw_bill_ranap_detail_tmno" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>box box-primary">
<?php if ($vw_bill_ranap_detail_tmno_list->TotalRecs > 0 || $vw_bill_ranap_detail_tmno->CurrentAction == "gridedit") { ?>
<table id="tbl_vw_bill_ranap_detail_tmnolist" class="table ewTable table-bordered table-striped table-condensed table-hover">
...
// Header row
$vw_bill_ranap_detail_tmno_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$vw_bill_ranap_detail_tmno_list->RenderListOptions();

// Render list options (header, left)
$vw_bill_ranap_detail_tmno_list->ListOptions->Render("header", "left");
?>
<?php if ($vw_bill_ranap_detail_tmno->tanggal->Visible) { // tanggal ?>
	...
	<?php } else { ?>
		<th data-name="tanggal"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tmno->SortUrl($vw_bill_ranap_detail_tmno->tanggal) ?>',2);"><div id="elh_vw_bill_ranap_detail_tmno_tanggal" class="vw_bill_ranap_detail_tmno_tanggal">
...
<?php if ($vw_bill_ranap_detail_tmno->kode_tindakan->Visible) { // kode_tindakan ?>
	...
	<?php } else { ?>
		<th data-name="kode_tindakan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tmno->SortUrl($vw_bill_ranap_detail_tmno->kode_tindakan) ?>',2);"><div id="elh_vw_bill_ranap_detail_tmno_kode_tindakan" class="vw_bill_ranap_detail_tmno_kode_tindakan">
...
<?php if ($vw_bill_ranap_detail_tmno->kode_dokter->Visible) { // kode_dokter ?>
	...
	<?php } else { ?>
		<th data-name="kode_dokter"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tmno->SortUrl($vw_bill_ranap_detail_tmno->kode_dokter) ?>',2);"><div id="elh_vw_bill_ranap_detail_tmno_kode_dokter" class="vw_bill_ranap_detail_tmno_kode_dokter">
...
<?php if ($vw_bill_ranap_detail_tmno->qty->Visible) { // qty ?>
	...
	<?php } else { ?>
		<th data-name="qty"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tmno->SortUrl($vw_bill_ranap_detail_tmno->qty) ?>',2);"><div id="elh_vw_bill_ranap_detail_tmno_qty" class="vw_bill_ranap_detail_tmno_qty">
...
<?php if ($vw_bill_ranap_detail_tmno->tarif->Visible) { // tarif ?>
	...
	<?php } else { ?>
		<th data-name="tarif"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tmno->SortUrl($vw_bill_ranap_detail_tmno->tarif) ?>',2);"><div id="elh_vw_bill_ranap_detail_tmno_tarif" class="vw_bill_ranap_detail_tmno_tarif">
...
<?php if ($vw_bill_ranap_detail_tmno->bhp->Visible) { // bhp ?>
	...
	<?php } else { ?>
		<th data-name="bhp"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tmno->SortUrl($vw_bill_ranap_detail_tmno->bhp) ?>',2);"><div id="elh_vw_bill_ranap_detail_tmno_bhp" class="vw_bill_ranap_detail_tmno_bhp">
...
<?php if ($vw_bill_ranap_detail_tmno->user->Visible) { // user ?>
	...
	<?php } else { ?>
		<th data-name="user"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tmno->SortUrl($vw_bill_ranap_detail_tmno->user) ?>',2);"><div id="elh_vw_bill_ranap_detail_tmno_user" class="vw_bill_ranap_detail_tmno_user">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_bill_ranap_detail_tmno->user->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($vw_bill_ranap_detail_tmno->user->getSort() == "ASC") { ?> <i class="fa fa-sort-amount-asc text-muted"></i><?php } elseif ($vw_bill_ranap_detail_tmno->user->getSort() == "DESC") { ?> <i class="fa fa-sort-amount-desc text-muted"></i><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_bill_ranap_detail_tmno->no_ruang->Visible) { // no_ruang ?>
	...
	<?php } else { ?>
		<th data-name="no_ruang"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_bill_ranap_detail_tmno->SortUrl($vw_bill_ranap_detail_tmno->no_ruang) ?>',2);"><div id="elh_vw_bill_ranap_detail_tmno_no_ruang" class="vw_bill_ranap_detail_tmno_no_ruang">
...
$vw_bill_ranap_detail_tmno_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($vw_bill_ranap_detail_tmno->ExportAll && $vw_bill_ranap_detail_tmno->Export <> "") {
	$vw_bill_ranap_detail_tmno_list->StopRec = $vw_bill_ranap_detail_tmno_list->TotalRecs;
} else {

	// Set the last record to display
	if ($vw_bill_ranap_detail_tmno_list->TotalRecs > $vw_bill_ranap_detail_tmno_list->StartRec + $vw_bill_ranap_detail_tmno_list->DisplayRecs - 1)
		$vw_bill_ranap_detail_tmno_list->StopRec = $vw_bill_ranap_detail_tmno_list->StartRec + $vw_bill_ranap_detail_tmno_list->DisplayRecs - 1;
	else
		$vw_bill_ranap_detail_tmno_list->StopRec = $vw_bill_ranap_detail_tmno_list->TotalRecs;
}
$vw_bill_ranap_detail_tmno_list->RecCnt = $vw_bill_ranap_detail_tmno_list->StartRec - 1;
if ($vw_bill_ranap_detail_tmno_list->Recordset && !$vw_bill_ranap_detail_tmno_list->Recordset->EOF) {
	$vw_bill_ranap_detail_tmno_list->Recordset->MoveFirst();
	$bSelectLimit = $vw_bill_ranap_detail_tmno_list->UseSelectLimit;
	if (!$bSelectLimit && $vw_bill_ranap_detail_tmno_list->StartRec > 1)
		$vw_bill_ranap_detail_tmno_list->Recordset->Move($vw_bill_ranap_detail_tmno_list->StartRec - 1);
} elseif (!$vw_bill_ranap_detail_tmno->AllowAddDeleteRow && $vw_bill_ranap_detail_tmno_list->StopRec == 0) {
	$vw_bill_ranap_detail_tmno_list->StopRec = $vw_bill_ranap_detail_tmno->GridAddRowCount;
}

// Initialize aggregate
$vw_bill_ranap_detail_tmno->RowType = EW_ROWTYPE_AGGREGATEINIT;
$vw_bill_ranap_detail_tmno->ResetAttrs();
$vw_bill_ranap_detail_tmno_list->RenderRow();
while ($vw_bill_ranap_detail_tmno_list->RecCnt < $vw_bill_ranap_detail_tmno_list->StopRec) {
	$vw_bill_ranap_detail_tmno_list->RecCnt++;
	if (intval($vw_bill_ranap_detail_tmno_list->RecCnt) >= intval($vw_bill_ranap_detail_tmno_list->StartRec)) {
		$vw_bill_ranap_detail_tmno_list->RowCnt++;

		// Set up key count
		$vw_bill_ranap_detail_tmno_list->KeyCount = $vw_bill_ranap_detail_tmno_list->RowIndex;
		...
		if ($vw_bill_ranap_detail_tmno->CurrentAction == "gridadd") {
			...
			$vw_bill_ranap_detail_tmno_list->LoadRowValues($vw_bill_ranap_detail_tmno_list->Recordset); // Load row values
		}
		$vw_bill_ranap_detail_tmno->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$vw_bill_ranap_detail_tmno->RowAttrs = array_merge($vw_bill_ranap_detail_tmno->RowAttrs, array('data-rowindex'=>$vw_bill_ranap_detail_tmno_list->RowCnt, 'id'=>'r' . $vw_bill_ranap_detail_tmno_list->RowCnt . '_vw_bill_ranap_detail_tmno', 'data-rowtype'=>$vw_bill_ranap_detail_tmno->RowType));
...
$vw_bill_ranap_detail_tmno_list->ListOptions->Render("body", "left", $vw_bill_ranap_detail_tmno_list->RowCnt);
?>
	<?php if ($vw_bill_ranap_detail_tmno->tanggal->Visible) { // tanggal ?>
		<td data-name="tanggal"<?php echo $vw_bill_ranap_detail_tmno->tanggal->CellAttributes() ?>>
<span id="el<?php echo $vw_bill_ranap_detail_tmno_list->RowCnt ?>_vw_bill_ranap_detail_tmno_tanggal" class="vw_bill_ranap_detail_tmno_tanggal">
...
<a id="<?php echo $vw_bill_ranap_detail_tmno_list->PageObjName . "_row_" . $vw_bill_ranap_detail_tmno_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($vw_bill_ranap_detail_tmno->kode_tindakan->Visible) { // kode_tindakan ?>
		<td data-name="kode_tindakan"<?php echo $vw_bill_ranap_detail_tmno->kode_tindakan->CellAttributes() ?>>
<span id="el<?php echo $vw_bill_ranap_detail_tmno_list->RowCnt ?>_vw_bill_ranap_detail_tmno_kode_tindakan" class="vw_bill_ranap_detail_tmno_kode_tindakan">
...
<span id="el<?php echo $vw_bill_ranap_detail_tmno_list->RowCnt ?>_vw_bill_ranap_detail_tmno_kode_dokter" class="vw_bill_ranap_detail_tmno_kode_dokter">
...
<span id="el<?php echo $vw_bill_ranap_detail_tmno_list->RowCnt ?>_vw_bill_ranap_detail_tmno_qty" class="vw_bill_ranap_detail_tmno_qty">
...
<span id="el<?php echo $vw_bill_ranap_detail_tmno_list->RowCnt ?>_vw_bill_ranap_detail_tmno_tarif" class="vw_bill_ranap_detail_tmno_tarif">
...
<span id="el<?php echo $vw_bill_ranap_detail_tmno_list->RowCnt ?>_vw_bill_ranap_detail_tmno_bhp" class="vw_bill_ranap_detail_tmno_bhp">
...
<span id="el<?php echo $vw_bill_ranap_detail_tmno_list->RowCnt ?>_vw_bill_ranap_detail_tmno_user" class="vw_bill_ranap_detail_tmno_user">
...
<span id="el<?php echo $vw_bill_ranap_detail_tmno_list->RowCnt ?>_vw_bill_ranap_detail_tmno_no_ruang" class="vw_bill_ranap_detail_tmno_no_ruang">
...
$vw_bill_ranap_detail_tmno_list->ListOptions->Render("body", "right", $vw_bill_ranap_detail_tmno_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($vw_bill_ranap_detail_tmno->CurrentAction <> "gridadd")
		$vw_bill_ranap_detail_tmno_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->CurrentAction == "") { ?>
...
// Close recordset
if ($vw_bill_ranap_detail_tmno_list->Recordset)
	$vw_bill_ranap_detail_tmno_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($vw_bill_ranap_detail_tmno->CurrentAction <> "gridadd" && $vw_bill_ranap_detail_tmno->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($vw_bill_ranap_detail_tmno_list->Pager)) $vw_bill_ranap_detail_tmno_list->Pager = new cPrevNextPager($vw_bill_ranap_detail_tmno_list->StartRec, $vw_bill_ranap_detail_tmno_list->DisplayRecs, $vw_bill_ranap_detail_tmno_list->TotalRecs) ?>
<?php if ($vw_bill_ranap_detail_tmno_list->Pager->RecordCount > 0 && $vw_bill_ranap_detail_tmno_list->Pager->Visible) { ?>
<div class="text-center">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($vw_bill_ranap_detail_tmno_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $vw_bill_ranap_detail_tmno_list->PageUrl() ?>start=<?php echo $vw_bill_ranap_detail_tmno_list->Pager->FirstButton->Start ?>"><span class="fa fa-fast-backward"></span></a>
	<?php } else { ?>
...
<!--previous page button-->
	<?php if ($vw_bill_ranap_detail_tmno_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $vw_bill_ranap_detail_tmno_list->PageUrl() ?>start=<?php echo $vw_bill_ranap_detail_tmno_list->Pager->PrevButton->Start ?>"><span class="fa fa-step-backward"></span></a>
	<?php } else { ?>
...
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $vw_bill_ranap_detail_tmno_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($vw_bill_ranap_detail_tmno_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $vw_bill_ranap_detail_tmno_list->PageUrl() ?>start=<?php echo $vw_bill_ranap_detail_tmno_list->Pager->NextButton->Start ?>"><span class="fa fa-step-forward"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="fa fa-step-forward"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($vw_bill_ranap_detail_tmno_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $vw_bill_ranap_detail_tmno_list->PageUrl() ?>start=<?php echo $vw_bill_ranap_detail_tmno_list->Pager->LastButton->Start ?>"><span class="fa fa-fast-forward"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $vw_bill_ranap_detail_tmno_list->Pager->PageCount ?></span>
</div>
<div class="text-center">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $vw_bill_ranap_detail_tmno_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $vw_bill_ranap_detail_tmno_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $vw_bill_ranap_detail_tmno_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $vw_bill_ranap_detail_tmno_list->Pager->Visible)) { ?>
<div class="text-center">
<input type="hidden" name="t" value="vw_bill_ranap_detail_tmno">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($vw_bill_ranap_detail_tmno_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($vw_bill_ranap_detail_tmno_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($vw_bill_ranap_detail_tmno_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($vw_bill_ranap_detail_tmno_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($vw_bill_ranap_detail_tmno_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($vw_bill_ranap_detail_tmno_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno_list->TotalRecs == 0 && $vw_bill_ranap_detail_tmno->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($vw_bill_ranap_detail_tmno_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fvw_bill_ranap_detail_tmnolistsrch.FilterList = <?php echo $vw_bill_ranap_detail_tmno_list->GetFilterList() ?>;
fvw_bill_ranap_detail_tmnolistsrch.Init();
fvw_bill_ranap_detail_tmnolist.Init();
</script>
<?php
$vw_bill_ranap_detail_tmno_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_bill_ranap_detail_tmno_list->Page_Terminate();
?>
