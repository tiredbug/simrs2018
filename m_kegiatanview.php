...
$m_kegiatan_view = NULL; // Initialize page object first

class cm_kegiatan_view extends cm_kegiatan {
	...
	// Page object name
	var $PageObjName = 'm_kegiatan_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "m_kegiatanlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "m_kegiatanlist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "m_kegiatanlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$item = &$option->Add("detail_m_sub_kegiatan");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("m_sub_kegiatan", "TblCaption");
		$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("m_sub_kegiatanlist.php?" . EW_TABLE_SHOW_MASTER . "=m_kegiatan&fk_kode_program=" . urlencode(strval($this->kode_program->CurrentValue)) . "&fk_kode_kegiatan=" . urlencode(strval($this->kode_kegiatan->CurrentValue)) . "") . "\">" . $body . "</a>";
		$links = "";
		if ($GLOBALS["m_sub_kegiatan_grid"] && $GLOBALS["m_sub_kegiatan_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'm_sub_kegiatan')) {
		...
		if ($GLOBALS["m_sub_kegiatan_grid"] && $GLOBALS["m_sub_kegiatan_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'm_sub_kegiatan')) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=m_sub_kegiatan")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
			if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
			$DetailEditTblVar .= "m_sub_kegiatan";
		}
		if ($GLOBALS["m_sub_kegiatan_grid"] && $GLOBALS["m_sub_kegiatan_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'm_sub_kegiatan')) {
			$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=m_sub_kegiatan")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'm_sub_kegiatan');
		if ($item->Visible) {
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("m_sub_kegiatan", $DetailTblVar)) {
				...
				if ($GLOBALS["m_sub_kegiatan_grid"]->DetailView) {
					$GLOBALS["m_sub_kegiatan_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($m_kegiatan_view)) $m_kegiatan_view = new cm_kegiatan_view();

// Page init
$m_kegiatan_view->Page_Init();

// Page main
$m_kegiatan_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$m_kegiatan_view->Page_Render();
?>
...
var CurrentForm = fm_kegiatanview = new ew_Form("fm_kegiatanview", "view");

// Form_CustomValidate event
fm_kegiatanview.Form_CustomValidate = 
...
fm_kegiatanview.ValidateRequired = true;
<?php } else { ?>
fm_kegiatanview.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$m_kegiatan_view->IsModal) { ?>
<?php } ?>
<?php $m_kegiatan_view->ExportOptions->Render("body") ?>
<?php
	foreach ($m_kegiatan_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$m_kegiatan_view->IsModal) { ?>
...
<?php $m_kegiatan_view->ShowPageHeader(); ?>
<?php
$m_kegiatan_view->ShowMessage();
?>
<form name="fm_kegiatanview" id="fm_kegiatanview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($m_kegiatan_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $m_kegiatan_view->Token ?>">
<?php } ?>
...
<?php if ($m_kegiatan->kode_program->Visible) { // kode_program ?>
	<tr id="r_kode_program">
		<td><span id="elh_m_kegiatan_kode_program"><?php echo $m_kegiatan->kode_program->FldCaption() ?></span></td>
...
<?php echo $m_kegiatan->kode_program->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($m_kegiatan->kode_kegiatan->Visible) { // kode_kegiatan ?>
	<tr id="r_kode_kegiatan">
		<td><span id="elh_m_kegiatan_kode_kegiatan"><?php echo $m_kegiatan->kode_kegiatan->FldCaption() ?></span></td>
...
<?php echo $m_kegiatan->kode_kegiatan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($m_kegiatan->nama_kegiatan->Visible) { // nama_kegiatan ?>
	<tr id="r_nama_kegiatan">
		<td><span id="elh_m_kegiatan_nama_kegiatan"><?php echo $m_kegiatan->nama_kegiatan->FldCaption() ?></span></td>
...
<?php echo $m_kegiatan->nama_kegiatan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("m_sub_kegiatan", explode(",", $m_kegiatan->getCurrentDetailTable())) && $m_sub_kegiatan->DetailView) {
?>
...
<script type="text/javascript">
fm_kegiatanview.Init();
</script>
<?php
$m_kegiatan_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$m_kegiatan_view->Page_Terminate();
?>
