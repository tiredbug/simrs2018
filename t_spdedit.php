...
$t_spd_edit = NULL; // Initialize page object first

class ct_spd_edit extends ct_spd {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("t_spdlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// jenis_peraturan
			$this->jenis_peraturan->SetDbValueDef($rsnew, $this->jenis_peraturan->CurrentValue, NULL, $this->jenis_peraturan->ReadOnly);

			// tanggal
			$this->tanggal->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tanggal->CurrentValue, 7), NULL, $this->tanggal->ReadOnly);

			// no_spd
			$this->no_spd->SetDbValueDef($rsnew, $this->no_spd->CurrentValue, NULL, $this->no_spd->ReadOnly);
			...
			// no_sk_dir
			$this->no_sk_dir->SetDbValueDef($rsnew, $this->no_sk_dir->CurrentValue, NULL, $this->no_sk_dir->ReadOnly);

			// tgl_sk_dir
			$this->tgl_sk_dir->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tgl_sk_dir->CurrentValue, 7), NULL, $this->tgl_sk_dir->ReadOnly);

			// th_anggaran
			$this->th_anggaran->SetDbValueDef($rsnew, $this->th_anggaran->CurrentValue, NULL, $this->th_anggaran->ReadOnly);

			// tentang
			$this->tentang->SetDbValueDef($rsnew, $this->tentang->CurrentValue, NULL, $this->tentang->ReadOnly);
...
// Create page object
if (!isset($t_spd_edit)) $t_spd_edit = new ct_spd_edit();

// Page init
$t_spd_edit->Page_Init();

// Page main
$t_spd_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_spd_edit->Page_Render();
?>
...
var CurrentForm = ft_spdedit = new ew_Form("ft_spdedit", "edit");

// Validate form
ft_spdedit.Validate = function() {
...
ft_spdedit.Form_CustomValidate = 
...
ft_spdedit.ValidateRequired = true;
<?php } else { ?>
ft_spdedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_spdedit.Lists["x_jenis_peraturan"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft_spdedit.Lists["x_jenis_peraturan"].Options = <?php echo json_encode($t_spd->jenis_peraturan->Options()) ?>;
...
<?php if (!$t_spd_edit->IsModal) { ?>
<?php } ?>
<?php $t_spd_edit->ShowPageHeader(); ?>
<?php
$t_spd_edit->ShowMessage();
?>
<form name="ft_spdedit" id="ft_spdedit" class="<?php echo $t_spd_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_spd_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_spd_edit->Token ?>">
<?php } ?>
...
<?php if ($t_spd->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_t_spd_id" class="col-sm-2 control-label ewLabel"><?php echo $t_spd->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $t_spd->id->CellAttributes() ?>>
<span id="el_t_spd_id">
<span<?php echo $t_spd->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $t_spd->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="t_spd" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($t_spd->id->CurrentValue) ?>">
<?php echo $t_spd->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("ft_spdedit", "x_tanggal", 7);
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("ft_spdedit", "x_tgl_sk_dir", 7);
</script>
<?php } ?>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_spd_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
