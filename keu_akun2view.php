...
$keu_akun2_view = NULL; // Initialize page object first

class ckeu_akun2_view extends ckeu_akun2 {
	...
	// Page object name
	var $PageObjName = 'keu_akun2_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "keu_akun2list.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "keu_akun2list.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "keu_akun2list.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// "detail_keu_akun3"
		$item = &$option->Add("detail_keu_akun3");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("keu_akun3", "TblCaption");
		$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("keu_akun3list.php?" . EW_TABLE_SHOW_MASTER . "=keu_akun2&fk_kel1=" . urlencode(strval($this->kel1->CurrentValue)) . "&fk_kel2=" . urlencode(strval($this->kel2->CurrentValue)) . "") . "\">" . $body . "</a>";
		$links = "";
		if ($GLOBALS["keu_akun3_grid"] && $GLOBALS["keu_akun3_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'keu_akun3')) {
		...
		if ($GLOBALS["keu_akun3_grid"] && $GLOBALS["keu_akun3_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'keu_akun3')) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=keu_akun3")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
			if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
			$DetailEditTblVar .= "keu_akun3";
		}
		if ($GLOBALS["keu_akun3_grid"] && $GLOBALS["keu_akun3_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'keu_akun3')) {
			$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=keu_akun3")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'keu_akun3');
		if ($item->Visible) {
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("keu_akun3", $DetailTblVar)) {
				...
				if ($GLOBALS["keu_akun3_grid"]->DetailView) {
					$GLOBALS["keu_akun3_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($keu_akun2_view)) $keu_akun2_view = new ckeu_akun2_view();

// Page init
$keu_akun2_view->Page_Init();

// Page main
$keu_akun2_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$keu_akun2_view->Page_Render();
?>
...
var CurrentForm = fkeu_akun2view = new ew_Form("fkeu_akun2view", "view");

// Form_CustomValidate event
fkeu_akun2view.Form_CustomValidate = 
...
fkeu_akun2view.ValidateRequired = true;
<?php } else { ?>
fkeu_akun2view.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$keu_akun2_view->IsModal) { ?>
<?php } ?>
<?php $keu_akun2_view->ExportOptions->Render("body") ?>
<?php
	foreach ($keu_akun2_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$keu_akun2_view->IsModal) { ?>
...
<?php $keu_akun2_view->ShowPageHeader(); ?>
<?php
$keu_akun2_view->ShowMessage();
?>
<form name="fkeu_akun2view" id="fkeu_akun2view" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($keu_akun2_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $keu_akun2_view->Token ?>">
<?php } ?>
...
<?php if ($keu_akun2->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_keu_akun2_id"><?php echo $keu_akun2->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $keu_akun2->id->CellAttributes() ?>>
<span id="el_keu_akun2_id">
<span<?php echo $keu_akun2->id->ViewAttributes() ?>>
<?php echo $keu_akun2->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($keu_akun2->kd_akun->Visible) { // kd_akun ?>
	<tr id="r_kd_akun">
		<td><span id="elh_keu_akun2_kd_akun"><?php echo $keu_akun2->kd_akun->FldCaption() ?></span></td>
...
<?php echo $keu_akun2->kd_akun->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($keu_akun2->kel1->Visible) { // kel1 ?>
	<tr id="r_kel1">
		<td><span id="elh_keu_akun2_kel1"><?php echo $keu_akun2->kel1->FldCaption() ?></span></td>
		<td data-name="kel1"<?php echo $keu_akun2->kel1->CellAttributes() ?>>
<span id="el_keu_akun2_kel1">
<span<?php echo $keu_akun2->kel1->ViewAttributes() ?>>
<?php echo $keu_akun2->kel1->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($keu_akun2->kel2->Visible) { // kel2 ?>
	<tr id="r_kel2">
		<td><span id="elh_keu_akun2_kel2"><?php echo $keu_akun2->kel2->FldCaption() ?></span></td>
...
<?php echo $keu_akun2->kel2->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($keu_akun2->nmkel2->Visible) { // nmkel2 ?>
	<tr id="r_nmkel2">
		<td><span id="elh_keu_akun2_nmkel2"><?php echo $keu_akun2->nmkel2->FldCaption() ?></span></td>
...
<?php echo $keu_akun2->nmkel2->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($keu_akun2->kode_akun1->Visible) { // kode_akun1 ?>
	<tr id="r_kode_akun1">
		<td><span id="elh_keu_akun2_kode_akun1"><?php echo $keu_akun2->kode_akun1->FldCaption() ?></span></td>
		<td data-name="kode_akun1"<?php echo $keu_akun2->kode_akun1->CellAttributes() ?>>
<span id="el_keu_akun2_kode_akun1">
<span<?php echo $keu_akun2->kode_akun1->ViewAttributes() ?>>
<?php echo $keu_akun2->kode_akun1->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("keu_akun3", explode(",", $keu_akun2->getCurrentDetailTable())) && $keu_akun3->DetailView) {
?>
