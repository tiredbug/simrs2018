...
//
// Table class for keu_akun2
//
class ckeu_akun2 extends cTable {
	var $id;
	var $kd_akun;
	var $kel1;
	var $kel2;
	...
	function __construct() {
		...
		$this->TableVar = 'keu_akun2';
		$this->TableName = 'keu_akun2';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`keu_akun2`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('keu_akun2', 'keu_akun2', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// kd_akun
		$this->kd_akun = new cField('keu_akun2', 'keu_akun2', 'x_kd_akun', 'kd_akun', '`kd_akun`', '`kd_akun`', 200, -1, FALSE, '`kd_akun`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kd_akun->Sortable = TRUE; // Allow sort
		$this->fields['kd_akun'] = &$this->kd_akun;

		// kel1
		$this->kel1 = new cField('keu_akun2', 'keu_akun2', 'x_kel1', 'kel1', '`kel1`', '`kel1`', 200, -1, FALSE, '`kel1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel1->Sortable = TRUE; // Allow sort
		$this->fields['kel1'] = &$this->kel1;

		// kel2
		$this->kel2 = new cField('keu_akun2', 'keu_akun2', 'x_kel2', 'kel2', '`kel2`', '`kel2`', 200, -1, FALSE, '`kel2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel2->Sortable = TRUE; // Allow sort
		$this->fields['kel2'] = &$this->kel2;

		// nmkel2
		$this->nmkel2 = new cField('keu_akun2', 'keu_akun2', 'x_nmkel2', 'nmkel2', '`nmkel2`', '`nmkel2`', 200, -1, FALSE, '`nmkel2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nmkel2->Sortable = TRUE; // Allow sort
		$this->fields['nmkel2'] = &$this->nmkel2;

		// kode_akun1
		$this->kode_akun1 = new cField('keu_akun2', 'keu_akun2', 'x_kode_akun1', 'kode_akun1', '`kode_akun1`', '`kode_akun1`', 3, -1, FALSE, '`kode_akun1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kode_akun1->Sortable = TRUE; // Allow sort
		$this->kode_akun1->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['kode_akun1'] = &$this->kode_akun1;
	}
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "keu_akun1") {
			if ($this->kel1->getSessionValue() <> "")
				$sMasterFilter .= "`kel1`=" . ew_QuotedValue($this->kel1->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_keu_akun1() {
		return "`kel1`=@kel1@";
	}

	// Detail filter
	function SqlDetailFilter_keu_akun1() {
		return "`kel1`='@kel1@'";
	}
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "keu_akun3") {
			$sDetailUrl = $GLOBALS["keu_akun3"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_kel1=" . urlencode($this->kel1->CurrentValue);
			$sDetailUrl .= "&fk_kel2=" . urlencode($this->kel2->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "keu_akun2list.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`keu_akun2`";
	}
	...
	function getSqlWhere() { // Where
		...
		$this->TableFilter = "`kel2` IS NOT NULL";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("keu_akun2view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("keu_akun2view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "keu_akun2add.php?" . $this->UrlParm($parm);
		else
			$url = "keu_akun2add.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("keu_akun2edit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("keu_akun2edit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("keu_akun2add.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("keu_akun2add.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("keu_akun2delete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "keu_akun1" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->nmkel2->EditValue = $this->nmkel2->CurrentValue;
		...
		$this->kode_akun1->EditValue = $this->kode_akun1->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nmkel2->Exportable) $Doc->ExportCaption($this->nmkel2);
					if ($this->kode_akun1->Exportable) $Doc->ExportCaption($this->kode_akun1);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->kd_akun->Exportable) $Doc->ExportCaption($this->kd_akun);
					if ($this->kel1->Exportable) $Doc->ExportCaption($this->kel1);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->nmkel2->Exportable) $Doc->ExportField($this->nmkel2);
						if ($this->kode_akun1->Exportable) $Doc->ExportField($this->kode_akun1);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->kd_akun->Exportable) $Doc->ExportField($this->kd_akun);
						if ($this->kel1->Exportable) $Doc->ExportField($this->kel1);
