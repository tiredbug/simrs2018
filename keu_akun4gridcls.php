...
$keu_akun4_grid = NULL; // Initialize page object first

class ckeu_akun4_grid extends ckeu_akun4 {
	...
	// Page object name
	var $PageObjName = 'keu_akun4_grid';

	// Grid form hidden field names
	var $FormName = 'fkeu_akun4grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["keu_akun4"];

		}
		$this->AddUrl = "keu_akun4add.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_kd_akun") && $objForm->HasValue("o_kd_akun") && $this->kd_akun->CurrentValue <> $this->kd_akun->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_kel4") && $objForm->HasValue("o_kel4") && $this->kel4->CurrentValue <> $this->kel4->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nmkel4") && $objForm->HasValue("o_nmkel4") && $this->nmkel4->CurrentValue <> $this->nmkel4->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->kel4->setOldValue($objForm->GetValue("o_kel4"));
		if (!$this->nmkel4->FldIsDetailKey) {
			$this->nmkel4->setFormValue($objForm->GetValue("x_nmkel4"));
		}
		$this->nmkel4->setOldValue($objForm->GetValue("o_nmkel4"));
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->kel1->getSessionValue() <> "") {
			$rsnew['kel1'] = $this->kel1->getSessionValue();
		}

		// kel2
		if ($this->kel2->getSessionValue() <> "") {
			$rsnew['kel2'] = $this->kel2->getSessionValue();
		}

		// kel3
		if ($this->kel3->getSessionValue() <> "") {
			$rsnew['kel3'] = $this->kel3->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "keu_akun3") {
			$this->kel1->Visible = FALSE;
			if ($GLOBALS["keu_akun3"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->kel2->Visible = FALSE;
			if ($GLOBALS["keu_akun3"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->kel3->Visible = FALSE;
			if ($GLOBALS["keu_akun3"]->EventCancelled) $this->EventCancelled = TRUE;
		}
