...
$vw_bill_ranap_detail_tmno_edit = NULL; // Initialize page object first

class cvw_bill_ranap_detail_tmno_edit extends cvw_bill_ranap_detail_tmno {
	...
	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_tmno_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("vw_bill_ranap_detail_tmnolist.php"));
			else
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("vw_bill_ranap_detail_tmnolist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("vw_bill_ranap_detail_tmnolist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "vw_bill_ranap_detail_tmnolist.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("vw_bill_ranap_detail_tmnolist.php"), "", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($vw_bill_ranap_detail_tmno_edit)) $vw_bill_ranap_detail_tmno_edit = new cvw_bill_ranap_detail_tmno_edit();

// Page init
$vw_bill_ranap_detail_tmno_edit->Page_Init();

// Page main
$vw_bill_ranap_detail_tmno_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_bill_ranap_detail_tmno_edit->Page_Render();
?>
...
var CurrentForm = fvw_bill_ranap_detail_tmnoedit = new ew_Form("fvw_bill_ranap_detail_tmnoedit", "edit");

// Validate form
fvw_bill_ranap_detail_tmnoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tmno->id_admission->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_statusbayar");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tmno->statusbayar->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelas");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tmno->kelas->FldErrMsg()) ?>");
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tmno->kelompok_tindakan->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok1");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tmno->kelompok1->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kelompok2");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($vw_bill_ranap_detail_tmno->kelompok2->FldErrMsg()) ?>");
...
fvw_bill_ranap_detail_tmnoedit.Form_CustomValidate = 
...
fvw_bill_ranap_detail_tmnoedit.ValidateRequired = true;
<?php } else { ?>
fvw_bill_ranap_detail_tmnoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_bill_ranap_detail_tmnoedit.Lists["x_kode_tindakan"] = {"LinkField":"x_kode","Ajax":true,"AutoFill":true,"DisplayFields":["x_nama_tindakan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_bill_ranap_data_tarif_tindakan"};
fvw_bill_ranap_detail_tmnoedit.Lists["x_kode_dokter"] = {"LinkField":"x_kd_dokter","Ajax":true,"AutoFill":false,"DisplayFields":["x_NAMADOKTER","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"vw_dokter_jaga_ranap"};
...
<?php if (!$vw_bill_ranap_detail_tmno_edit->IsModal) { ?>
<?php } ?>
<?php $vw_bill_ranap_detail_tmno_edit->ShowPageHeader(); ?>
<?php
$vw_bill_ranap_detail_tmno_edit->ShowMessage();
?>
<form name="fvw_bill_ranap_detail_tmnoedit" id="fvw_bill_ranap_detail_tmnoedit" class="<?php echo $vw_bill_ranap_detail_tmno_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_bill_ranap_detail_tmno_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_bill_ranap_detail_tmno_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->getCurrentMasterTable() == "vw_bill_ranap") { ?>
...
<?php } ?>
<div>
<?php if ($vw_bill_ranap_detail_tmno->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_id" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->id->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_id">
<span<?php echo $vw_bill_ranap_detail_tmno->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tmno->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="vw_bill_ranap_detail_tmno" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->id->CurrentValue) ?>">
<?php echo $vw_bill_ranap_detail_tmno->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->id_admission->Visible) { // id_admission ?>
	<div id="r_id_admission" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_id_admission" for="x_id_admission" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->id_admission->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->id_admission->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tmno->id_admission->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tmno_id_admission">
<span<?php echo $vw_bill_ranap_detail_tmno->id_admission->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tmno->id_admission->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_id_admission" name="x_id_admission" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->id_admission->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tmno_id_admission">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_id_admission" name="x_id_admission" id="x_id_admission" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->id_admission->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->id_admission->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->id_admission->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tmno->id_admission->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->nomr->Visible) { // nomr ?>
	<div id="r_nomr" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_nomr" for="x_nomr" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->nomr->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->nomr->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tmno->nomr->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tmno_nomr">
<span<?php echo $vw_bill_ranap_detail_tmno->nomr->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tmno->nomr->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_nomr" name="x_nomr" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->nomr->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tmno_nomr">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_nomr" name="x_nomr" id="x_nomr" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->nomr->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->nomr->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->nomr->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tmno->nomr->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->statusbayar->Visible) { // statusbayar ?>
	<div id="r_statusbayar" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_statusbayar" for="x_statusbayar" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->statusbayar->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->statusbayar->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tmno->statusbayar->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tmno_statusbayar">
<span<?php echo $vw_bill_ranap_detail_tmno->statusbayar->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tmno->statusbayar->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_statusbayar" name="x_statusbayar" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->statusbayar->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tmno_statusbayar">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_statusbayar" name="x_statusbayar" id="x_statusbayar" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->statusbayar->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->statusbayar->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->statusbayar->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tmno->statusbayar->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->kelas->Visible) { // kelas ?>
	<div id="r_kelas" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_kelas" for="x_kelas" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->kelas->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->kelas->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tmno->kelas->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tmno_kelas">
<span<?php echo $vw_bill_ranap_detail_tmno->kelas->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $vw_bill_ranap_detail_tmno->kelas->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_kelas" name="x_kelas" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->kelas->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tmno_kelas">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_kelas" name="x_kelas" id="x_kelas" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->kelas->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->kelas->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->kelas->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tmno->kelas->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->tanggal->Visible) { // tanggal ?>
	<div id="r_tanggal" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_tanggal" for="x_tanggal" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->tanggal->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->tanggal->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_tanggal">
...
<script type="text/javascript">
ew_CreateCalendar("fvw_bill_ranap_detail_tmnoedit", "x_tanggal", 7);
</script>
<?php } ?>
</span>
<?php echo $vw_bill_ranap_detail_tmno->tanggal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->kode_tindakan->Visible) { // kode_tindakan ?>
	<div id="r_kode_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_kode_tindakan" for="x_kode_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->kode_tindakan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->kode_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_kode_tindakan">
...
<select data-table="vw_bill_ranap_detail_tmno" data-field="x_kode_tindakan" data-value-separator="<?php echo $vw_bill_ranap_detail_tmno->kode_tindakan->DisplayValueSeparatorAttribute() ?>" id="x_kode_tindakan" name="x_kode_tindakan"<?php echo $vw_bill_ranap_detail_tmno->kode_tindakan->EditAttributes() ?>>
<?php echo $vw_bill_ranap_detail_tmno->kode_tindakan->SelectOptionListHtml("x_kode_tindakan") ?>
</select>
<input type="hidden" name="s_x_kode_tindakan" id="s_x_kode_tindakan" value="<?php echo $vw_bill_ranap_detail_tmno->kode_tindakan->LookupFilterQuery() ?>">
<input type="hidden" name="ln_x_kode_tindakan" id="ln_x_kode_tindakan" value="x_tarif,x_bhp">
</span>
<?php echo $vw_bill_ranap_detail_tmno->kode_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->kode_dokter->Visible) { // kode_dokter ?>
	<div id="r_kode_dokter" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_kode_dokter" for="x_kode_dokter" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->kode_dokter->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->kode_dokter->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_kode_dokter">
<select data-table="vw_bill_ranap_detail_tmno" data-field="x_kode_dokter" data-value-separator="<?php echo $vw_bill_ranap_detail_tmno->kode_dokter->DisplayValueSeparatorAttribute() ?>" id="x_kode_dokter" name="x_kode_dokter"<?php echo $vw_bill_ranap_detail_tmno->kode_dokter->EditAttributes() ?>>
<?php echo $vw_bill_ranap_detail_tmno->kode_dokter->SelectOptionListHtml("x_kode_dokter") ?>
</select>
<input type="hidden" name="s_x_kode_dokter" id="s_x_kode_dokter" value="<?php echo $vw_bill_ranap_detail_tmno->kode_dokter->LookupFilterQuery() ?>">
</span>
<?php echo $vw_bill_ranap_detail_tmno->kode_dokter->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->qty->Visible) { // qty ?>
	<div id="r_qty" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_qty" for="x_qty" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->qty->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->qty->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_qty">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_qty" name="x_qty" id="x_qty" size="1" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->qty->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->qty->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->qty->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tmno->qty->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->tarif->Visible) { // tarif ?>
	<div id="r_tarif" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_tarif" for="x_tarif" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->tarif->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->tarif->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_tarif">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_tarif" name="x_tarif" id="x_tarif" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->tarif->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->tarif->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->tarif->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tmno->tarif->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->bhp->Visible) { // bhp ?>
	<div id="r_bhp" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_bhp" for="x_bhp" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->bhp->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->bhp->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_bhp">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_bhp" name="x_bhp" id="x_bhp" size="5" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->bhp->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->bhp->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->bhp->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tmno->bhp->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->nama_tindakan->Visible) { // nama_tindakan ?>
	<div id="r_nama_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_nama_tindakan" for="x_nama_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->nama_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->nama_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_nama_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_nama_tindakan" name="x_nama_tindakan" id="x_nama_tindakan" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->nama_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->nama_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->nama_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tmno->nama_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->kelompok_tindakan->Visible) { // kelompok_tindakan ?>
	<div id="r_kelompok_tindakan" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_kelompok_tindakan" for="x_kelompok_tindakan" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->kelompok_tindakan->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->kelompok_tindakan->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_kelompok_tindakan">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_kelompok_tindakan" name="x_kelompok_tindakan" id="x_kelompok_tindakan" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->kelompok_tindakan->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->kelompok_tindakan->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->kelompok_tindakan->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tmno->kelompok_tindakan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->kelompok1->Visible) { // kelompok1 ?>
	<div id="r_kelompok1" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_kelompok1" for="x_kelompok1" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->kelompok1->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->kelompok1->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_kelompok1">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_kelompok1" name="x_kelompok1" id="x_kelompok1" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->kelompok1->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->kelompok1->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->kelompok1->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tmno->kelompok1->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->kelompok2->Visible) { // kelompok2 ?>
	<div id="r_kelompok2" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_kelompok2" for="x_kelompok2" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->kelompok2->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->kelompok2->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_kelompok2">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_kelompok2" name="x_kelompok2" id="x_kelompok2" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->kelompok2->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->kelompok2->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->kelompok2->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tmno->kelompok2->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->user->Visible) { // user ?>
	<div id="r_user" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_user" for="x_user" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->user->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->user->CellAttributes() ?>>
<span id="el_vw_bill_ranap_detail_tmno_user">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_user" name="x_user" id="x_user" size="1" maxlength="45" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->user->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->user->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->user->EditAttributes() ?>>
</span>
<?php echo $vw_bill_ranap_detail_tmno->user->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($vw_bill_ranap_detail_tmno->no_ruang->Visible) { // no_ruang ?>
	<div id="r_no_ruang" class="form-group">
		<label id="elh_vw_bill_ranap_detail_tmno_no_ruang" for="x_no_ruang" class="col-sm-2 control-label ewLabel"><?php echo $vw_bill_ranap_detail_tmno->no_ruang->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $vw_bill_ranap_detail_tmno->no_ruang->CellAttributes() ?>>
<?php if ($vw_bill_ranap_detail_tmno->no_ruang->getSessionValue() <> "") { ?>
<span id="el_vw_bill_ranap_detail_tmno_no_ruang">
...
<input type="hidden" id="x_no_ruang" name="x_no_ruang" value="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->no_ruang->CurrentValue) ?>">
<?php } else { ?>
<span id="el_vw_bill_ranap_detail_tmno_no_ruang">
<input type="text" data-table="vw_bill_ranap_detail_tmno" data-field="x_no_ruang" name="x_no_ruang" id="x_no_ruang" size="30" placeholder="<?php echo ew_HtmlEncode($vw_bill_ranap_detail_tmno->no_ruang->getPlaceHolder()) ?>" value="<?php echo $vw_bill_ranap_detail_tmno->no_ruang->EditValue ?>"<?php echo $vw_bill_ranap_detail_tmno->no_ruang->EditAttributes() ?>>
</span>
<?php } ?>
<?php echo $vw_bill_ranap_detail_tmno->no_ruang->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$vw_bill_ranap_detail_tmno_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vw_bill_ranap_detail_tmno_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvw_bill_ranap_detail_tmnoedit.Init();
</script>
<?php
$vw_bill_ranap_detail_tmno_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_bill_ranap_detail_tmno_edit->Page_Terminate();
?>
