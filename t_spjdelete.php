...
$t_spj_delete = NULL; // Initialize page object first

class ct_spj_delete extends ct_spj {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_spjlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_spj class, t_spjinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_spjlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_spj_delete)) $t_spj_delete = new ct_spj_delete();

// Page init
$t_spj_delete->Page_Init();

// Page main
$t_spj_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_spj_delete->Page_Render();
?>
...
var CurrentForm = ft_spjdelete = new ew_Form("ft_spjdelete", "delete");

// Form_CustomValidate event
ft_spjdelete.Form_CustomValidate = 
...
ft_spjdelete.ValidateRequired = true;
<?php } else { ?>
ft_spjdelete.ValidateRequired = false; 
<?php } ?>
...
<?php $t_spj_delete->ShowPageHeader(); ?>
<?php
$t_spj_delete->ShowMessage();
?>
<form name="ft_spjdelete" id="ft_spjdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_spj_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_spj_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_spj">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_spj_delete->RecKeys as $key) { ?>
...
<?php if ($t_spj->id->Visible) { // id ?>
		<th><span id="elh_t_spj_id" class="t_spj_id"><?php echo $t_spj->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_spj->no_spj->Visible) { // no_spj ?>
		<th><span id="elh_t_spj_no_spj" class="t_spj_no_spj"><?php echo $t_spj->no_spj->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_spj->tgl_spj->Visible) { // tgl_spj ?>
		<th><span id="elh_t_spj_tgl_spj" class="t_spj_tgl_spj"><?php echo $t_spj->tgl_spj->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_spj_delete->RecCnt = 0;
$i = 0;
while (!$t_spj_delete->Recordset->EOF) {
	$t_spj_delete->RecCnt++;
	$t_spj_delete->RowCnt++;

	// Set row properties
	$t_spj->ResetAttrs();
	$t_spj->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_spj_delete->LoadRowValues($t_spj_delete->Recordset);

	// Render row
	$t_spj_delete->RenderRow();
?>
	<tr<?php echo $t_spj->RowAttributes() ?>>
<?php if ($t_spj->id->Visible) { // id ?>
		<td<?php echo $t_spj->id->CellAttributes() ?>>
<div id="orig<?php echo $t_spj_delete->RowCnt ?>_t_spj_id" class="hide">
<span id="el<?php echo $t_spj_delete->RowCnt ?>_t_spj_id" class="t_spj_id">
...
<span id="el<?php echo $t_spj_delete->RowCnt ?>_t_spj_no_spj" class="t_spj_no_spj">
...
<span id="el<?php echo $t_spj_delete->RowCnt ?>_t_spj_tgl_spj" class="t_spj_tgl_spj">
...
<?php
	$t_spj_delete->Recordset->MoveNext();
}
$t_spj_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_spj_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_spjdelete.Init();
