...
class cMenu {
	...
	var $MenuLinkDropdownClass = "dropdown-toggle";
	...
	function Render($ret = FALSE) {
		...
		foreach ($this->ItemData as $item) {
			if ($this->RenderItem($item)) {
				$i++;
				if (!$this->IsMobile && $gtitle && ($gcnt >= 1 || $this->IsRoot)) $str .= "<li class=\"" . $this->MenuDividerClassName . "\"></li>\n"; // add divider for previous group
				if ($item->GroupTitle && (!$this->IsRoot || !$this->MenuRootGroupTitleAsSubMenu)) { // Group title
					...
					if (strval($item->Text) <> "") {
						if ($this->IsMobile) {
							...
							$str .= "<li class=\"treeview\"><a href=\"#\">" . $item->Text . "</a>\n";
						}
					}
					if (!is_null($item->SubMenu)) {
				...
				} else { // not a group title, regular link
					$gtitle = FALSE;
	...
	function Render($aclass = "", $liclass = "", $mobile = FALSE) {
		...
		if ($mobile) {
			...
			if (!is_null($this->SubMenu) && $this->SubMenu->MenuLinkDropdownClass <> "" && $submenuhtml <> "") {
				if (defined('EPI_TOPMENU') && EPI_TOPMENU == "Top") ew_PrependClass($aclass, $this->SubMenu->MenuLinkDropdownClass);
			}
			$attrs = array("class" => $aclass, "href" => $url, "target" => $this->Target);
			if (!is_null($this->SubMenu) && $this->SubMenu->MenuLinkDropdownClass <> "" && $submenuhtml <> "") {
				if (defined('EPI_TOPMENU') && EPI_TOPMENU == "Top") $attrs["data-toggle"] = 'dropdown';
			}
		}
		if (defined('EPI_TOPMENU') && EPI_TOPMENU == "Top") {
			$text = $this->Text;
		} else {
			if (substr($this->Text,0,2) !== '<i') {
				$text = '<i class="fa fa-circle-o"></i> ' . $this->Text;
			} else {
				$text = $this->Text;
			}
		}

//		$text = $this->Text; // force users to add <i> tags always
		if (!is_null($this->SubMenu) && $submenuhtml <> "") {
			if ($this->Parent->SubMenuDropdownIconClassName <> "")
				$text .= "<span class=\"" . $this->Parent->SubMenuDropdownIconClassName . "\"><i class=\"fa fa-angle-left pull-right\"></i></span>";
