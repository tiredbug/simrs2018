...
$training_view = NULL; // Initialize page object first

class ctraining_view extends ctraining {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "traininglist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "traininglist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "traininglist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($training_view)) $training_view = new ctraining_view();

// Page init
$training_view->Page_Init();

// Page main
$training_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$training_view->Page_Render();
?>
...
var CurrentForm = ftrainingview = new ew_Form("ftrainingview", "view");

// Form_CustomValidate event
ftrainingview.Form_CustomValidate = 
...
ftrainingview.ValidateRequired = true;
<?php } else { ?>
ftrainingview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftrainingview.Lists["x_namakegiatan"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_namakegiatan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"l_trainingkode"};
...
<?php if (!$training_view->IsModal) { ?>
<?php } ?>
<?php $training_view->ExportOptions->Render("body") ?>
<?php
	foreach ($training_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$training_view->IsModal) { ?>
...
<?php $training_view->ShowPageHeader(); ?>
<?php
$training_view->ShowMessage();
?>
<form name="ftrainingview" id="ftrainingview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($training_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $training_view->Token ?>">
<?php } ?>
...
<?php if ($training->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_training_id"><?php echo $training->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $training->id->CellAttributes() ?>>
<span id="el_training_id">
<span<?php echo $training->id->ViewAttributes() ?>>
<?php echo $training->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($training->namakegiatan->Visible) { // namakegiatan ?>
	<tr id="r_namakegiatan">
		<td><span id="elh_training_namakegiatan"><?php echo $training->namakegiatan->FldCaption() ?></span></td>
...
<?php echo $training->namakegiatan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($training->petugas->Visible) { // petugas ?>
	<tr id="r_petugas">
		<td><span id="elh_training_petugas"><?php echo $training->petugas->FldCaption() ?></span></td>
...
<?php echo $training->petugas->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($training->tanggal->Visible) { // tanggal ?>
	<tr id="r_tanggal">
		<td><span id="elh_training_tanggal"><?php echo $training->tanggal->FldCaption() ?></span></td>
...
<?php echo $training->tanggal->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
