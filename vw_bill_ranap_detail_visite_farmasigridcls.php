...
$vw_bill_ranap_detail_visite_farmasi_grid = NULL; // Initialize page object first

class cvw_bill_ranap_detail_visite_farmasi_grid extends cvw_bill_ranap_detail_visite_farmasi {
	...
	// Page object name
	var $PageObjName = 'vw_bill_ranap_detail_visite_farmasi_grid';

	// Grid form hidden field names
	var $FormName = 'fvw_bill_ranap_detail_visite_farmasigrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["vw_bill_ranap_detail_visite_farmasi"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_kode_tindakan") && $objForm->HasValue("o_kode_tindakan") && $this->kode_tindakan->CurrentValue <> $this->kode_tindakan->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_kode_farmasi") && $objForm->HasValue("o_kode_farmasi") && $this->kode_farmasi->CurrentValue <> $this->kode_farmasi->OldValue)
			return FALSE;
		...
		if ($objForm->HasValue("x_bhp") && $objForm->HasValue("o_bhp") && $this->bhp->CurrentValue <> $this->bhp->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nama_tindakan") && $objForm->HasValue("o_nama_tindakan") && $this->nama_tindakan->CurrentValue <> $this->nama_tindakan->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->kode_farmasi->setOldValue($objForm->GetValue("o_kode_farmasi"));
		...
		$this->nama_tindakan->setOldValue($objForm->GetValue("o_nama_tindakan"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "vw_bill_ranap") {
			$this->id_admission->Visible = FALSE;
			if ($GLOBALS["vw_bill_ranap"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->nomr->Visible = FALSE;
			if ($GLOBALS["vw_bill_ranap"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->statusbayar->Visible = FALSE;
			if ($GLOBALS["vw_bill_ranap"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->kelas->Visible = FALSE;
			if ($GLOBALS["vw_bill_ranap"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->no_ruang->Visible = FALSE;
			if ($GLOBALS["vw_bill_ranap"]->EventCancelled) $this->EventCancelled = TRUE;
		}
