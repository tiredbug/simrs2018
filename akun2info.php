...
//
// Table class for akun2
//
class cakun2 extends cTable {
	var $kel1;
	var $kel2;
	var $kel3;
	var $nmkel2;
	...
	function __construct() {
		...
		$this->TableVar = 'akun2';
		$this->TableName = 'akun2';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`akun2`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kel1
		$this->kel1 = new cField('akun2', 'akun2', 'x_kel1', 'kel1', '`kel1`', '`kel1`', 200, -1, FALSE, '`kel1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel1->Sortable = TRUE; // Allow sort
		$this->fields['kel1'] = &$this->kel1;

		// kel2
		$this->kel2 = new cField('akun2', 'akun2', 'x_kel2', 'kel2', '`kel2`', '`kel2`', 200, -1, FALSE, '`kel2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel2->Sortable = TRUE; // Allow sort
		$this->fields['kel2'] = &$this->kel2;

		// kel3
		$this->kel3 = new cField('akun2', 'akun2', 'x_kel3', 'kel3', '`kel3`', '`kel3`', 200, -1, FALSE, '`kel3`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel3->Sortable = TRUE; // Allow sort
		$this->fields['kel3'] = &$this->kel3;

		// nmkel2
		$this->nmkel2 = new cField('akun2', 'akun2', 'x_nmkel2', 'nmkel2', '`nmkel2`', '`nmkel2`', 200, -1, FALSE, '`nmkel2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`akun2`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kel3', $rs))
				ew_AddFilter($where, ew_QuotedName('kel3', $this->DBID) . '=' . ew_QuotedValue($rs['kel3'], $this->kel3->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@kel3@", ew_AdjustSql($this->kel3->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("akun2view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("akun2view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "akun2add.php?" . $this->UrlParm($parm);
		else
			$url = "akun2add.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("akun2edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("akun2add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("akun2delete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kel3:" . ew_VarToJson($this->kel3->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kel3->CurrentValue)) {
			$sUrl .= "kel3=" . urlencode($this->kel3->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["kel3"]))
				$arKeys[] = ew_StripSlashes($_POST["kel3"]);
			elseif (isset($_GET["kel3"]))
				$arKeys[] = ew_StripSlashes($_GET["kel3"]);
			else
