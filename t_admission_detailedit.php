...
$t_admission_detail_edit = NULL; // Initialize page object first

class ct_admission_detail_edit extends ct_admission_detail {
	...
	// Page object name
	var $PageObjName = 't_admission_detail_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("t_admission_detaillist.php"); // Invalid key, return to list
		}
...
// Create page object
if (!isset($t_admission_detail_edit)) $t_admission_detail_edit = new ct_admission_detail_edit();

// Page init
$t_admission_detail_edit->Page_Init();

// Page main
$t_admission_detail_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_admission_detail_edit->Page_Render();
?>
...
var CurrentForm = ft_admission_detailedit = new ew_Form("ft_admission_detailedit", "edit");

// Validate form
ft_admission_detailedit.Validate = function() {
...
ft_admission_detailedit.Form_CustomValidate = 
...
ft_admission_detailedit.ValidateRequired = true;
<?php } else { ?>
ft_admission_detailedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$t_admission_detail_edit->IsModal) { ?>
<?php } ?>
<?php $t_admission_detail_edit->ShowPageHeader(); ?>
<?php
$t_admission_detail_edit->ShowMessage();
?>
<form name="ft_admission_detailedit" id="ft_admission_detailedit" class="<?php echo $t_admission_detail_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_admission_detail_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_admission_detail_edit->Token ?>">
<?php } ?>
...
<?php if ($t_admission_detail->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_t_admission_detail_id" class="col-sm-2 control-label ewLabel"><?php echo $t_admission_detail->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $t_admission_detail->id->CellAttributes() ?>>
<span id="el_t_admission_detail_id">
<span<?php echo $t_admission_detail->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $t_admission_detail->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="t_admission_detail" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($t_admission_detail->id->CurrentValue) ?>">
<?php echo $t_admission_detail->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_admission_detail_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft_admission_detailedit.Init();
</script>
<?php
$t_admission_detail_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_admission_detail_edit->Page_Terminate();
?>
