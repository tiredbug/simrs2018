...
//
// Table class for akun5
//
class cakun5 extends cTable {
	var $kel1;
	var $kel2;
	var $kel3;
	var $kel41;
	...
	function __construct() {
		...
		$this->TableVar = 'akun5';
		$this->TableName = 'akun5';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`akun5`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kel1
		$this->kel1 = new cField('akun5', 'akun5', 'x_kel1', 'kel1', '`kel1`', '`kel1`', 200, -1, FALSE, '`kel1`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel1->Sortable = TRUE; // Allow sort
		$this->fields['kel1'] = &$this->kel1;

		// kel2
		$this->kel2 = new cField('akun5', 'akun5', 'x_kel2', 'kel2', '`kel2`', '`kel2`', 200, -1, FALSE, '`kel2`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel2->Sortable = TRUE; // Allow sort
		$this->fields['kel2'] = &$this->kel2;

		// kel3
		$this->kel3 = new cField('akun5', 'akun5', 'x_kel3', 'kel3', '`kel3`', '`kel3`', 200, -1, FALSE, '`kel3`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel3->Sortable = TRUE; // Allow sort
		$this->fields['kel3'] = &$this->kel3;

		// kel41
		$this->kel41 = new cField('akun5', 'akun5', 'x_kel41', 'kel41', '`kel41`', '`kel41`', 200, -1, FALSE, '`kel41`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel41->Sortable = TRUE; // Allow sort
		$this->fields['kel41'] = &$this->kel41;

		// kel4
		$this->kel4 = new cField('akun5', 'akun5', 'x_kel4', 'kel4', '`kel4`', '`kel4`', 200, -1, FALSE, '`kel4`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel4->Sortable = TRUE; // Allow sort
		$this->fields['kel4'] = &$this->kel4;

		// kel5
		$this->kel5 = new cField('akun5', 'akun5', 'x_kel5', 'kel5', '`kel5`', '`kel5`', 200, -1, FALSE, '`kel5`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel5->Sortable = TRUE; // Allow sort
		$this->fields['kel5'] = &$this->kel5;

		// kel6
		$this->kel6 = new cField('akun5', 'akun5', 'x_kel6', 'kel6', '`kel6`', '`kel6`', 200, -1, FALSE, '`kel6`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kel6->Sortable = TRUE; // Allow sort
		$this->fields['kel6'] = &$this->kel6;

		// nmkel5
		$this->nmkel5 = new cField('akun5', 'akun5', 'x_nmkel5', 'nmkel5', '`nmkel5`', '`nmkel5`', 200, -1, FALSE, '`nmkel5`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nmkel5->Sortable = TRUE; // Allow sort
		$this->fields['nmkel5'] = &$this->nmkel5;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`akun5`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('kel6', $rs))
				ew_AddFilter($where, ew_QuotedName('kel6', $this->DBID) . '=' . ew_QuotedValue($rs['kel6'], $this->kel6->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@kel6@", ew_AdjustSql($this->kel6->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("akun5view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("akun5view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "akun5add.php?" . $this->UrlParm($parm);
		else
			$url = "akun5add.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("akun5edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("akun5add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("akun5delete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "kel6:" . ew_VarToJson($this->kel6->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kel6->CurrentValue)) {
			$sUrl .= "kel6=" . urlencode($this->kel6->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["kel6"]))
				$arKeys[] = ew_StripSlashes($_POST["kel6"]);
			elseif (isset($_GET["kel6"]))
				$arKeys[] = ew_StripSlashes($_GET["kel6"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->kel41->EditValue = $this->kel41->CurrentValue;
		...
		$this->nmkel5->EditValue = $this->nmkel5->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->kel41->Exportable) $Doc->ExportCaption($this->kel41);
					if ($this->kel4->Exportable) $Doc->ExportCaption($this->kel4);
					if ($this->kel5->Exportable) $Doc->ExportCaption($this->kel5);
					if ($this->kel6->Exportable) $Doc->ExportCaption($this->kel6);
					if ($this->nmkel5->Exportable) $Doc->ExportCaption($this->nmkel5);
				} else {
					if ($this->kel1->Exportable) $Doc->ExportCaption($this->kel1);
					if ($this->kel2->Exportable) $Doc->ExportCaption($this->kel2);
					if ($this->kel3->Exportable) $Doc->ExportCaption($this->kel3);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->kel41->Exportable) $Doc->ExportField($this->kel41);
						if ($this->kel4->Exportable) $Doc->ExportField($this->kel4);
						if ($this->kel5->Exportable) $Doc->ExportField($this->kel5);
						if ($this->kel6->Exportable) $Doc->ExportField($this->kel6);
						if ($this->nmkel5->Exportable) $Doc->ExportField($this->nmkel5);
					} else {
						if ($this->kel1->Exportable) $Doc->ExportField($this->kel1);
						if ($this->kel2->Exportable) $Doc->ExportField($this->kel2);
						if ($this->kel3->Exportable) $Doc->ExportField($this->kel3);
