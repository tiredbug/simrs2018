...
//
// Table class for fake_sales
//
class cfake_sales extends cTable {
	var $sales_id;
	var $sales_month;
	var $sales_total;
	var $sales_category;
	...
	function __construct() {
		...
		$this->TableVar = 'fake_sales';
		$this->TableName = 'fake_sales';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`fake_sales`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// sales_id
		$this->sales_id = new cField('fake_sales', 'fake_sales', 'x_sales_id', 'sales_id', '`sales_id`', '`sales_id`', 3, -1, FALSE, '`sales_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// sales_month
		$this->sales_month = new cField('fake_sales', 'fake_sales', 'x_sales_month', 'sales_month', '`sales_month`', '`sales_month`', 3, -1, FALSE, '`sales_month`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sales_month->Sortable = TRUE; // Allow sort
		$this->sales_month->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sales_month'] = &$this->sales_month;

		// sales_total
		$this->sales_total = new cField('fake_sales', 'fake_sales', 'x_sales_total', 'sales_total', '`sales_total`', '`sales_total`', 3, -1, FALSE, '`sales_total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sales_total->Sortable = TRUE; // Allow sort
		$this->sales_total->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sales_total'] = &$this->sales_total;

		// sales_category
		$this->sales_category = new cField('fake_sales', 'fake_sales', 'x_sales_category', 'sales_category', '`sales_category`', '`sales_category`', 200, -1, FALSE, '`sales_category`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sales_category->Sortable = TRUE; // Allow sort
		$this->fields['sales_category'] = &$this->sales_category;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`fake_sales`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("fake_salesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("fake_salesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "fake_salesadd.php?" . $this->UrlParm($parm);
		else
			$url = "fake_salesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("fake_salesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("fake_salesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("fake_salesdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->sales_month->EditValue = $this->sales_month->CurrentValue;
		...
		$this->sales_total->EditValue = $this->sales_total->CurrentValue;
		...
		$this->sales_category->EditValue = $this->sales_category->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->sales_month->Exportable) $Doc->ExportCaption($this->sales_month);
					if ($this->sales_total->Exportable) $Doc->ExportCaption($this->sales_total);
					if ($this->sales_category->Exportable) $Doc->ExportCaption($this->sales_category);
				} else {
					if ($this->sales_id->Exportable) $Doc->ExportCaption($this->sales_id);
					if ($this->sales_month->Exportable) $Doc->ExportCaption($this->sales_month);
					if ($this->sales_total->Exportable) $Doc->ExportCaption($this->sales_total);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->sales_month->Exportable) $Doc->ExportField($this->sales_month);
						if ($this->sales_total->Exportable) $Doc->ExportField($this->sales_total);
						if ($this->sales_category->Exportable) $Doc->ExportField($this->sales_category);
					} else {
						if ($this->sales_id->Exportable) $Doc->ExportField($this->sales_id);
						if ($this->sales_month->Exportable) $Doc->ExportField($this->sales_month);
						if ($this->sales_total->Exportable) $Doc->ExportField($this->sales_total);
