...
$l_jenis_detail_spp_grid = NULL; // Initialize page object first

class cl_jenis_detail_spp_grid extends cl_jenis_detail_spp {
	...
	// Page object name
	var $PageObjName = 'l_jenis_detail_spp_grid';

	// Grid form hidden field names
	var $FormName = 'fl_jenis_detail_sppgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["l_jenis_detail_spp"];
	...
	function Page_Init() {
		...
		if (@$_POST["ajax"] == "autofill") {
			...
			// Process auto fill for detail table 'vw_spp_ls_gaji_tunjangan'
			if (@$_POST["grid"] == "fvw_spp_ls_gaji_tunjangangrid") {
				if (!isset($GLOBALS["vw_spp_ls_gaji_tunjangan_grid"])) $GLOBALS["vw_spp_ls_gaji_tunjangan_grid"] = new cvw_spp_ls_gaji_tunjangan_grid;
				$GLOBALS["vw_spp_ls_gaji_tunjangan_grid"]->Page_Init();
				$this->Page_Terminate();
				exit();
			}
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "l_jenis_spp") {
			global $l_jenis_spp;
			$rsmaster = $l_jenis_spp->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("l_jenis_spplist.php"); // Return to master page
			} else {
				$l_jenis_spp->LoadListRowValues($rsmaster);
				$l_jenis_spp->RowType = EW_ROWTYPE_MASTER; // Master row
				$l_jenis_spp->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_detail_jenis_spp") && $objForm->HasValue("o_detail_jenis_spp") && $this->detail_jenis_spp->CurrentValue <> $this->detail_jenis_spp->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->detail_jenis_spp->setOldValue($objForm->GetValue("o_detail_jenis_spp"));
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Check referential integrity for master table 'l_jenis_spp'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_l_jenis_spp();
			$KeyValue = isset($rsnew['id_jenis']) ? $rsnew['id_jenis'] : $rsold['id_jenis'];
			if (strval($KeyValue) <> "") {
			...
			if ($bValidMasterRecord) {
				if (!isset($GLOBALS["l_jenis_spp"])) $GLOBALS["l_jenis_spp"] = new cl_jenis_spp();
				$rsmaster = $GLOBALS["l_jenis_spp"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "l_jenis_spp", $Language->Phrase("RelatedRecordRequired"));
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "l_jenis_spp") {
				$this->id_jenis->CurrentValue = $this->id_jenis->getSessionValue();
			}

		// Check referential integrity for master table 'l_jenis_spp'
		$bValidMasterRecord = TRUE;
		$sMasterFilter = $this->SqlMasterFilter_l_jenis_spp();
		if ($this->id_jenis->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@id@", ew_AdjustSql($this->id_jenis->getSessionValue(), "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// id_jenis
		if ($this->id_jenis->getSessionValue() <> "") {
			$rsnew['id_jenis'] = $this->id_jenis->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "l_jenis_spp") {
			$this->id_jenis->Visible = FALSE;
			if ($GLOBALS["l_jenis_spp"]->EventCancelled) $this->EventCancelled = TRUE;
		}
