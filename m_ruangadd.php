...
$m_ruang_add = NULL; // Initialize page object first

class cm_ruang_add extends cm_ruang {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "m_ruangview.php")
	...
	function LoadDefaultValues() {
		...
		$this->ruang->CurrentValue = NULL;
		$this->ruang->OldValue = $this->ruang->CurrentValue;
		$this->jumlah_tt->CurrentValue = NULL;
		$this->jumlah_tt->OldValue = $this->jumlah_tt->CurrentValue;
		$this->ket_ruang->CurrentValue = NULL;
		$this->ket_ruang->OldValue = $this->ket_ruang->CurrentValue;
		$this->fasilitas->CurrentValue = NULL;
		$this->fasilitas->OldValue = $this->fasilitas->CurrentValue;
		$this->keterangan->CurrentValue = NULL;
		$this->keterangan->OldValue = $this->keterangan->CurrentValue;
		$this->kepala_ruangan->CurrentValue = NULL;
		$this->kepala_ruangan->OldValue = $this->kepala_ruangan->CurrentValue;
		$this->nip_kepala_ruangan->CurrentValue = NULL;
		$this->nip_kepala_ruangan->OldValue = $this->nip_kepala_ruangan->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// ruang
		$this->ruang->SetDbValueDef($rsnew, $this->ruang->CurrentValue, "", FALSE);

		// jumlah_tt
		$this->jumlah_tt->SetDbValueDef($rsnew, $this->jumlah_tt->CurrentValue, 0, FALSE);

		// ket_ruang
		$this->ket_ruang->SetDbValueDef($rsnew, $this->ket_ruang->CurrentValue, "", FALSE);

		// fasilitas
		$this->fasilitas->SetDbValueDef($rsnew, $this->fasilitas->CurrentValue, "", FALSE);

		// keterangan
		$this->keterangan->SetDbValueDef($rsnew, $this->keterangan->CurrentValue, "", FALSE);

		// kepala_ruangan
		$this->kepala_ruangan->SetDbValueDef($rsnew, $this->kepala_ruangan->CurrentValue, NULL, FALSE);

		// nip_kepala_ruangan
		$this->nip_kepala_ruangan->SetDbValueDef($rsnew, $this->nip_kepala_ruangan->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($m_ruang_add)) $m_ruang_add = new cm_ruang_add();

// Page init
$m_ruang_add->Page_Init();

// Page main
$m_ruang_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$m_ruang_add->Page_Render();
?>
...
var CurrentForm = fm_ruangadd = new ew_Form("fm_ruangadd", "add");

// Validate form
fm_ruangadd.Validate = function() {
...
fm_ruangadd.Form_CustomValidate = 
...
fm_ruangadd.ValidateRequired = true;
<?php } else { ?>
fm_ruangadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$m_ruang_add->IsModal) { ?>
<?php } ?>
<?php $m_ruang_add->ShowPageHeader(); ?>
<?php
$m_ruang_add->ShowMessage();
?>
<form name="fm_ruangadd" id="fm_ruangadd" class="<?php echo $m_ruang_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($m_ruang_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $m_ruang_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $m_ruang_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
